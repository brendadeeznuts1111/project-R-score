...
- ✅ Separate from environment variables (reduces exposure risk)
- ✅ OS-native encryption (platform-specific secure storage)
- ✅ Audit trail (secrets access can be logged)
...
- ⚠️ May appear in logs or error messages
- ⚠️ Accessible to all processes with same environment
...
3. Grant bot permissions (critical for functionality):
   - ✅ **Can send messages** - Required for all notifications
   - ✅ **Can pin messages** - Required for critical alerts (e.g., `CovertSteamEvent`)
   - ✅ **Can manage topics** - Required for topic-based routing
   - ✅ **Can delete messages** - Optional, useful for cleanup
   - ❌ **Can restrict members** - Not required for golden config
...
- ✅ Version-controlled topic configuration
- ✅ Reproducible deployments
- ✅ Automatic `message_thread_id` capture for storage
...
- ✅ Runtime topic ID resolution by name
...
bun run telegram send "🧪 Test message for Covert Steam" --thread-id 2 --priority high --bookmaker "DraftKings"

# Or using --topic alias
bun run telegram send "🧪 Test message for Covert Steam" --topic 2

# Send and pin
bun run telegram send "📌 Pinned test message" --thread-id 2 --pin
...
curl -X POST http://localhost:3001/telegram/topics/2/send \
  -H "Content-Type: application/json" \
  -d '{"message": "🧪 Test message from API", "pin": false}'
```
...
const result = await bot.sendMessage(chatId, "🧪 Test message", 2); // 2 = message_thread_id

// Send and pin
await bot.sendAndPin(chatId, "📌 Pinned message", 2);
```
...
bun run telegram send "📌 Test pinned message" --thread-id 2 --pin
...
bun run telegram send "📌 Pinned message" --topic 2 --pin
```
...
**Strategic Benefits**:
- ✅ Reduced context switching (developers stay in GitHub)
- ✅ Immediate visibility of production issues in PR context
- ✅ Automated linking of alerts to code changes
- ✅ Bi-directional traceability (Telegram ↔ GitHub)
...
class GitHubTelegramBridge {
  ...
  async notifyCovertSteamLinked(prNumber: number, eventId: string): Promise<void> {
    ...
    await telegramClient.sendMessage({
      chatId: await Bun.secrets.get({ service: "nexus", name: "telegram.chatId" }),
      text: `🔴 Covert Steam Event Linked to PR #${prNumber}\n\nEvent ID: ${eventId}\nPR: https://github.com/org/repo/pull/${prNumber}`,
...
2. Grant permissions:
   ...
   - ✅ **Workflows**: Write (for workflow dispatch)
...
const telegramMessage = `🚨 Performance Regression Detected

Function: ${functionName}
Delta: ${delta}%

🔗 GitHub PR: ${prUrl}
📊 Full Details: ${telegramTopicUrl}`;
```
...
|-----------|------|----------|-------------|---------|
| `id` | string | ✅ Yes | Unique identifier for the specific alert instance, ensuring traceability | `covert-steam-20250106-123456` |
...
| Parameter | Type | Required | Description | Example |
|-----------|------|----------|-------------|---------|
| `bm` | string | ❌ No | If applicable, the affected bookmaker | `DraftKings`, `Betfair` |
| `ev` | string | ❌ No | If applicable, the sports event | `NFL-2025-001` |
| `node` | string | ❌ No | For market node-specific alerts | `node_abc123` |
...
- ⚠️: High severity alerts (severity 7-8)
- 📈: Market move alerts
- 🛠️: Debug/System alerts
...
**Ripgrep**: `rg "categoryEmoji|🚨|⚠️|📈" src/telegram/`
...
const formattedMessage = `<b>🚨 CRITICAL: Covert Steam Detected</b>
...
**Benefits**:
- ✅ **Visual Hierarchy**: Bold headers draw immediate attention
- ✅ **Code Formatting**: Event IDs and identifiers in monospace for clarity
- ✅ **Direct Links**: Clickable links to Hyper-Bun dashboard
- ✅ **Structured Data**: Italicized labels improve readability
...
parse_mode: HTML
template: |
  <b>🚨 CRITICAL: Covert Steam Detected</b>
...
parse_mode: HTML
template: |
  <b>⚠️ Performance Regression Detected</b>
...
- ✅ **Fast Startup**: Templates compiled once at startup
- ✅ **Zero Runtime Overhead**: Pre-compiled templates execute instantly
- ✅ **Memory Efficient**: Bun's JavaScript engine optimizes compiled templates
- ✅ **Type Safety**: TypeScript types for template data ensure correctness
...
const inlineKeyboard = {
    inline_keyboard: [[
        { text: '✅ Acknowledge', callback_data: callbackData },
        { text: '📊 View Dashboard', url: 'https://nexus.example.com/events/evt_1234567890' }
    ]]
};
```
...
- ✅ **Policy Enforcement**: Consistent routing rules across all subsystems
- ✅ **Priority Management**: High-priority messages processed first
- ✅ **Abstraction**: Subsystems don't need to know Telegram API details
- ✅ **Observability**: Centralized logging and metrics for all messages
- ✅ **Dynamic Routing**: Bookmaker- and market-specific routing based on alert context
- ✅ **Intelligent Targeting**: Ensures alerts reach the most relevant specialists
...
**Routing Rules**:
- **CovertSteamEventDetector** → `OPERATIONS` chat → `CRITICAL_ALERTS` topic
- **UrlAnomalyPatternEngine** → `RESEARCH` chat → `ANOMALY_DETECTION` topic
- **PerformanceMonitor** → `OPERATIONS` chat → `PERFORMANCE_ALERTS` topic
- **ArbitrageScanner** → `OPERATIONS` chat → `ARBITRAGE_OPPORTUNITIES` topic
...
**Severity Mapping**:
- **CRITICAL** (9-10) → `PERFORMANCE_ALERTS` topic (auto-pinned)
- **HIGH** (7-8) → `MARKET_ANOMALIES` topic
- **MEDIUM** (4-6) → `ANALYTICS` topic
- **LOW** (1-3) → `GENERAL` topic
...
**Use Cases**:
- ✅ **Dedicated Bookmaker Channels**: DraftKings alerts → DraftKings-specific channel
- ✅ **Bookmaker-Specific Topics**: Betfair alerts → Betfair topic within operations chat
- ✅ **Team Specialization**: Different teams monitor different bookmakers
- ✅ **Reduced Noise**: Operators only see alerts for their assigned bookmakers
...
**Use Cases**:
- ✅ **Event-Specific Topics**: All alerts for `nfl-2025-001` → `Event-nfl-2025-001` topic
- ✅ **Market-Specific Topics**: Alerts for specific market nodes → Dedicated topic per market
- ✅ **Granular Investigation**: Operators can focus on a single event/market
- ✅ **Historical Context**: All related alerts in one topic for easy review
...
**Bun Performance Benefits**:
- ✅ **Fast Async**: Bun's optimized async/await handles concurrent messages efficiently
- ✅ **Low Overhead**: Minimal queue management overhead
- ✅ **Scalable**: Can process hundreds of messages per second
...
**Challenges**:
- ✅ **Telegram API Rate Limits**: Telegram enforces strict rate limits (e.g., 30 messages per second per bot, 20 messages per minute per chat)
- ✅ **High Alert Volume**: Market volatility can generate hundreds of alerts per minute
- ✅ **Resource Exhaustion**: Uncontrolled message sending can overwhelm Hyper-Bun's resources
- ✅ **Critical Alert Guarantee**: Critical alerts must always get through, even during high load
...
**Bun Performance Benefits**:
- ✅ **Fast Arrays**: Bun's optimized JavaScript engine handles large arrays efficiently
- ✅ **Native Sleep**: `Bun.sleep()` for precise timing control
- ✅ **Async Processing**: Non-blocking queue processing maintains responsiveness
...
**Strategic Value**:
- ✅ **Mobile-First Trading**: Execute trades from anywhere via Telegram mobile app
- ✅ **Real-Time Intelligence**: Access Hyper-Bun's latest market intelligence instantly
- ✅ **Reduced Latency**: Eliminate context switching between alert and trading platform
- ✅ **Operational Agility**: Respond to `CovertSteamEvent`s and arbitrage opportunities within seconds
- ✅ **Unified Interface**: Single point of control for monitoring, alerts, and trading
...
```
miniapp-standalone/
├── index.html          # Main TMA entry point
├── config.js           # Environment configuration (injected at build)
├── assets/
│   ├── css/
│   │   └── trading-ui.css
│   ├── js/
...
1. **Bot Token Configuration (`9.1.1.1.1.0.0`) → Secrets Management**:
...
2. **Deep-Link RFC (`9.1.1.9.1.0.0`) → RFC Document**:
...
3. **MCP Alerting Integration (`9.1.1.10.4.0.0`) → Alerting Subsystem**:
...
3. **UI Policy Integration (`9.1.1.10.2.3.1`) → Policy Management** (Cross-reference: `4.1.1.3.3.0.0` Feature Flag Management Policies & `config/telegram-bookmaker-map.yaml`):
...
4. **Dashboard Integration (`9.1.3.0.0.0`) → HTMLRewriter**:
...
5. **Sub-Market Integration (`9.1.1.10.2.4.2`) → Research System**:
...
**Benefits**:
- ✅ **Knowledge Graph**: Developers can navigate from Telegram setup to related systems
- ✅ **Context Discovery**: Related documentation is always accessible
- ✅ **Consistency**: Cross-references ensure documentation stays synchronized
- ✅ **Onboarding**: New developers can follow links to understand the full system
...
**Status Metrics**:
- ✅ **Connection Health**: Connection status to Telegram API (connected/disconnected/error)
- ✅ **API Rate Limit Usage**: Current rate limit consumption (e.g., "15/30 messages per second")
- ✅ **Queue Size**: Current message queue length (`messageQueue.length`)
- ✅ **Prioritized/Dropped Messages**: Count of messages prioritized or dropped due to throttling (from `9.1.1.10.3.0.0`)
- ✅ **Last Message Sent**: Timestamp of last successfully sent message
- ✅ **Error Rate**: Percentage of failed message sends
...
- ✅ **Bookmaker Mapping Table**: Shows all bookmaker → chat/topic mappings
- ✅ **Severity Mapping Table**: Shows severity levels → topic mappings
- ✅ **Routing Flow Diagram**: Visual representation of routing precedence
- ✅ **Edit Controls**: (Admin only) Inline editing of routing configurations
...
<div id="telegram-routing-config" data-feature="telegram-routing" data-access="admin">
  <h3>🔀 Telegram Routing Configuration</h3>
...
- ✅ **Sample Link Generation**: Generate example deep-link for selected routing rule
- ✅ **Link Validation**: Verify deep-link format matches RFC 001 specification
- ✅ **Clickable Preview**: Open sample deep-link in new tab to test dashboard route
- ✅ **Parameter Display**: Show all query parameters that would be included
...
**Features**:
- ✅ **Test Message Form**: Input fields for bookmaker, severity, message text
- ✅ **Topic Selection**: Dropdown to select target topic
- ✅ **Send Button**: Triggers notification via API
- ✅ **Result Display**: Shows success/failure and message ID
- ✅ **Deep-Link Validation**: Generates and validates deep-link per RFC 001 before sending (`9.1.1.9.1.0.0`)
...
- ✅ **Rate Limits**: Bot messages per second, chat messages per minute
- ✅ **Queue Thresholds**: Maximum queue size, dropping thresholds per severity
- ✅ **Summarization Thresholds**: When to summarize vs. drop messages
- ✅ **Backpressure Settings**: Queue size limits, drop policies
...
**Security Benefits**:
- ✅ Encrypted credential storage (OS-native)
- ✅ Separation from environment variables
...
**Key Benefits**:
- ✅ **Reduced Context Switching**: Developers stay in GitHub, receive Telegram alerts as GitHub comments
- ✅ **Immediate Visibility**: Production issues appear directly in PR context
- ✅ **Automated Traceability**: Alerts automatically linked to code changes
- ✅ **Bi-directional Linking**: Telegram ↔ GitHub cross-references
- ✅ **Faster Resolution**: Developers see issues where they work (GitHub)
...
**Integration Points**:
- CI/CD workflows → Telegram topics → GitHub PR comments
- Performance regressions → Telegram alerts → GitHub checks
- `CovertSteamEvent` detection → Telegram alerts → GitHub issue creation
- System status → Telegram topics → GitHub status updates
...
**Key Benefits**:
- ✅ **Visual Clarity**: HTML formatting creates clear visual hierarchy
- ✅ **Structured Data**: Templates ensure consistent, complete information in every alert
- ✅ **Immediate Actions**: Inline buttons enable one-click responses (acknowledge, rerun, pause)
- ✅ **Deep-Linked Navigation**: RFC 001 standardized deep-links transform alerts into actionable entry points (`9.1.1.9.1.0.0`)
- ✅ **Context-Rich**: Links to dashboard, GitHub PRs, and related resources embedded directly
- ✅ **Traceable Intelligence**: Every alert includes traceable deep-link with full context parameters
- ✅ **Reduced Cognitive Load**: Operators don't need to switch tools to understand or act on alerts
...
**Key Benefits**:
- ✅ **Consistent Routing**: All messages follow the same routing policies
- ✅ **Priority Handling**: Critical alerts processed ahead of lower-priority messages
- ✅ **Source-Based Routing**: Subsystems automatically route to correct chats/topics
- ✅ **Severity-Based Topics**: Alerts organized by severity for better operational awareness
- ✅ **Centralized Observability**: All message routing logged and monitored in one place
- ✅ **Abstraction**: Subsystems don't need Telegram API knowledge
...
**Routing Policies**:
- **Source-Based**: CovertSteamEventDetector → OPERATIONS chat, UrlAnomalyPatternEngine → RESEARCH chat
- **Severity-Based**: CRITICAL → PERFORMANCE_ALERTS topic (auto-pinned), HIGH → MARKET_ANOMALIES topic
...
**Key Benefits**:
- ✅ **Specialized Teams**: Different teams monitor different bookmakers without cross-contamination
- ✅ **Granular Focus**: Market-specific topics enable deep investigation of individual events/markets
- ✅ **Reduced Noise**: Operators only see alerts relevant to their specialization
- ✅ **Faster Response**: Targeted routing means alerts reach the right person immediately
- ✅ **Historical Context**: All related alerts for a market/event in one topic for easy review
- ✅ **Scalable Organization**: As Hyper-Bun monitors more bookmakers, routing scales automatically
...
**Routing Precedence**:
1. **Market-Specific** (highest): `nodeId` or `event_identifier` + `market_internal_id` → Dedicated topic
2. **Bookmaker-Specific**: `bookmaker_name` → Bookmaker channel/topic
3. **Severity-Based**: `severity` → Severity-based topic (CRITICAL, HIGH, MEDIUM, LOW)
4. **Default** (fallback): General operations topic

**Use Cases**:
- **Covert Steam Detection**: DraftKings deceptive line → DraftKings channel → DraftKings Alerts topic
- **Market Anomaly**: `nfl-2025-001:total_q1:draftkings:q1` anomaly → `Event-nfl-2025-001-Market-total_q1` topic
- **Arbitrage Opportunity**: Betfair concealed arb → Betfair channel → Arbitrage Opportunities topic
- **Performance Regression**: Function-specific regression → Performance Alerts topic (severity-based)
...
**Key Benefits**:
- ✅ **Documentation Hub Integration**: Seamless navigation between Telegram setup and related systems (MCP, UI policies, research) (`9.1.2.0.0.0.0`)
- ✅ **UI Feedback & Control**: Real-time status display, routing visualization, and dynamic throttling policy adjustment (`9.1.3.0.0.0.0`)
- ✅ **Load Balancing**: Intelligent throttling and prioritization for system stability under high alert volumes (`9.1.1.10.3.0.0`)
- ✅ **Policy Integration**: Telegram routing policies integrated with `UIPolicyManager` for centralized management (`8.1.3.3.0.0.0`)
- ✅ **Self-Documenting**: Cross-references create a knowledge graph within Hyper-Bun's documentation (`9.1.2.2.0.0.0`)
...
The full-featured Telegram Mini App (TMA) provides unprecedented operational and trading agility. Operators can now execute Hyper-Bun's most critical function — trading based on detected intelligence — directly from their mobile device within Telegram, drastically cutting down the time from alert to action. This represents a significant competitive advantage and a pinnacle of integrated intelligence execution.

**Strategic Advantages**:
- ✅ **Mobile-First Trading**: Execute trades from anywhere via Telegram mobile app
- ✅ **Zero Context Switching**: Alert → Analysis → Execution all within Telegram
- ✅ **Real-Time Intelligence Access**: Instant access to latest market intelligence and opportunities
- ✅ **Reduced Latency**: Eliminate delays between alert detection and trade execution
- ✅ **Unified Operational Hub**: Single interface for monitoring, alerts, and trading
