...
if bun run typecheck 2>&1 | rg -q "error" || [ $? -ne 0 ]; then
...
if [ -f "test/ticks/fuzz-correlation.test.ts" ]; then
    ...
    if bun test test/ticks/fuzz-correlation.test.ts 2>&1 | tee /tmp/fuzz-result.txt | rg -q "passing|pass"; then
        PASS_COUNT=$(rg -o "(\d+) passing" /tmp/fuzz-result.txt | rg -o "\d+" | head -1 || echo "0")
...
if rg -q "layer4_tick_correlation" src/api/routes/17.16.7-market-patterns.ts; then
...
if rg -q "handleLayer4TickCorrelation" src/api/routes/17.16.7-market-patterns.ts; then
