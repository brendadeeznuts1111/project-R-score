...
export class ContentTypeDemo {
  ...
  async demonstrateBlobContentType(): Promise<Response> {
    console.info('📦 Demonstrating Blob Content-Type handling...');
    ...
    console.info(`Text blob type: ${textBlob.type}`);
    console.info(`JSON blob type: ${jsonBlob.type}`);
    console.info(`HTML blob type: ${htmlBlob.type}`);
    ...
    console.info(`Blob POST response status: ${response.status}`);
  ...
  async demonstrateFormDataContentType(): Promise<Response> {
    console.info('📋 Demonstrating FormData Content-Type handling...');
    ...
    console.info(`FormData POST response status: ${response.status}`);
  ...
  async demonstrateConnectionPooling(): Promise<void> {
    console.info('🔗 Demonstrating connection pooling...');
    
    const testUrl = `${API_BASE_URL}/api/typedarray/urls`;
    
    // Test 1: Default connection pooling (keep-alive enabled)
    console.info('Test 1: Default connection pooling (keep-alive enabled)');
    const start1 = Date.now();
    const response1 = await fetch(testUrl);
    const duration1 = Date.now() - start1;
    console.info(`First request: ${duration1}ms, Connection: ${response1.headers.get('connection')}`);
    
    // Test 2: Second request (should reuse connection)
    console.info('Test 2: Second request (should reuse connection)');
    const start2 = Date.now();
    const response2 = await fetch(testUrl);
    const duration2 = Date.now() - start2;
    console.info(`Second request: ${duration2}ms, Connection: ${response2.headers.get('connection')}`);
    
    // Test 3: Disable keep-alive
    console.info('Test 3: Disable keep-alive');
    const start3 = Date.now();
    const response3 = await fetch(testUrl, {
      headers: { 'Connection': 'close' }
    });
    const duration3 = Date.now() - start3;
    console.info(`No keep-alive: ${duration3}ms, Connection: ${response3.headers.get('connection')}`);
    
    console.info(`Connection pooling demo completed. Reused connection was ${duration1 > duration2 ? 'faster' : 'similar'} to first request.`);
  }
  
  // Demonstrate sendfile optimization conditions
  async demonstrateSendfileConditions(): Promise<void> {
    console.info('📁 Demonstrating sendfile optimization conditions...');
    ...
    console.info('Sendfile optimization conditions:');
    ...
    console.info(`Created test blob: ${largeBlob.size} bytes`);
    ...
    try {
      ...
      console.info(`Large file upload: ${response.status}`);
      console.info('Note: sendfile optimization happens at OS level for HTTP requests');
      
    } catch (error) {
      console.info(`Upload test failed: ${error.message}`);
    }
  }
  
  // Demonstrate ContentTypeHandler utility
  async demonstrateContentTypeHandler(): Promise<void> {
    console.info('🔧 Demonstrating ContentTypeHandler utility...');
    ...
    console.info(`JSON Request Content-Type: ${jsonRequest.headers.get('content-type')}`);
    ...
    console.info(`Binary Request Content-Type: ${binaryRequest.headers.get('content-type')}`);
    ...
    console.info(`FormData Request Content-Type: ${formDataRequest.headers.get('content-type')}`);
    console.info(`FormData has boundary: ${formDataRequest.headers.get('content-type')?.includes('boundary')}`);
  }

  // Demonstrate S3 automatic signing
  async demonstrateS3Signing(): Promise<void> {
    console.info('🪣 Demonstrating S3 automatic signing...');
    
    console.info('S3 operations automatically handle:');
    console.info('  - Request signing with AWS credentials');
    console.info('  - Authentication header merging');
    console.info('  - Multipart upload for streaming bodies');
    
    // Example S3 URL pattern (would need actual credentials to work)
    const s3Example = 's3://my-bucket/path/to/object';
    
    console.info(`Example S3 URL: ${s3Example}`);
    console.info('Note: Actual S3 operations require AWS credentials in environment or explicit config');
    
    // Show the S3 fetch pattern from Bun docs
    console.info('S3 fetch pattern:');
    console.info(`await fetch("${s3Example}", {`);
    console.info('  s3: {');
  ...
  async runAllDemos(): Promise<void> {
    console.info('🎯 Running Content-Type and Implementation demos...\n');
    
    try {
      // Content-Type demos
      console.info('=== CONTENT-TYPE HANDLING DEMOS ===');
