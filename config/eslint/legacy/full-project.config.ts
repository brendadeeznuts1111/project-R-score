...
/**
 * ARCHIVED full-project ESLint policy (not the repo default).
 * Root eslint.config.ts re-exports eslint.harness.config.ts.
 * Keep for one release as reference; do not wire package scripts here.
 */
...
import { bunNativeLintRollout } from '../bun-native.ts';
import { HARNESS_BUN_GLOBALS, HARNESS_PATHS } from '../harness/rollout.ts';
import bunPlugin, { bunPluginRules } from '../plugin-bun/index.ts';
import { projectEslintConfig } from './project.config.ts';
...
export function generateTypeScriptConfig(
  ...
  return {
    ...
    include: ['src/**/*', 'lib/**/*', 'test/**/*', 'benchmarks/**/*', '*.ts', '*.tsx'],
    exclude: ['node_modules', 'dist', 'build', '.bun', 'coverage'],
  };
}
...
export async function runESLint(
  ...
  const args = [
    ...
    '.ts,.tsx',
    '--config',
    'config/eslint/legacy/full-project.config.ts',
  ];
...
import { runESLint } from './full-project.config.ts';
...
if (import.meta.main) {
  ...
  if (args[0] === 'init') {
    ...
    console.info('Run: bun config/eslint/legacy/full-project.config.ts lint');
  } else if (args[0] === 'lint') {
    ...
    console.info(`
Usage:
  bun config/eslint/legacy/full-project.config.ts init          Initialize ESLint configuration
  bun config/eslint/legacy/full-project.config.ts lint          Run ESLint
  bun config/eslint/legacy/full-project.config.ts lint --fix    Run ESLint with auto-fix
  bun config/eslint/legacy/full-project.config.ts lint --json   Output in JSON format
