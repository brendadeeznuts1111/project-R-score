...
- **`github-issue-governance`** — issue-body/provider JSON parses once; audit is
  read-only; label synchronization is deletion-free and requires explicit
  write confirmation (`unit` + `boundary`) _Ratchet_ →
...
continuous 31 · workflow 9 · human-only 32.
