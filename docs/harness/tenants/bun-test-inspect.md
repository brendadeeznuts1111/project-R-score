...
- `tmp/inspect/inspect-events.jsonl` — `TestReporter.*` · `LifecycleReporter.error` · `Console.messageAdded`
...
missing early `found` events; Bun ≥1.3.7 also replays on late `TestReporter.enable`).

## Event fields (soft)

Treat `name` / `title` / `testId` / `status` / `id` as soft — use
`testDisplayName()` / `testStatus()` helpers.
