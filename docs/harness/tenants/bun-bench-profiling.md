...
Harness-level Bun microbench and CPU-profile entrypoints. Nested `projects/` /
`scratch/` demos are **not** this contract — they may show more APIs (heap
...
h3 — SSOT [`lib/docs/benchmarking-profile-loci.ts`](../../../lib/docs/benchmarking-profile-loci.ts)).
...
| Brand hot-path CPU sample             | `brand:bench:profile` (`--cpu-prof` + `--cpu-prof-md`)                                                 |
...
Types SSOT for brand metrics:
[`scripts/lib/brand-bench-types.ts`](../../../scripts/lib/brand-bench-types.ts).  
Search
thresholds:
[`scripts/lib/search-benchmark-thresholds.ts`](../../../scripts/lib/search-benchmark-thresholds.ts).
...
dashboards (`benchmark-*.html`). Only the `.ts` runners are harness metrics. See
...
- Full UI bundle/route audit (not Bun microbench):
  [`full-ui-performance-audit.md`](full-ui-performance-audit.md)
- Factory scaffold bench scripts:
  [`.bun-create/factory-library/`](../../../.bun-create/factory-library/)
- Path constants: `CANONICAL_REPO_DOCS.bunBenchProfiling` /
  `CANONICAL_TOOLS.consoleDepthBench` in
  [`lib/docs/repo-docs.ts`](../../../lib/docs/repo-docs.ts)
- Bun 1.4 Observability contracts:
