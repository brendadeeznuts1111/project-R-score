...
|------|----------|---------------------|-------------------------|
| *(default console)* | Human reporter | `test`, `test:dev`, subsets | — |
...
| `-t` / `--test-name-pattern` | Filter by name | **ad hoc** · Cursor metadata “Test Filter” | yes |
...
| `--coverage` | Coverage run | harness coverage scripts / thresholds in bunfig | not every `test:ci` |
...
- Treat hosted GitHub Actions annotations as merge authority (local `bun run bun:ci`)
