...
**Soft ↔ Factory accounting views** ([`docs/design/soft-handshake.md`](../../design/soft-handshake.md)): Factory wire v1 ships (`lib/telegram/soft-accounting-export.ts` · `/registry/soft-accounting-export.json` · `bun run soft:accounting:bake` / `:from-ct` / `:check`). Soft may export read-only play rows (`source: soft-ct` or toc-ops fixture); week/book-type arrays stay empty until Soft tags them. Soft never depends on Factory `ops.view.*` chrome IDs. **No new glossary mints** — collapse onto existing `accounting.*` / `event.*` / `book.type.*`.
