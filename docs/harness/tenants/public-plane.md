...
| Documented bake orphans (INFO, non-blocking)  | **Documented**        | `partner-profile-coverage.json` (partner-surface inventory href only) · `stale-anchors.json` (limit-raise analysis bake) — no portal board required; `public:discover` INFO only |
