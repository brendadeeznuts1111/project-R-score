...
| Documented bake orphans (allowlisted)         | **Documented**        | `partner-profile-coverage.json` · `stale-anchors.json` — no portal board; code SSOT `DOCUMENTED_ORPHAN_REGISTRY_ARTIFACTS` in `lib/public-discovery.ts` (keep in lockstep) |
