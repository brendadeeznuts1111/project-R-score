...
FactoryWager’s **day loop** is the local (and CI-adjacent) cycle that keeps the
spine healthy: type-check, affected builds/tests, import-graph
`bun test --changed`, optional isolate/parallel/shard, then fast harness before
push. Default iterate is `bun run test:changed` / `test:watch`; full
`bun run test` is the parallel full `tests/` tree — not a `--dots` merge proof.

This is **not** Bun upstream’s CI. It lives in this repo under
...
4. **Changed tests** — `test:changed` / `test:changed:main` (Bun `--changed`
   import graph).
5. **Optional speed / isolation** — `test:isolate`, `test:parallel`,
   `test:shard`.
...
bun run test:changed:main -- --parallel=4 # bounded merge-base proof
...
| -------------------- | ---------------------------------------------------------------------- | --------------------------------------------------- |
| `test:changed`       | `scripts/bun-test-changed.ts` → `--changed --timings --update-timings` | Import-graph affected tests, slow-known files first |
| `test:changed:main`  | same + `--main-head`; merge harness adds `--parallel=4`                | Affected since main                                 |
...
≠ `bun run --parallel` (workspace Foreman). That is
[pm/filter#parallel-and-sequential-mode](https://bun.com/docs/pm/filter#parallel-and-sequential-mode).
...
| `.cache/bun-test-timings.json`     | Bun 1.4 adaptive per-file timing cache                               |
...
Ship: [v1.3.13](https://bun.com/blog/bun-v1.3.13) · flags matrix:
[`tenants/bun-test-flags.md`](tenants/bun-test-flags.md) · Platform:
