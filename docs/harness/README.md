...
- Codex thread titles / value ranking / pin parity / bring-home queue → `bun run threads:portfolio` · [`tenants/codex-thread-portfolio.md`](tenants/codex-thread-portfolio.md) · apply `bun run threads:portfolio:apply`
...
- Tennis HQ cloud agent registry auth (`FACTORY_WAGER_TOKEN` · remote sandbox) → [`tenants/tennis-hq-registry.md`](tenants/tennis-hq-registry.md) · `/registry/tennis/agent-auth.json` · `/portal/tennis/`
