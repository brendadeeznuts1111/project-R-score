...
- Maintain workspace (session · delivery · daily/weekly pulses · Cursor automation prompts) → [`tenants/maintain-workspace.md`](tenants/maintain-workspace.md) · `bun run lane:status` · `bun run harness:status`
...
- Agent skill definitions / loop registry alignment → `bun run skills:validate` · portable Project R agent contract → `bun run agents:contract:check` · operator-machine installed parity → `bun run agents:contract:check -- --installed` · [`.agents/skills/project-r-skill-maintenance/SKILL.md`](https://github.com/brendadeeznuts1111/project-R-score/blob/main/.agents/skills/project-r-skill-maintenance/SKILL.md) · [`config/project-r-agent-contract.json`](https://github.com/brendadeeznuts1111/project-R-score/blob/main/config/project-r-agent-contract.json)
- Codex `RTH-###` identity / titles / value ranking / lane entrypoints and boundaries / local-reference integrity / pin and root-subagent parity / bring-home queue → `bun run threads:portfolio` · [`tenants/codex-thread-portfolio.md`](tenants/codex-thread-portfolio.md) · daily weakest-three research `bun run threads:research` · apply `bun run threads:portfolio:apply`
...
Full map: [`day-loop.md`](day-loop.md) · Bun test flags matrix: [`tenants/bun-test-flags.md`](tenants/bun-test-flags.md).
