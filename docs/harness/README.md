...
- Monorepo health score → `bun run check:monorepo-health` (ci:core) · pre-commit tests-only when health sources staged · claim `monorepo-health-score` · operator `bun run monorepo:health` · [`tenants/monorepo-health.md`](tenants/monorepo-health.md)
- Packages graph map + env inventory → `bun run audit:packages:env` / `env:inventory:bake` · `/portal/packages/` · `/portal/env/` · [`tenants/monorepo-health.md`](tenants/monorepo-health.md) · [`tenants/proton-integration.md`](tenants/proton-integration.md) · claim `packages-graph-map-v12`
