...
| Runtime API drift ratchet           | `bun run bun:api-drift:check` — AST-checks executable static `Bun.*` paths against the installed runtime shape, not forward `bun-types`; ignores comments, strings, declarations, type-only positions, and shadowed bindings                                                                                                                                                                                                                                                                        |
