...
Docs research splits into **token** and **catalog** planes (plus blog / RSS —
see rows below). Do not treat “token/catalog” as one channel.

| Plane     | Owns                                                                                                                         | Does not own                                      |
| --------- | ---------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| **Token** | `TokenRef` → `BunToken` export · `bun-doc-refs` suggest / check / history / `@see` gates · [`README-token-ref.md`](../lib/docs/README-token-ref.md) | Materialized bake bytes (`bun-docs-catalog.json`) |
| **Catalog** | Committed index + catalog bake · `docs:catalog:*` · `tools/bun-docs-catalog.ts` · coverage prove against catalog names     | Agent export contract shape (`BunToken`)          |
| **Blog**  | Marketing HTML URLs · scrape / canonicalize ([`bun-blog-url.ts`](../lib/docs/bun-blog-url.ts))                                | Dated feed items                                  |
| **RSS**   | Dated release feed parse ([`bun-rss.ts`](../lib/docs/bun-rss.ts) · `sources.rss`)                                             | Blog index HTML                                   |
...
| Missing-reference backlog           | `bun tools/bun-doc-refs.ts backlog [--json] [--limit=N] [--max=N] <paths...>` — dry-run grouping by project and API; exact official mappings are safe, ambiguous CLI flags stay manual; `--write` requires explicit targets and a positive limit capped at 100, and writes only compact `@see` lines                                                                                                                                                                                              |
| Runtime API drift ratchet           | `bun run bun:api-drift:check` — AST-checks executable static `Bun.*` paths and named value imports from `"bun"` against the installed runtime shape, not forward `bun-types`; type assertions and ambient declarations cannot hide drift; comments, strings, declaration bodies, type-only positions, runtime-shadowed bindings, and `bun:*` submodules are ignored                                                                                                                               |
...
| Blog URL shape                      | [`bun-site-url.ts`](../lib/docs/bun-site-url.ts) (`CANONICAL_SOURCES.blog`, release patterns) · [`bun-blog-url.ts`](../lib/docs/bun-blog-url.ts) (version + canonicalize marketing vs release-notes)                                                                                                                                                                                                                                                                                             |
| RSS feed shape                      | [`bun-rss.ts`](../lib/docs/bun-rss.ts) (`parseRssChannelItems` — one walk for index / contracts / MCP) · channel URL in `config/bun-channels.toml` `sources.rss`                                                                                                                                                                                                                                                                                                                                |
...
rebuild it. **Prefer `docs:refresh:fast`** when only llms.txt / `@see` /
...
`/reference/...` entry through `bun-doc-refs.ts` (**token** plane).
