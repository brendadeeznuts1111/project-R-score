...
| `lib/portal/ui-html.ts` · `components/portal-ui.js` | Pure HTML builders — `renderPortalTable` · `renderPortalStatGrid` · `renderToneChip` · `renderPortalPanel` (import once, reuse in bakes + browser boards). Bakes: identity · vault · failures. Boards dual-class `portal-table` + board alias when specialty CSS remains (`partners-table`, `tools-table`, …). |
