...
| `lib/portal/ui-html.ts` · `components/portal-ui.js` | Pure HTML builders — `renderPortalTable` (+ `tableAttrs`) · **`renderPortalTableRows`** (`rowAttrs` · `emptyHtml`) · `renderPortalStatGrid` · chips/panel · `renderPortalError` · `renderPortalSkeleton` · `renderPortalGate`. Bakes: identity · vault · failures. Boards: partners · account · packages · bookmakers · surfaces · tools hub. Dual-class `portal-table` + board alias when specialty CSS remains. |
