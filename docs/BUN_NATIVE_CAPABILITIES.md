...
| `Bun.nanoseconds` / `Bun.sleep` / `Bun.sleepSync` / `Bun.randomUUIDv7` | high-res timing (`number` ns) · sleep ms/`Date` · monotonic UUID v7 (timestamp-encoded) | helpers [`lib/time.ts`](../lib/time.ts) · TEST-003 `evidenceId` + `elapsedMs` · `bun test tests/time.test.ts` · [nanoseconds](https://bun.com/docs/runtime/utils#bun-nanoseconds) · [sleep](https://bun.com/docs/runtime/utils#bun-sleep) · [sleepSync](https://bun.com/docs/runtime/utils#bun-sleepsync) · [randomUUIDv7](https://bun.com/docs/runtime/utils#bun-randomuuidv7) |
