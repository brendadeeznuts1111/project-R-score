...
| Runtime environment controls | typed catalog [`lib/bun-runtime-env.ts`](../lib/bun-runtime-env.ts) → env hygiene + vault-autofill guard · `--env-file` / `--no-env-file` in `config/runtime-flags.json` · `bun test tests/bun-runtime-env.test.ts tests/env-defaults-scan.test.ts tests/portal-secret.test.ts tests/portal-cli-bun-flags.test.ts` · [configuring Bun](https://bun.com/docs/runtime/environment-variables#configuring-bun) |
