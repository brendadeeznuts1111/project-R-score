...
| Runtime environment controls | typed assessment [`lib/bun-runtime-env.ts`](../lib/bun-runtime-env.ts) → env hygiene + vault-autofill guard + value-free `portal-cli doctor --group runtime` · `--env-file` / `--no-env-file` in `config/runtime-flags.json` · `bun test tests/bun-runtime-env.test.ts tests/bun-env-loading.test.ts tests/portal-cli-doctor.test.ts` · [configuring Bun](https://bun.com/docs/runtime/environment-variables#configuring-bun) |
