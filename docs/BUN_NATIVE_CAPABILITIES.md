...
| `Bun.inspect.table(tabularData, properties, options)` | returns a string (unlike `console.table`) · properties filter · `{ colors: true }` | `logTable` in [`lib/console-depth.ts`](../lib/console-depth.ts) · `bun run harness:status -- --table` · [docs](https://bun.com/docs/runtime/utils#bun-inspect-table-tabulardata-properties-options) |
