...
| `Bun.inspect.table` | opt-in tabular CLI (ratchets / proof paths) | `bun run harness:status -- --table` · [inspect.table](https://bun.com/docs/runtime/utils#bun-inspect-table-tabulardata-properties-options) · helper `logTable` in `lib/console-depth.ts` |
