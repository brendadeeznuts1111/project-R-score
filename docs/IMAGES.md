...
exactly one terminal operation (`bytes`, `buffer`, `blob`, `toBase64`,
`dataurl`, `write`, `placeholder`, or `metadata`). Work runs off the JavaScript
thread after the terminal is awaited. Do not mutate borrowed typed-array input
while a terminal is pending.
...
| Metadata    | Reads width, height, and format without decoding pixels                                     | [Metadata](https://bun.com/docs/runtime/image#metadata)                                                             |
...
| Output      | Awaited terminal executes the pipeline                                                      | [Terminals](https://bun.com/docs/runtime/image#terminals)                                                           |
