...
| ------------------------------ | -------------------------------------- | --------------------------------------------------------------------------------- | --------------------------------------------------------- |
| Inspect native package release | `bun publish --dry-run`                | Bun packs the current directory and runs the local lifecycle                      | No upload; inspect the intended native publish operation  |
...
`prepack` is the release gate: it refreshes and validates `files.md`, type
checks source/tests/scripts, runs tests, and builds the source entry point.
...
defaults. Do not commit a registry URL or credential into the template. A
scoped/private package must explicitly choose `restricted` access before its
release.
...
| ------------------------ | --------------------------- | ------------------------------------------------- | ------------------------------------------------------------------------------- |
| `publishConfig.access`   | package manifest            | Default package access (`public` or `restricted`) | Template defaults to `public`; revise deliberately for a scoped/private package |
...
| `NPM_CONFIG_TOKEN`       | CI / operator environment   | Non-interactive Bun registry authentication       | Secret; never write to `.env.example`, `files.md`, or source control            |
...
| ---------------------- | -------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Release                | `--dry-run`, `--access`, `--tag`, `--tolerate-republish`, `--gzip-level`                                       | Normal release controls. Use dry-run before a real publish.                                             |
...
source change
  → bun run check
  → bun publish --dry-run
  → review package files / tag / access / registry
  → bun publish                 (native registry route)
       or
...
- `bun run prepack` passes in a fresh local scaffold.
