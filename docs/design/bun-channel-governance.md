...
explains the policy; it does not duplicate its version values.
...
therefore not a valid Project R invariant. The invariant is that each package
matches its own selected channel and the resulting TypeScript gates pass.
...
the executing `Bun.version` to equal `.bun-version`, require `packageManager` to
equal `bun@<that version>`, and require that exact pin to satisfy `engines.bun`.
`bun:ci` then resolves and verifies the current process's Bun executable and
reuses it for every child so a later `PATH` lookup cannot cross channels.
