...
| `bun-create` manifest examples                  | Bun removes the field from generated `package.json`. The upstream example includes `preinstall`, `postinstall`, and `start`; its lifecycle table/order explicitly documents pre/post execution.                                                       | No lifecycle hooks: this source library has no setup that belongs before or after install. `bun run check` remains explicit proof.                                 | Fresh local scaffold             |
...
| Manifest materialization | local only           | Writes destination-derived `name` and removes `bun-create` when present.                                                                                                   | Source-first exports/type metadata remain; this template declares no lifecycle metadata.                                   | Generated `package.json` inspection.            |
| Lifecycle hooks          | local only           | Bun runs declared template hooks in its documented lifecycle order.                                                                                                        | None declared; installation has no template-owned side effect or version-specific message.                                 | Source manifest + generated package inspection. |
