...
4. ~~**Cron cadence + window**~~ — **resolved**: `Bun.cron` `0 0 * * 0` (Sunday midnight, `SETTLEMENT_CRON_SCHEDULE`), default period = current ISO week (Monday 00:00 UTC → now) so the `period-<weekStart>` reference is stable across re-runs. **Wiring**: OS-persistent worker (`lib/partner-profile/settlement-cron-worker.ts`) registered via `bun run partner:settlement:cron:register` (system-local time; `remove`/`preview` also available); the in-process complement is `partner:settlement:run --cron` (UTC). Run the register command from the deployment supervisor (reboot-surviving, logs at `/tmp/bun.cron.partner-settlement.stdout.log`).
