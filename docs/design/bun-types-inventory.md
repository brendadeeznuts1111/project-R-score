...
MUST be `#` + REF:ID (or empty/`—`, which implies it). Section id `4.1` sits on
the line immediately above this heading. Rules: kebab-case keywords (2–32
chars); reserved leaves `index` · `top` · `toc` · `anchor`; unique per doc;
every table/tool REF:ID has a matching HTML anchor. Validate:
`bun run docs:refid:check` · suggest:
`bun run docs:refid:suggest -- --section=4.1 --keyword=…` (also part of
`docs:map:check`; skip with `--skip-refid-check`). Doc **current** cannot show
process argv — live default vs current is printed by `bun run bun:types-status`
(Flags section · `report.json` `flags[]`).
