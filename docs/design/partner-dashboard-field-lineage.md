...
| bookmakers registry | canonical `sportsbookId`                                                     | public v0.4 catalog parser enforces identity, safe URLs, unique hosts, and ops-field redaction               | partial                | connector loading and resilience remain; display limits are never execution authority                    |
...
| `partners[].outs[].sportsbookId`             | bookmakers            | parsed public bookmaker catalog plus declared book ref                    | resolve exact hosts or explicit aliases to canonical ID                                | branded ID, key/id/slug equality, unique normalized hosts        | source loading and adapter reconciliation remain                                                       |
...
| `partners[].integrations.sportsTerminal`     | trading/operations    | `parseSportsTerminalIntegrationHealth` + bake/reconcile                     | partner-keyed availability from integration-health fixture/live path                   | status/time shape                                                | money-free; ExternalPartnerRefs joined from the same observation                                         |
