...
bun run docs:refid:write-hrefs     # fill empty/— href cells with #REF:ID
bun run docs:map:check             # includes REF:ID (unless --skip-refid-check)
```

Empty or `—` href cells imply `#` + REF:ID. Section id (e.g. `4.1`) must sit on
the line immediately above the Flags heading. Optional
`<!-- REF:ID 4.1.refresh -->` comments require a matching `<a id>`.
