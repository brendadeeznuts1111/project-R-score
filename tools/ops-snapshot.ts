...
async function maybePublishJsonArtifact(
  ...
  const token = Bun.env.FACTORY_WAGER_LOCAL_REGISTRY_TOKEN?.trim();
  if (!configuredBase || !token) {
    return {
      ok: false,
      error:
        'FACTORY_WAGER_LOCAL_REGISTRY_WRITE_URL + FACTORY_WAGER_LOCAL_REGISTRY_TOKEN required for --publish',
    };
  }
