...
const BASE = Bun.env.COMPLIANCE_URL || 'http://100.64.250.26:8787';
...
async function main(): Promise<void> {
  ...
  if (pulseRaw) {
    try {
      const parsed = extractJson(pulseRaw) as { vix?: unknown };
      vix = String(parsed.vix);
    } catch {
      /* ignore parse errors */
    }
  }
  ...
  const widths = [28, 24, 44, 6] as const;
  const sep =
    '─'.repeat(widths[0]) +
    '┼' +
    '─'.repeat(widths[1]) +
  ...
  console.log('Depth: ' + parseInt(Bun.env.BUN_CONSOLE_DEPTH || '2', 10) + ' | ' + BASE + '\n');

  console.log(
    [
      'Feature'.padEnd(widths[0]),
      'Before'.padEnd(widths[1]),
      'After'.padEnd(widths[2]),
      'Status'.padStart(4),
    ].join('|')
  );
  console.log(sep);
  for (const r of rows) {
    console.log(
      [r.f.padEnd(widths[0]), r.b.padEnd(widths[1]), r.a.padEnd(widths[2]), r.s.padStart(4)].join(
        '|'
      )
    );
  }
