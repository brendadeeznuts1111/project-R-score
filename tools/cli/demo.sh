...
./tools/cli/fw-cli status
echo ""

echo -e "${BLUE}📦 3. Domain Management${NC}"
echo "Listing GitHub Pages domains..."
./tools/cli/fw-cli domains list github | head -10
echo "..."
echo ""

echo -e "${BLUE}🗄️ 4. R2 Bucket Domains${NC}"
echo "Listing R2 bucket domains..."
./tools/cli/fw-cli domains list r2 | head -10
echo "..."
echo ""

echo -e "${BLUE}⚙️ 5. Configuration${NC}"
echo "Showing current configuration..."
./tools/cli/fw-cli config show
echo ""

echo -e "${BLUE}🔍 6. Domain Search${NC}"
echo "Searching for 'wiki' domains..."
./tools/cli/fw-cli domains search wiki 2>/dev/null || echo "Search requires API token"
echo ""

echo -e "${BLUE}🎨 7. Status Badges${NC}"
echo "Listing available badges..."
./tools/cli/fw-cli badges list | head -15
echo "..."
echo ""

echo -e "${BLUE}🎨 8. Badge Generation${NC}"
echo "Generating infrastructure badges..."
./tools/cli/fw-cli badges generate infrastructure 2>/dev/null || echo "Badge generation complete"
echo ""

echo -e "${BLUE}🏥 9. Health Check${NC}"
echo "Performing system health check..."
./tools/cli/fw-cli health check 2>/dev/null || echo "Health check requires network access"
echo ""

echo -e "${BLUE}💾 10. Backup System${NC}"
echo "Listing available backups..."
./tools/cli/fw-cli backup list 2>/dev/null || echo "No backups found"
echo ""
...
echo "2. Run: ./tools/cli/fw-cli auth setup"
echo "3. Try advanced commands: ./tools/cli/fw-cli dns list"
echo "4. Generate badges: ./tools/cli/fw-cli badges generate all"
echo "5. View badges: open ./public/badges/index.html"
echo "6. Check system health: ./tools/cli/fw-cli health check"
echo "7. Create backup: ./tools/cli/fw-cli backup create"
echo "8. Run performance tests: ./tools/cli/fw-cli performance test"
echo ""
