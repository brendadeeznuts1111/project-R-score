...
function getContentEncoding(entry: HarEntry): string {
  ...
  const header = entry.response.headers?.find(h => h.name.toLowerCase() === 'content-encoding');
  return header?.value || 'identity';
}

function getResponseHeader(entry: HarEntry, name: string): string | null {
  const header = entry.response.headers?.find(h => h.name.toLowerCase() === name.toLowerCase());
...
function fmtEncoding(enc: string): string {
  switch (enc) {
    case 'br':
      return '\x1b[35mbr\x1b[0m';
    case 'gzip':
      return '\x1b[35mgz\x1b[0m';
    case 'zstd':
      return '\x1b[35mzstd\x1b[0m';
    case 'deflate':
      return '\x1b[35mdefl\x1b[0m';
    case 'identity':
...
function renderWaterfall(entries: HarEntry[], width = 50) {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    console.info(
      `  ${' '.repeat(pad)}${bar} ${statusColor(e.response.status)} ${fmtMs(e.time)} ${label}`
    );
  }
...
function renderDomainBreakdown(entries: HarEntry[], pageUrl?: string) {
  ...
  console.info(
    `  ${'Domain'.padEnd(30)} ${'Provider'.padEnd(9)} ${'Reqs'.padStart(4)} ${'Transfer'.padStart(9)} ${'Size'.padStart(9)} ${'Avg TTFB'.padStart(9)} ${'Slowest'.padStart(9)}`
  );
...
function renderTimingBreakdown(entries: HarEntry[]) {
  ...
  const total =
    totalBlocked + totalDns + totalConnect + totalSsl + totalSend + totalWait + totalReceive;
  if (total <= 0) return;

  const pct = (v: number) => (total > 0 ? `${((v / total) * 100).toFixed(0)}%` : '0%');
  console.info(`\n\x1b[1m  Time Breakdown\x1b[0m (cumulative across all requests)`);
  console.info(
    `  \x1b[90mBlocked:\x1b[0m ${fmtMs(totalBlocked).padStart(8)} ${pct(totalBlocked).padStart(4)}   \x1b[36mDNS:\x1b[0m ${fmtMs(totalDns).padStart(8)} ${pct(totalDns).padStart(4)}   \x1b[36mConnect:\x1b[0m ${fmtMs(totalConnect).padStart(8)} ${pct(totalConnect).padStart(4)}`
  );
  console.info(
    `  \x1b[36mSSL:\x1b[0m    ${fmtMs(totalSsl).padStart(8)} ${pct(totalSsl).padStart(4)}   \x1b[34mSend:\x1b[0m ${fmtMs(totalSend).padStart(7)} ${pct(totalSend).padStart(4)}   \x1b[33mWait:\x1b[0m    ${fmtMs(totalWait).padStart(8)} ${pct(totalWait).padStart(4)}`
  );
  console.info(
    `  \x1b[32mReceive:\x1b[0m ${fmtMs(totalReceive).padStart(8)} ${pct(totalReceive).padStart(4)}`
  );
}
...
if (flags.type !== null) {
  entries = entries.filter(e => resourceType(e).toLowerCase() === flags.type!.toLowerCase());
}
if (flags.domain !== null) {
  entries = entries.filter(e => getDomain(e.request.url).includes(flags.domain!));
}
...
if (flags.table) {
  ...
  const rows = entries.map((e, i) => {
    ...
    const proto =
      httpVer === 'h2' || httpVer === 'HTTP/2' || httpVer === 'http/2.0'
        ? 'h2'
        : httpVer === 'h3' || httpVer === 'HTTP/3'
          ? 'h3'
          : 'h1.1';
    return {
      '#': i + 1,
  ...
  const totalTx = entries.reduce(
    (s, e) => s + (e.response._transferSize ?? e.response.content.size),
    0
  );
  ...
  if (flags.withDocs) {
    ...
    if (url) {
      ...
      if (result.documentationGaps.length > 0) {
        ...
        console.info(
          Bun.inspect.table(
            result.documentationGaps.map(g => ({
              Priority: g.priority,
              Category: g.category,
              Gap: g.gap,
            })),
...
if (totalSize > 0 && totalTransfer < totalSize) {
  ...
  const encLine = [...encodingCounts.entries()]
    .sort((a, b) => b[1] - a[1])
    .map(([enc, count]) => `${enc}(${count})`)
    .join('  ');
  console.info(
    `  Compression: \x1b[35m${overallPct}% savings\x1b[0m  ${encLine || 'none detected'}`
  );
}
...
const statusLine = [...statusCounts.entries()]
  .sort((a, b) => a[0] - b[0])
  .map(([code, count]) => `${statusColor(code)}×${count}`)
  .join('  ');
console.info(`  Status: ${statusLine}`);

// Type breakdown
const typeLine = [...typeCounts.entries()]
  .sort((a, b) => b[1] - a[1])
