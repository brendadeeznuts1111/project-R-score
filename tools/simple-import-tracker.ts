...
plugin({
  ...
  setup(build) {
    ...
    build.onLoad({ filter: /\.(ts|js)$/ }, async ({ path }) => {
      try {
        ...
        for (const importInfo of imports) {
          ...
          console.info(`   📦 ${importPath} (${trackedImports[importPath]})`);
        }
        ...
        console.info(`❌ Error scanning ${path}:`, error.message);
      }
    ...
    build.onLoad({ filter: /import-stats\.js$/ }, async ({ defer }) => {
      console.info(`📊 Generating import statistics...`);

      // Wait for all files to be processed
      await defer();

      console.info(`📋 Final import statistics:`);
      Object.entries(trackedImports).forEach(([path, count]) => {
        console.info(`   ${path}: ${count} times`);
      });
...
console.info('🔍 Import Analysis Results:');
console.info('Total unique imports:', Object.keys(importStats).length);
console.info('');
Object.entries(importStats).forEach(([path, count]) => {
  console.info(\`  \${path}: \${count} import\${count === 1 ? '' : 's'}\`);
});
