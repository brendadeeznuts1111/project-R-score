#!/usr/bin/env bun
// tools/secret-helper.ts — FactoryWager secret management helper CLI
//
// Usage:
//   bun tools/secret-helper.ts get API_KEY_V3 HIGH
//   bun tools/secret-helper.ts docs
//   bun tools/secret-helper.ts list
//   bun tools/secret-helper.ts validate
//   bun tools/secret-helper.ts audit
...
function printHeader(title: string) {
  ...
  console.info(colorize(`  ${title}`, 'bright') + colorize(' - FactoryWager Enterprise', 'dim'));
  console.info(colorize('='.repeat(60), 'cyan'));
}
...
function printSecret(secret: string, key: string, level: string, metadata?: any) {
  ...
  console.info(colorize('│', 'cyan') + ` Key: ${colorize(key, 'bright')}`);
  console.info(colorize('│', 'cyan') + ` Level: ${colorize(level, levelColor)}`);
...
async function getSecret(key: string, level: string) {
  try {
    ...
    if (!result) {
      console.info(colorize(`❌ Secret '${key}' not found`, 'red'));
    ...
    try {
      ...
      console.info(colorize(`📋 Secret saved to /tmp/secret_clipboard`, 'dim'));
    } catch (error) {
...
${colorize('• Get secret:', 'dim')} bun tools/secret-helper.ts get <KEY> <LEVEL>
${colorize('• List secrets:', 'dim')} bun tools/secret-helper.ts list
${colorize('• Show docs:', 'dim')} bun tools/secret-helper.ts docs
${colorize('• Validate secrets:', 'dim')} bun tools/secret-helper.ts validate
${colorize('• Audit secrets:', 'dim')} bun tools/secret-helper.ts audit
...
async function listSecrets() {
  try {
    ...
    if (mockSecrets.length === 0) {
      console.info(colorize('No secrets found', 'yellow'));
      return;
    }
...
async function validateSecrets() {
  try {
    ...
    console.info(colorize('🔍 Validating secret configuration...', 'yellow'));
...
async function auditSecrets() {
  try {
    ...
    console.info(colorize('🔍 Generating audit report...', 'yellow'));
...
${colorize('Usage:', 'yellow')}
  bun tools/secret-helper.ts <command> [options]
...
async function main() {
  try {
    switch (command) {
      case 'get':
        if (args.length < 3) {
          console.info(colorize('❌ Usage: bun tools/secret-helper.ts get <KEY> <LEVEL>', 'red'));
          process.exit(1);
        }
      ...
      default:
        console.info(colorize(`❌ Unknown command: ${command}`, 'red'));
        showHelp();
        process.exit(1);
    }
  } catch (error) {
    console.info(colorize(`❌ Fatal error: ${error.message}`, 'red'));
    process.exit(1);
  }
}
