...
const FEEDS = [
  { file: 'all.xml', id: asFeedId('bun-1.4:all') },
...
export async function buildBun14ReleaseSnapshot(): Promise<{
  ...
  const manifest = JSON.parse(
    new TextDecoder().decode(input['registry/bun-1.4-assets.json'])
  ) as AssetManifest;
  const capabilityRegistry = JSON.parse(
    new TextDecoder().decode(input['registry/bun-1.4-capabilities.json'])
  ) as Bun14CapabilityRegistry;
  const projectRSSRegistry = parseProjectRSSChannelRegistry(
    JSON.parse(new TextDecoder().decode(input['registry/project-rss-channels.json']))
  );
  ...
  const actualGuids = (itemsByChannel.get('bun-1.4:all') ?? [])
    .map(item => (typeof item.guid === 'string' ? item.guid : item.guid?.['#text']))
    .sort();
  ...
  const channelGuids = new Map(
    [...itemsByChannel].map(([channelId, items]) => [
      channelId,
      new Set(
        items.map(item => (typeof item.guid === 'string' ? item.guid : item.guid?.['#text']))
      ),
    ])
  );
  for (const asset of manifest.assets) {
    const guid = `bun-1.4:asset:${asset.id}`;
    const kindChannel = CHANNEL_ID_BY_KIND[asset.kind];
    if (!channelGuids.get(kindChannel)?.has(guid)) {
  ...
  const chapters = capabilityRegistry.chapters.map(chapter => ({
    ...
    itemCount: [...chaptersByAsset.values()].filter(chapterIds => chapterIds.has(chapter.id))
      .length,
  }));
  ...
  return {
    ...
    snapshot: {
      ...
      items: manifest.assets.map(asset => ({
        ...
        channels: [asFeedId('bun-1.4:all'), CHANNEL_ID_BY_KIND[asset.kind]],
