#!/usr/bin/env bun
// tools/enhanced-status-checker.ts — Enhanced documentation status checker with fragment support
...
async function runStatusCheck() {
  console.info(`${colors.cyan}🔍 Enhanced Documentation Status Checker${colors.reset}`);
  console.info(`${colors.gray}Checking all constants and URLs INCLUDING FRAGMENTS...${colors.reset}\n`);
  ...
  try {
    ...
    if (options.fragments || options.verbose) {
      console.info(`${colors.magenta}Fragment Checking:${colors.reset} ${options.fragments ? 'ENABLED' : 'DISABLED'}`);
    }
