...
const EXCLUDE = new Set(['node_modules', '.npm-cache', '.git', '.cache', 'dist', 'build', 'coverage', '.wrangler']);
...
function triage(rel: string): string {
  ...
  if (rel === '.' || rel.startsWith('packages/') || rel.startsWith('lib/') || rel.startsWith('tools/') || rel.startsWith('server/') || rel.startsWith('config/')) return 'core';
  if (rel.startsWith('factorywager/') || rel.startsWith('kimiremote/')) return 'core';
  return 'active';
}
...
console.info(`| ${'Package'.padEnd(45)} | ${'Version'.padEnd(12)} | ${'Registry'.padEnd(15)} | ${'Triage'.padEnd(12)} |`);
console.info(`|${'-'.repeat(47)}|${'-'.repeat(14)}|${'-'.repeat(17)}|${'-'.repeat(14)}|`);
for (const [tag, name, ver, reg] of rows) {
  console.info(`| ${name.padEnd(45)} | ${ver.padEnd(12)} | ${reg.padEnd(15)} | ${tag.padEnd(12)} |`);
}
