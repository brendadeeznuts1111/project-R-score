...
async function demoBunWrite() {
  ...
  console.info(`${c.green}[json]${c.reset}    Wrote ${jsonBytes} bytes → ${jsonPath}`);
...
async function demoBunDelete() {
  ...
  console.info(`${c.dim}Created ${filesToDelete.length} temp files${c.reset}`);
  ...
  try {
    ...
    console.info(`${c.yellow}[delete]${c.reset}  non-existent file: no error thrown`);
  } catch (e: any) {
    console.info(`${c.yellow}[delete]${c.reset}  non-existent file threw: ${e.message}`);
  }
