...
class Doctor {
  ...
  async checkRipgrepInstallation(): Promise<void> {
    try {
      ...
      if (proc.success && proc.stdout) {
        ...
        this.addCheck('Ripgrep Installation', 'pass', `Ripgrep is installed: ${versionLine}`, {
          version: versionLine,
        });
      } else {
    ...
    try {
      if (fs.existsSync(cacheDir)) {
        ...
        try {
          ...
          this.addCheck('Cache Directory', 'pass', `Cache directory created: ${cacheDir}`, {
            path: cacheDir,
            created: true,
          });
        } catch (createError) {
          this.addCheck('Cache Directory', 'fail', `Cannot create cache directory: ${cacheDir}`, {
            error: createError.message,
          });
        }
      }
    } catch (error) {
      this.addCheck('Cache Directory', 'fail', `Cache directory check failed: ${error.message}`, {
        error: error.message,
      });
    }
  }
  ...
  async checkBunSpawnPerformance(): Promise<void> {
    try {
      ...
      this.addCheck('Bun.spawn Performance', 'fail', `Bun.spawn test failed: ${error.message}`, {
        error: error.message,
      });
    }
  }
  ...
  async checkParallelSearch(): Promise<void> {
    try {
      ...
      this.addCheck('Parallel Search', 'fail', `Parallel search test failed: ${error.message}`, {
        error: error.message,
      });
    }
  }
  ...
  async checkMemoryUsage(): Promise<void> {
    ...
    try {
      ...
      this.addCheck('Memory Usage', 'fail', `Memory test failed: ${error.message}`, {
        error: error.message,
      });
    }
  }
