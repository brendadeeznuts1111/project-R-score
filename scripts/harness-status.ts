...
const ratchets: Array<{ cmd: string; purpose: string }> = [
  ...
  { cmd: 'bun run test:changed', purpose: 'Import-graph vs dirty (or bun test --changed=REF)' },
  { cmd: 'bun run test:changed:watch', purpose: 'Re-filter on restart (bun test --changed --watch)' },
