...
const API_REGISTRY: Record<string, APIEndpoint[]> = {
  'ai-operations': [
    {
      ...
      examples: [{ type: 'optimize', input: 'Optimize cache performance', priority: 'high' }],
    },
    {
      ...
      responses: [{ code: 200, description: 'Insights retrieved successfully' }],
    },
    {
      path: '/api/ai/predict',
      method: 'POST',
      ...
      parameters: [{ name: 'timeframe', type: 'string', required: true }],
      responses: [{ code: 200, description: 'Prediction generated successfully' }],
    },
  ],
  security: [
    {
  ...
  cache: [
    ...
    },
    {
      ...
      responses: [{ code: 200, description: 'Value stored successfully' }],
    },
  ],
  monitoring: [
    {
      ...
      responses: [{ code: 200, description: 'Metrics retrieved successfully' }],
    },
    {
      ...
      responses: [{ code: 200, description: 'Anomalies retrieved successfully' }],
    },
  ],
};
...
async function generateDeepLinks(config: DeepLinkConfig): Promise<DeepLink[]> {
  ...
  for (const endpoint of endpoints) {
    ...
    const insight = await aiOperations
      .getInsights({
        type: 'performance',
        minConfidence: 0.6,
      })
    ...
    const deepLink: DeepLink = {
      ...
      examples: config.includeExamples
        ...
        : undefined,
      security:
        config.securityLevel !== 'public'
          ? [`Requires ${config.securityLevel} access`, 'Zero-trust authentication required']
          : undefined,
    };
  ...
  console.info(
    color(`Generated ${deepLinks.length} deep links in ${(elapsed / 1e6).toFixed(2)}ms`, 'gray')
  );
...
function displayDeepLinks(links: DeepLink[], config: DeepLinkConfig) {
  ...
  console.info(
    color(
      `\n📋 API: ${color(config.apiName, 'green')} | Version: ${color(config.version, 'green')} | Security: ${color(config.securityLevel, 'yellow')}`,
      'white'
    )
  );
  ...
  Object.entries(grouped).forEach(([category, categoryLinks]) => {
    ...
    categoryLinks.forEach((link, index) => {
      const methodColor =
        link.method === 'GET' ? 'green' : link.method === 'POST' ? 'blue' : 'yellow';
  ...
  console.info(
    `  GET Methods: ${color(links.filter(l => l.method === 'GET').length.toString(), 'green')}`
  );
  console.info(
    `  POST Methods: ${color(links.filter(l => l.method === 'POST').length.toString(), 'blue')}`
  );
...
async function main() {
  ...
  const securityLevel = args.includes('--restricted')
    ? 'restricted'
  ...
  try {
    if (shortMode) {
      ...
      console.info(
        `  API: ${color(apiName, 'green')} | Security: ${color(securityLevel, 'yellow')} | Examples: ${color(includeExamples ? 'Yes' : 'No', 'cyan')}`
      );
      console.info(
        `  Docs: ${color(`https://bun.sh/docs/${apiName.toLowerCase().replace('bun.', '')}`, 'blue')}`
      );
      console.info(
        `  Source: ${color(`https://github.com/oven-sh/bun/blob/main/src/${apiName.replace('Bun.', '').toLowerCase()}.ts`, 'blue')}`
      );
      return;
    }
