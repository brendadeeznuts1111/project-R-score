...
async function displayQuickInfo(status: SystemStatus) {
  ...
  console.info(`  Secrets Hit Rate: ${color(formatPercentage(status.cache.secrets.hitRate), 'cyan')}`);
  console.info(`  Config Hit Rate: ${color(formatPercentage(status.cache.config.hitRate), 'cyan')}`);
  ...
  console.info(`  Last Check: ${color(new Date(status.performance.lastCheck).toLocaleTimeString(), 'cyan')}`);
  ...
  console.info(`  Overall Status: ${color(memoryUsage > 0.8 ? '⚠️  Warning' : '✅ Healthy', healthStatus)}`);
  ...
  if (status.ai.insights > 0) {
    ...
    topInsights.forEach((insight, i) => {
      console.info(`  ${i + 1}. ${color(insight.title, 'cyan')} (${formatPercentage(insight.confidence)} confidence)`);
    });
  }
...
async function main() {
  ...
  if (shortMode || summaryMode) {
    ...
    try {
      ...
      console.info(`  Memory: \x1b[36m${formatMemory(status.memory.heapUsed)}\x1b[0m | AI: \x1b[32m${status.ai.insights.toString()}\x1b[0m | Cache: \x1b[33m${formatPercentage(status.cache.secrets.hitRate)}\x1b[0m`);
      console.info(`  Health: ${status.memory.heapUsed / status.memory.heapTotal > 0.8 ? '\x1b[33m⚠️\x1b[0m' : '\x1b[32m✅\x1b[0m'}`);
    } catch (error) {
