...
if (command === 'matrix') {
  ...
  const routineSteps = [
    {
      ...
      message: 'Version 1.3.9, all systems green',
    },
    {
      name: 'GitHub Integration',
      status: 'success' as const,
      severity: 'medium' as const,
      message: 'All checks passed, latest commit synced',
    },
    {
      name: 'Deep Links',
      status: 'info' as const,
      severity: 'low' as const,
      message: 'Bun.secrets API links generated',
    },
    {
      name: 'MCP Monitor',
      status: 'warning' as const,
      severity: 'medium' as const,
      message: 'Cache hit rate at 72%, optimization needed',
    },
    {
      name: 'AI Insights',
      status: 'success' as const,
      severity: 'high' as const,
      message: 'Performance optimizations ready',
    },
  ];
  ...
  console.info(
    colorize('✨ All systems operational with perceptual color optimization!', 'green', true)
  );
  ...
  ['low', 'medium', 'high', 'critical'].forEach(severity => {
    ...
    console.info(
      `  Hex: ${newDisplay.hex} | Brightness: ${(newDisplay.brightness * 100).toFixed(1)}%`
    );
    console.info();
  });
} else {
  ...
  console.info(
    '  bun run bun-status-matrix-demo.ts single [status] [severity] [context]  # Single status'
  );
  console.info(
    '  bun run bun-status-matrix-demo.ts routine                    # Daily routine demo'
  );
  console.info(
    '  bun run bun-status-matrix-demo.ts compare [status]           # Compare old vs new'
  );
  console.info();
