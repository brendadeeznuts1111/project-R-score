...
async function main() {
  ...
  await Bun.write(
    Bun.stdout,
    Bun.color('green', 'ansi') + 'Success: Operation completed\n' + '\x1b[0m'
  );
  await Bun.write(
    Bun.stdout,
    Bun.color('yellow', 'ansi') + 'Warning: Check configuration\n' + '\x1b[0m'
  );
  await Bun.write(
    Bun.stdout,
    Bun.color('red', 'ansi') + 'Error: Something went wrong\n' + '\x1b[0m'
  );
  ...
  await Bun.write(
    Bun.stderr,
    Bun.color('red', 'ansi') + '[STDERR] This is an error message\n' + '\x1b[0m'
  );
  ...
  try {
    ...
    if (chunks.length > 0) {
      ...
      writeColored(
        `Streamed input: ${chunks.length} chunk(s), ${streamedText.length} characters\n`,
        'green'
      );
    }
  } catch (error) {
