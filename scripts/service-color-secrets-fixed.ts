...
const formatStatusCell = (
  ...
  const ansi =
    Bun.color(
      `hsl(${hue}, ${config.saturationMod * 100}%, ${config.lightnessMod * 100}%)`,
      'ansi'
    ) || '';
...
async function scanProjectConfigs(
  rootDir = '.'
): Promise<{ path: string; config: Record<string, any> }[]> {
...
function validateProjectConfigs(
  configs: { path: string; config: Record<string, any> }[],
...
export const renderSecretMatrix = async (profile: string): Promise<void> => {
  ...
  try {
    ...
    throw new Error(
      `Failed to resolve secret statuses: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
  ...
  console.info(`${c.dim}Profile: ${c.reset}${c.bold}${profile}${c.reset}`);
  ...
  if (error > 0) {
    console.info(`${c.red}Critical: Missing secrets detected${c.reset}`);
  ...
  if (process.argv.includes('--html')) {
    try {
      const html = `<h1>Secret Status Report</h1><table border="1"><tr>${BUN_SECRET_STATUS_COLUMNS.map(c => `<th>${escapeHTML(c)}</th>`).join('')}</tr>${tableData
        .map(
          row =>
      ...
      console.info('Exported HTML report: secrets-report.html');
    } catch (error) {
      console.error(
        `${c.red}Failed to export HTML report: ${error instanceof Error ? error.message : 'Unknown error'}${c.reset}`
      );
    }
  }
  ...
  if (process.argv.includes('--plain')) {
    try {
      ...
      console.error(
        `${c.red}Failed to export plain text report: ${error instanceof Error ? error.message : 'Unknown error'}${c.reset}`
      );
    }
  }
...
if (import.meta.main) {
  ...
  if (args[0] === 'matrix' || args.length === 0) {
    ...
    console.info('Usage: bun run service-color-secrets.ts [matrix] --profile=<name>');
  }
}
