...
class YAMLMigrator {
  ...
  private migrateFile(filePath: string): MigrationResult {
    ...
    try {
      ...
      const fsYamlPattern =
        /fs\.readFileSync\([^,]+,\s*['"]utf8['"]\)\s*\)\s*[,;]?\s*\n\s*.*load\(/g;
      const fsMatches = content.match(fsYamlPattern);
      if (fsMatches) {
        console.info(
          `  ⚠️  Manual review needed: ${fsMatches.length} fs.readFileSync + Bun.yaml.parse patterns`
        );
  ...
  async migrate(basePath: string = '.'): Promise<void> {
    ...
    for (const file of files) {
      try {
        ...
        const hasYAMLUsage =
          /require\(['"](yaml|js-yaml)['"]\)|import.*from\s*['"](yaml|js-yaml)['"]|yaml\.(load|parse|dump|stringify)\(|\b(load|dump)\(/.test(
            content
          );
