...
class PerformanceTest {
  ...
  async testRipgrepPerformance(): Promise<void> {
    ...
    for (const query of this.queries) {
      const { result, metrics } = await this.measureOperation(`Ripgrep: ${query}`, () =>
        searcher.search(query, { maxResults: 10 })
      );

      console.info(
        `✅ ${query}: ${metrics.duration}ms, ${metrics.results} matches, ${metrics.memoryAfter}MB`
      );
    }
  }
  ...
  async testEnhancedFetcherPerformance(): Promise<void> {
    ...
    for (const query of this.queries) {
      const { result, metrics } = await this.measureOperation(`Enhanced Fetcher: ${query}`, () =>
        fetcher.searchWithRipgrep(query, { maxResults: 10 })
      );

      console.info(
        `✅ ${query}: ${metrics.duration}ms, ${result.performance.totalMatches} total matches`
      );
    }
  }
  ...
  async testGhostSearchPerformance(): Promise<void> {
    ...
    for (const query of this.queries.slice(0, 4)) {
      // Test fewer queries for ghost search
      const { result, metrics } = await this.measureOperation(`Ghost Search: ${query}`, () =>
      ...
      const totalMatches =
        result.bunSh.length +
        result.bunCom.length +
        result.content.length +
        (result.projectCode?.length || 0);
      console.info(
        `✅ ${query}: ${metrics.duration}ms, ${totalMatches} total matches, ${result.performance.parallelSpeedup}x speedup`
      );
    }
  }
  ...
  async testRealTimeSearchPerformance(): Promise<void> {
    ...
    for (const query of rapidQueries) {
      const { result, metrics } = await this.measureOperation(`Real-time: "${query}"`, () =>
        realTimeSearch.search(query, { maxResults: 5 })
      );

      console.info(
        `✅ "${query}": ${metrics.duration}ms, ${result.performance.totalMatches} matches`
      );
    }
  }
  ...
  async testParallelVsSequential(): Promise<void> {
    ...
    // Parallel execution
    const { metrics: parallelMetrics } = await this.measureOperation('Parallel Search', () =>
