...
async function demoParallelSearch() {
  ...
  try {
    ...
    console.info(
      `📊 Total matches: ${result.bunSh.length + result.bunCom.length + result.content.length}`
    );
...
async function demoRealTimeSearch() {
  ...
  try {
    ...
    for (const query of queries) {
      ...
      console.info(
        `   "${query}": ${(endTime - startTime).toFixed(2)}ms, ${result.performance.totalMatches} matches`
      );
    }
