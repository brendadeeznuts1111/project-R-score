...
async function provisionPartner(code: string): Promise<void> {
  const apiKey = Buffer.from(await crypto.subtle.generateKey({ name: "AES-GCM", length: 256 }, true, ["encrypt", "decrypt"])).toString("hex") || crypto.randomUUID().replace(/-/g, "");
  const signingKey = Array.from(crypto.getRandomValues(new Uint8Array(32))).map(b => b.toString(16).padStart(2, "0")).join("");
  const webhookSecret = Array.from(crypto.getRandomValues(new Uint8Array(16))).map(b => b.toString(16).padStart(2, "0")).join("");
  ...
  const proc = Bun.spawnSync([
    ...
    env: { ...process.env, PROTON_PASS_AGENT_REASON: `Provisioning ${code} partner secrets` },
  });

  console.error(`[provision] ✅ ${code}: API key=${apiKey.slice(0, 8)}... signing=${signingKey.slice(0, 8)}... webhook=${webhookSecret.slice(0, 8)}...`);
