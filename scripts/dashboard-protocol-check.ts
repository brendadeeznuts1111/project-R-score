...
async function run(): Promise<number> {
  ...
  try {
    if (PROTOCOL && PROTOCOL_CASES[PROTOCOL]) {
      ...
      const hasProtocol = matrixRows.some((p: any) => {
        ...
        if (PROTOCOL === 'http')
          return protocolLabel.includes('http') || scheme.includes('http://');
        if (PROTOCOL === 'https')
          return protocolLabel.includes('https') || scheme.includes('https://');
        if (PROTOCOL === 's3') return protocolLabel.includes('s3') || scheme.includes('s3://');
        if (PROTOCOL === 'file')
          return protocolLabel.includes('file') || scheme.includes('file://');
      ...
      const scorecard = await fetchJson(
        `/api/control/protocol-scorecard?use=${encodeURIComponent(spec.scoreUse)}&size=${spec.scoreSize}`
      );
  ...
  const { base } = getDashboardTestConfig();
  console.info(
    `Checked ${checks.length} protocol assertions against ${base}${PROTOCOL ? ` (protocol=${PROTOCOL})` : ''}`
  );
