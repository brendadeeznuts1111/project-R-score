...
import { ensureDir } from './fs-bun.ts';
import { resolvePrePushProfile } from './pre-push-receipt-contract.ts';

type GitResult = { code: number; out: string };

/** Runs the receipt hook under an opt-in Bun Markdown profile, when requested. */
export async function runPrePushReceiptProfile(options: {
  root: string;
  entrypoint: string;
  args: string[];
  git: (args: string[]) => GitResult;
}): Promise<number | null> {
  if (Bun.env.PREPUSH_PROFILE !== '1' || Bun.env.PREPUSH_PROFILE_CHILD === '1') return null;
  const profileDir = `${options.root}/reports`;
  const tree = options.git(['rev-parse', 'HEAD^{tree}']);
