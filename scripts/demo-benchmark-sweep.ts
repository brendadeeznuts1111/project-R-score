...
async function main() {
  ...
  for (const id of ids) {
    ...
    const benchCommand =
      contract.modules[id]?.benchCommand || `bun run demo:module:bench --id=${id}`;
    const cmd = benchCommand.includes('--id=')
      ? benchCommand
      : `bun run demo:module:bench --id=${id}`;
  ...
  if (compareLast) {
    const previous = readJsonSafe<{ results?: Array<{ id: string; opsPerSec?: number | null }> }>(
      LATEST_PATH
    );
  ...
  console.info(
    JSON.stringify({ ...snapshot, paths: { latest: LATEST_PATH, snapshot: snapshotPath } }, null, 2)
  );
