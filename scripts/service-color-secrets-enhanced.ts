...
const formatStatusCell = (
  ...
  const hsl = getStatusColor(status, config.lightnessMod * 100, severity || 'medium');
  ...
  const ansi =
    Bun.color(`hsl(${adjustedHsl.h}, ${adjustedHsl.s}%, ${adjustedHsl.l}%)`, 'ansi') || '';
...
async function discoverProjects(root = './projects'): Promise<string[]> {
  try {
    ...
    console.warn(
      `${c.yellow}Failed to discover projects: ${error instanceof Error ? error.message : String(error)}${c.reset}`
    );
    return [];
  }
}
...
async function loadProjectPackage(projectPath: string): Promise<Record<string, any> | null> {
  try {
    ...
    console.warn(
      `${c.yellow}Failed to load package.json for ${projectPath}: ${error instanceof Error ? error.message : String(error)}${c.reset}`
    );
    return null;
  }
}
...
async function generateProjectRssFeed(
  projectPath: string,
...
<pubDate>${item.pubDate}</pubDate>
  ...
  .join('')}
    ...
    console.warn(
      `${c.yellow}Failed to generate RSS for ${projectPath}: ${error instanceof Error ? error.message : String(error)}${c.reset}`
    );
  }
}
...
export const resolveSecretStatuses = async (profile: string): Promise<SecretStatus[]> => {
  ...
  for (const secretName of BUN_PROFILES_SECRET_NAMES) {
    const envVar =
      Object.entries(BUN_PROFILES_ENV_MAP).find(([_, v]) => v === secretName)?.[0] ||
      secretName.toUpperCase();

    const keychainResult = await profileKeychainGet(service, secretName);
    const keychainStatus = keychainResult.ok
      ? 'found'
      : keychainResult.code === 'NOT_FOUND'
        ? 'missing'
        : 'error';
...
export const renderSecretMatrix = async (profile: string, scanProjects = false): Promise<void> => {
  ...
  try {
    ...
    console.error(
      `${c.red}Failed to resolve secret statuses: ${error instanceof Error ? error.message : String(error)}${c.reset}`
    );
    process.exit(1);
  }
  ...
  console.info(
    `${c.dim}Profile: ${c.reset}${c.bold}${sanitizeEnvVar(profile, 'local', false)}${c.reset}`
  );
  ...
  console.info(
    `${applyHsl(180 + config.hueShift, config.saturationMod * 100, config.lightnessMod * 50)}${HSL_PREFIX}${config.hueShift}${HSL_SUFFIX}${c.reset}\n`
  );
  ...
  const tableData = statuses.map(s => ({
    Secret: s.name,
    'Env Var': s.envVar,
    Keychain: formatStatusCell(
      s.keychainStatus === 'found'
        ? 'success'
        : s.keychainStatus === 'missing'
          ? 'error'
          : 'warning',
      config
    ),
  ...
  if (process.argv.includes('--html')) {
    try {
      const html = `<h1>Secret Status Report - ${escapeHTML(profile)}</h1><table border="1"><tr>${BUN_SECRET_STATUS_COLUMNS.map(c => `<th>${escapeHTML(c)}</th>`).join('')}</tr>${tableData
        .map(
          row =>
      ...
      console.error(
        `${c.red}Failed to export HTML report: ${error instanceof Error ? error.message : String(error)}${c.reset}`
      );
    }
  }
  ...
  if (process.argv.includes('--plain')) {
    try {
      ...
      console.error(
        `${c.red}Failed to export plain text report: ${error instanceof Error ? error.message : String(error)}${c.reset}`
      );
    }
  }
  ...
  try {
    ...
    for await (const path of glob.scan('.')) {
      ...
      try {
        ...
        console.warn(
          `Failed to parse ${path}: ${escapeHTML(e instanceof Error ? e.message : String(e))}`
        );
      }
    }
    ...
    console.warn(
      `${c.yellow}Failed to scan project configs: ${error instanceof Error ? error.message : String(error)}${c.reset}`
    );
  }
  ...
  try {
    ...
    console.warn(
      `${c.yellow}Failed to resolve project module: ${error instanceof Error ? error.message : String(error)}${c.reset}`
    );
  }
};
...
if (import.meta.main) {
  ...
  if (args[0] === 'matrix' || args.length === 0) {
    ...
    console.info(
      'Usage: bun run service-color-secrets.ts [matrix] --profile=<name> [--scan-projects] [--html] [--plain]'
    );
  }
}
