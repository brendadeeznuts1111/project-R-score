...
function buildMarkdown(status: LoopStatus): string {
  ...
  lines.push(
    `- Active Warnings: ${status.warnings.length > 0 ? status.warnings.join(', ') : 'none'}`
  );
  ...
  for (const s of status.stages) {
    lines.push(
      `| ${s.id} | ${s.status} | ${s.reason} | ${(s.evidence || []).join('<br/>') || '-'} |`
    );
  }
  lines.push('');
  return `${lines.join('\n')}\n`;
...
async function main(): Promise<void> {
  ...
  const isAligned = Boolean(
    latestSnapshotIdSeen && loopStatusSnapshotId && latestSnapshotIdSeen === loopStatusSnapshotId
  );
  ...
  const hasQualityWarn =
    warnings.includes('quality_drop_warn') ||
    warnings.includes('slop_rise_warn') ||
    warnings.includes('reliability_drop_warn');
  ...
  stages.push(
    latest?.id && strict
      ? stage('cli_search', 'pass', 'Strict profile found in latest snapshot.', [
          latestPath,
          `strict.quality=${Number(strict.qualityScore || 0).toFixed(2)}`,
        ])
  ...
  stages.push(
    latest?.id && latest.deltaBasis === 'same-pack' && Boolean(latest.baselineSnapshotId)
      ? stage('benchmark_snapshot', 'pass', 'Latest snapshot has same-pack baseline lineage.', [
          latestPath,
          `baseline=${latest.baselineSnapshotId}`,
        ])
      : stage(
          'benchmark_snapshot',
          latest?.id ? 'warn' : 'fail',
          latest?.id
            ? 'Latest snapshot exists but same-pack baseline is unavailable.'
  ...
  stages.push(
    coverage && coverage.lines > 0 && coverage.roots.length > 0
      ? stage('coverage_kpi', 'pass', 'Searchable LOC coverage is available.', [
          coveragePath,
          `lines=${coverage.lines}`,
          `roots=${coverage.roots.join(',')}`,
        ])
      : stage('coverage_kpi', coverage ? 'warn' : 'warn', 'Coverage KPI missing or incomplete.', [
          coveragePath,
        ])
  );
  ...
  stages.push(
    hasQualityWarn
      ? stage(
          'signal_quality',
          'warn',
          'Quality/slop/reliability warning present.',
          warnings.filter(w =>
            ['quality_drop_warn', 'slop_rise_warn', 'reliability_drop_warn'].includes(w)
          )
        )
      : stage('signal_quality', 'pass', 'Quality signal stable.', [
          strict ? `quality=${Number(strict.qualityScore || 0).toFixed(2)}` : 'strict=missing',
          strictReliability !== null
            ? `reliability=${strictReliability.toFixed(2)}`
            : 'reliability=n/a',
        ])
  );
  ...
  stages.push(
    hasLatencyWarn
      ? stage('signal_latency', 'warn', 'Latency warning present.', [
          strict ? `strict.p95=${Number(strict.latencyP95Ms || 0).toFixed(2)}ms` : 'strict.p95=n/a',
        ])
      : stage('signal_latency', 'pass', 'Latency signal stable.', [
          strict ? `strict.p95=${Number(strict.latencyP95Ms || 0).toFixed(2)}ms` : 'strict.p95=n/a',
        ])
  );
  ...
  stages.push(
    hasMemoryWarn
      ? stage('signal_memory', 'warn', 'Heap/RSS warning present.', [
          strict
            ? `strict.peakHeap=${Number(strict.peakHeapUsedMB || 0).toFixed(2)}MB`
            : 'strict.peakHeap=n/a',
          strict
            ? `strict.peakRss=${Number(strict.peakRssMB || 0).toFixed(2)}MB`
            : 'strict.peakRss=n/a',
        ])
  ...
  stages.push(
    parityInputsOk
      ? stage(
          'dashboard_parity',
          'pass',
          'Loop status includes snapshot/warnings/coverage for dashboard parity checks.',
          [latestPath, coveragePath]
        )
      : stage('dashboard_parity', 'fail', 'Missing parity inputs for dashboard stage.', [
          latestPath,
          coveragePath,
        ])
  );
  if (!latestSnapshotIdSeen || !loopStatusSnapshotId || !isAligned) {
    stages.push(
      stage(
        ...
        [
          `latestSeen=${latestSnapshotIdSeen || 'none'}`,
          `loopSnapshot=${loopStatusSnapshotId || 'none'}`,
        ]
      )
    );
    ...
    stages.push(
      stage('status_freshness', 'pass', 'Loop status snapshot is aligned and fresh.', [
