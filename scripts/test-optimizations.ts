...
function parseMetrics(report: string): Map<string, ProjectMetrics> {
  ...
  const projectSections = report.matchAll(
    /### (\w+[\w-]*)[\s\S]*?Function Objects.*?(\d{1,3}(?:,\d{3})*)[\s\S]*?Large Objects.*?(\d+)[\s\S]*?GC Roots.*?(\d{1,3}(?:,\d{3})*)/g
  );
...
function compareMetrics(
  ...
  for (const project of allProjects) {
    ...
    const funcPct =
      beforeMetrics.functions > 0
        ? ((funcChange / beforeMetrics.functions) * 100).toFixed(1)
        : '0.0';
    const funcImprovement = funcChange < 0 ? '✅' : funcChange > 0 ? '❌' : '➖';
    console.info(
      `| ${project} | Functions | ${beforeMetrics.functions.toLocaleString()} | ${afterFuncs.toLocaleString()} | ${funcChange > 0 ? '+' : ''}${funcChange.toLocaleString()} | ${funcImprovement} ${funcPct}% |`
    );

    // Large Objects
    const largeChange = afterLarge - beforeMetrics.largeObjects;
    const largePct =
      beforeMetrics.largeObjects > 0
        ? ((largeChange / beforeMetrics.largeObjects) * 100).toFixed(1)
        : '0.0';
    const largeImprovement = largeChange < 0 ? '✅' : largeChange > 0 ? '❌' : '➖';
    console.info(
      `| ${project} | Large Objects | ${beforeMetrics.largeObjects.toLocaleString()} | ${afterLarge.toLocaleString()} | ${largeChange > 0 ? '+' : ''}${largeChange.toLocaleString()} | ${largeImprovement} ${largePct}% |`
    );

    // GC Roots
    const gcChange = afterGC - beforeMetrics.gcRoots;
    const gcPct =
      beforeMetrics.gcRoots > 0 ? ((gcChange / beforeMetrics.gcRoots) * 100).toFixed(1) : '0.0';
    const gcImprovement = gcChange < 0 ? '✅' : gcChange > 0 ? '❌' : '➖';
    console.info(
      `| ${project} | GC Roots | ${beforeMetrics.gcRoots.toLocaleString()} | ${afterGC.toLocaleString()} | ${gcChange > 0 ? '+' : ''}${gcChange.toLocaleString()} | ${gcImprovement} ${gcPct}% |`
    );
  }
...
async function main() {
  ...
  try {
    ...
    console.info(
      `   Functions: ${totalFuncBefore.toLocaleString()} → ${totalFuncAfter.toLocaleString()} (${totalFuncAfter - totalFuncBefore > 0 ? '+' : ''}${(totalFuncAfter - totalFuncBefore).toLocaleString()})`
    );
    console.info(
      `   Large Objects: ${totalLargeBefore.toLocaleString()} → ${totalLargeAfter.toLocaleString()} (${totalLargeAfter - totalLargeBefore > 0 ? '+' : ''}${(totalLargeAfter - totalLargeBefore).toLocaleString()})`
    );
    console.info(
      `   GC Roots: ${totalGCBefore.toLocaleString()} → ${totalGCAfter.toLocaleString()} (${totalGCAfter - totalGCBefore > 0 ? '+' : ''}${(totalGCAfter - totalGCBefore).toLocaleString()})`
    );

    const funcImprovement = (((totalFuncBefore - totalFuncAfter) / totalFuncBefore) * 100).toFixed(
      1
    );
    const largeImprovement =
      totalLargeBefore > 0
        ? (((totalLargeBefore - totalLargeAfter) / totalLargeBefore) * 100).toFixed(1)
        : '0.0';
