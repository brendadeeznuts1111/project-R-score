...
class CouncilTicketFactory {
  ...
  static async initialize() {
    ...
    this.db.run(
      `CREATE INDEX IF NOT EXISTS idx_tickets_factory ON tickets_v4(factory, created_at)`
    );
  ...
  static getStats() {
    const total = this.db.query(`SELECT COUNT(*) as count FROM tickets_v4`).get() as {
      count: number;
    };
    const pending = this.db
      .query(`SELECT COUNT(*) as count FROM tickets_v4 WHERE status = 'pending'`)
      .get() as { count: number };
    const validated = this.db
      .query(`SELECT COUNT(*) as count FROM tickets_v4 WHERE status = 'validated'`)
      .get() as { count: number };
    const disputed = this.db
      .query(`SELECT COUNT(*) as count FROM tickets_v4 WHERE status = 'disputed'`)
      .get() as { count: number };
  ...
  static async validateTickets(threadCount: number = 4): Promise<{
    ...
    const results = await Promise.all(chunks.map(chunk => this.validateChunk(chunk)));
    ...
    for (const ticket of chunk) {
      try {
        ...
        if (ticket.hash !== expectedHash) {
          throw new Error(
            `Hash mismatch: ${ticket.hash.substring(0, 16)}... vs ${expectedHash.substring(0, 16)}...`
          );
        }
...
class CouncilDisputeResolution {
  ...
  static getStats() {
    const active = this.db
      .query(`SELECT COUNT(*) as count FROM disputes WHERE outcome IS NULL`)
      .get() as { count: number };
    const resolved = this.db
      .query(`SELECT COUNT(*) as count FROM disputes WHERE outcome IS NOT NULL`)
      .get() as { count: number };
  ...
  static async createDispute(ticketId: string, scope: string, councilMembers: string[]) {
    ...
    this.db.run(
      ...
      VALUES (?, ?, ?, ?, ?)
    `,
      [disputeId, ticketId, scope, JSON.stringify(councilMembers), JSON.stringify({})]
    );
