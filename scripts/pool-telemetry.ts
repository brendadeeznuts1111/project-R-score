...
class TelemetryPool {
  ...
  async insertProfile(
    ...
    try {
      ...
      db.run('INSERT OR REPLACE INTO profiles VALUES (?, ?, ?, ?, ?, ?)', [
        profileId,
        sessionId,
  ...
  async syncToR2(): Promise<void> {
    ...
    try {
      ...
      const allSessions = this.db.query(
        'SELECT * FROM sessions ORDER BY last_activity DESC LIMIT 50'
      );
      const allAnalytics = this.db.query(
      ...
      console.info(
        `✅ R2 sync complete: ${allProfiles.length} profiles in ${syncTime.toFixed(2)}ms`
      );
    } catch (error) {
  ...
  async getPoolStats(): Promise<any> {
    return {
      ...
      hitRate:
        this.poolStats.totalOperations > 0
          ? ((this.poolStats.hits / this.poolStats.totalOperations) * 100).toFixed(2) + '%'
          : '0%',
    };
  }
  ...
  private updateStats(latency: number, wasPoolHit: boolean) {
    ...
    this.poolStats.avgLatency =
      (this.poolStats.avgLatency * (this.poolStats.totalOperations - 1) + latency) /
      this.poolStats.totalOperations;
  }
...
export async function juniorProfilePooled(
  mdFile: string,
  pool: TelemetryPool,
  member: string = 'anonymous'
): Promise<string> {
  ...
  try {
    ...
    const sessionId = (globalThis as any).Bun.CryptoHasher('sha256')
      .update(Date.now().toString() + mdFile)
      .digest('hex')
      .slice(0, 8);
    ...
    try {
      switch (command) {
        ...
        case 'insert':
          ...
          const insertId = await pool.insertProfile(
            'manual-session',
            profileData,
            'manual-user',
            'manual-doc'
        ...
        case 'query':
          ...
          sessions.forEach(session => {
            console.info(
              `  ${session.id}: ${session.member} | ${new Date(session.timestamp).toISOString()}`
            );
          });
          break;
