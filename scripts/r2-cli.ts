...
function getSecret(name: string, service: string): Promise<string | null> {
  const secrets = (
    Bun as unknown as {
      secrets?: { get: (opts: { service: string; name: string }) => Promise<string | null> };
    }
  ).secrets;
...
async function loadConfig(): Promise<R2Config> {
  ...
  if (!cfg.accountId || !cfg.accessKeyId || !cfg.secretAccessKey || !cfg.bucketName) {
    throw new Error(
      'Missing R2 configuration. Provide Bun.secrets or env vars (R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY, R2_BUCKET_NAME).'
    );
  }
  return cfg;
}
...
async function listObjects(cfg: R2Config, prefix = ''): Promise<void> {
  const url = new URL(
    `https://api.cloudflare.com/client/v4/accounts/${cfg.accountId}/r2/buckets/${cfg.bucketName}/objects`
  );
