...
function pickMode(id: string): DemoBaseline['benchmark']['mode'] {
  if (id.includes('performance') || id.includes('markdown') || id.includes('stringwidth'))
    return 'string';
  if (id.includes('protocol') || id.includes('governance') || id.includes('health'))
    return 'map-size';
  return 'hash';
}
...
export function buildBaselineForDemo(
  ...
  return {
    ...
    sourceIds:
      Array.isArray(override?.sourceIds) && override.sourceIds.length > 0
        ? override.sourceIds
        : pickSources(id),
  };
}

export function validateTier1SourcesForDemo(id: string): {
  ok: boolean;
  errors: string[];
...
if (import.meta.main) {
  ...
  console.info(
    `[demo-tier1][pass] demos=${ids.length} providers=${new Set(Object.values(TIER1_SOURCES).map(s => s.provider)).size}`
  );
}
