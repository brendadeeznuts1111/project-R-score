 ...
 * - `(this as Record<string, unknown>).prop` -> `(this as Record<string, unknown>).prop`
 * - `(error as Record<string, unknown>).prop` -> `(error as Record<string, unknown>).prop`
 * - `(value as Record<string, unknown>).prop` -> `(value as Record<string, unknown>).prop`
 * - `(config as Record<string, unknown>).prop` -> `(config as Record<string, unknown>).prop`
 * - `(obj as unknown[])` -> `(obj as unknown[])`
 * - `(Bun as Record<string, unknown>).method` -> `(Bun as Record<string, unknown>).method`
 */

const argv = import.meta.main
  ? applyUnknownLongOptionGuardFor('fix:as-any', Bun.argv.slice(2))
  : Bun.argv.slice(2);
...
for (const file of walkFiles()) {
  ...
  // Apply general pattern: (variable as Record<string, unknown>).prop -> (variable as Record<string, unknown>).prop
