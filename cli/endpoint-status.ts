...
const DEFAULT_ENDPOINTS: Endpoint[] = [
  ...
  { name: 'R2 Storage', url: 'http://localhost:3000/r2/status', method: 'GET', category: 'storage' },
  { name: 'RSS Feeds', url: 'http://localhost:3000/rss/status', method: 'GET', category: 'feeds' },
  { name: 'Registry', url: 'http://localhost:3000/registry/status', method: 'GET', category: 'registry' },
];
...
function displayEndpointResults(results: EndpointResult[], context: ContextType = 'dark'): void {
  ...
  console.log(colorize('  ENDPOINT STATUS'.padEnd(30) + 'CODE'.padEnd(8) + 'TIME'.padEnd(10) + 'STATUS', 'white', true));
  console.log(colorize('═'.repeat(90), 'gray'));
  ...
  for (const category of categories) {
    ...
    for (const result of categoryResults) {
      ...
      const name = result.endpoint.name.padEnd(28);
      const code = result.statusCode.toString().padEnd(8);
  ...
  console.log(`  Total: ${total} | ✅ Success: ${success} | ⚠️  Warning: ${warning} | ❌ Error: ${error}`);
 ...
 */
async function watchEndpoints(endpoints: Endpoint[], interval = 5000, context: ContextType = 'dark'): Promise<void> {
