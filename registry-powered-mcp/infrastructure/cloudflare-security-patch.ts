// infrastructure/cloudflare-security-patch.ts
...
export class CloudflareSecurityPatch {
  ...
  private static patchBufferBigIntMethods(): void {
    const methods = ['writeBigInt64LE', 'writeBigInt64BE', 'writeBigUInt64LE', 'writeBigUInt64BE'];

    for (const method of methods) {
      const original = (Buffer.prototype as any)[method];

      (Buffer.prototype as any)[method] = function(
        value: bigint,
        offset: number = 0
      ): number {
        ...
        const max = method.includes('UInt') ? BigInt('0xFFFFFFFFFFFFFFFF') : BigInt('0x7FFFFFFFFFFFFFFF');
        const min = method.includes('UInt') ? BigInt(0) : BigInt('-0x8000000000000000');
  ...
  private static patchProcessTitle(): void {
    const originalSetter = Object.getOwnPropertyDescriptor(process, 'title')?.set;
        ...
        set: function(value: string) {
          ...
          for (let i = 0; i < value.length; i++) {
            ...
            if (code >= 0xD800 && code <= 0xDFFF) {
              ...
              if (code <= 0xDBFF) {
                if (i + 1 >= value.length || value.charCodeAt(i + 1) < 0xDCFF || value.charCodeAt(i + 1) > 0xDFFF) {
                  throw new Error("Invalid UTF-16 surrogate pair in process.title");
                }
  ...
  private static patchReadableStream(): void {
    ...
    if (ReadableStream) {
      const originalConstructor = ReadableStream.prototype.constructor;

      ReadableStream.prototype.constructor = function(underlyingSource: any, strategy?: any) {
        try {
          return new originalConstructor(underlyingSource, strategy);
        } catch (error: any) {
          // Proper exception handling
          if (error.message?.includes("Response.prototype.body")) {
            throw new TypeError("ReadableStream initialization failed for Response.body");
          }
          throw error;
        }
      };
