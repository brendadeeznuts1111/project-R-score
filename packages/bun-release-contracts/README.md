...
file's presence. The tracker's **Under consideration** section did not ship.
The release inventory marks a row `covered` only when its assigned suite makes
an exact assertion; environment-only and partial probes remain `planned`.
