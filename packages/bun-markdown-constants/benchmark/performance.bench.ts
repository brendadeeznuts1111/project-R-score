...
group('Bun.markdown.html() - SIMD Accelerated', () => {
  ...
  console.info(
    `  ${r1.name}: ${formatNumber(r1.opsPerSec)} ops/sec (${r1.avgTime.toFixed(2)} ns/op)`
  );
  ...
  console.info(
    `  ${r2.name}: ${formatNumber(r2.opsPerSec)} ops/sec (${r2.avgTime.toFixed(2)} ns/op)`
  );
  ...
  console.info(
    `  ${r3.name}: ${formatNumber(r3.opsPerSec)} ops/sec (${r3.avgTime.toFixed(2)} ns/op)`
  );
});
...
group('AbortSignal.abort() - No Listener Optimization', () => {
  ...
  console.info(
    `  ${r1.name}: ${formatNumber(r1.opsPerSec)} ops/sec (${(1000000000 / r1.opsPerSec).toFixed(2)} ns/op)`
  );
  ...
  console.info(
    `  ${r2.name}: ${formatNumber(r2.opsPerSec)} ops/sec (${(1000000000 / r2.opsPerSec).toFixed(2)} ns/op)`
  );
});
...
group('RegExp - SIMD Acceleration', () => {
  ...
  const r1 = bench(
    'SIMD prefix search (alternatives)',
    () => {
