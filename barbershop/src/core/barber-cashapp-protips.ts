...
export class NewAccountManager {
  ...
  private static async addToNurtureProgram(userId: string, paymentData: any) {
    await redis.hmset(`nurture:${userId}`, [
      ...
      'firstPayment', paymentData.amount,
      'stage', 'welcome'
    ]);
  }
}
...
export class BarberProTips {
  ...
  static async applyNewCustomerPromotion(customerId: string, barberId: string, service: string) {
    ...
    const firstService = db.query('SELECT COUNT(*) as count FROM services WHERE customer_id = ? AND barber_id = ?')
...
export const CashAppLimits = {
  ...
  established: { // 90+ days
    ...
    tips: [
      ...
      "Ask for monthly subscription",
      "Perfect for family bookings"
    ]
  }
};
