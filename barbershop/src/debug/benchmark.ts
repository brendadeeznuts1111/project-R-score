...
function benchmarkLogging(iterations: number = 10000) {
  ...
  console.log(`Console.log: ${consoleAvg.toFixed(4)}ms per call (${consoleTotal.toFixed(2)}ms total)`);
  console.log(`Structured Log: ${loggerAvg.toFixed(4)}ms per call (${loggerTotal.toFixed(2)}ms total)`);
...
async function runFullBenchmark() {
  ...
  console.log(`✅ Memory Efficiency: ${((memoryResults.memoryIncrease - memoryResults.memoryAfterGC) / memoryResults.memoryIncrease * 100).toFixed(1)}% reduction`);
  console.log(`✅ Filtering Performance: ${filteringResults.entriesPerSecond.toFixed(0)} entries/sec`);
