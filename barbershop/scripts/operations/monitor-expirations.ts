...
class ExpirationMonitor {
  ...
  async start(): Promise<void> {
    ...
    console.log(styled(`   Mode: ${this.options.daemon ? 'Daemon (continuous)' : 'One-time check'}`, 'muted'));
    ...
    console.log(styled(`   Slack alerts: ${this.options.slackAlerts ? 'enabled' : 'disabled'}`, 'muted'));
    console.log(styled(`   R2 reports: ${this.options.r2Reports ? 'enabled' : 'disabled'}`, 'muted'));
    console.log(styled(`   Email alerts: ${this.options.emailAlerts ? 'enabled' : 'disabled'}`, 'muted'));
    console.log();
  ...
  private async performCheck(): Promise<void> {
    ...
    try {
      ...
      console.log(styled(`   ✅ Check completed (${report.summary.total} secrets monitored)`, 'success'));
  ...
  private async generateExpirationReport(expiring: Array<{ key: string; expiresAt: Date; daysLeft: number }>, timestamp: Date): Promise<ExpirationReport> {
    const now = new Date();
    ...
    for (const secret of expiring) {
      try {
        ...
        const severity = secret.daysLeft <= 1 ? 'CRITICAL' : 
                         secret.daysLeft <= this.options.threshold ? 'WARNING' : 'INFO';
  ...
  private displaySummary(report: ExpirationReport): void {
    ...
    if (report.summary.warning > 0) {
      console.log(styled(`      ⏳ Warning (≤${this.options.threshold} days): ${report.summary.warning}`, 'warning'));
    }
    
    if (report.summary.expired === 0 && report.summary.critical === 0 && report.summary.warning === 0) {
      console.log(styled('      ✅ No secrets expiring soon', 'success'));
    }
    
    // Show critical/expired details
    const criticalItems = [...report.expired, ...report.expiring.filter(s => s.severity === 'CRITICAL')];
    if (criticalItems.length > 0) {
      console.log(styled('   🚨 Immediate attention required:', 'error'));
      criticalItems.forEach(item => {
        const daysText = 'daysExpired' in item ? `${item.daysExpired} days expired` : `${item.daysLeft} days left`;
