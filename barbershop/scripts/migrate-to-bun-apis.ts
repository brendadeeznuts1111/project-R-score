...
const PATTERNS = {
  ...
  buffer: [/Buffer\.alloc/, /Buffer\.from/, /Buffer\.concat/, /new Buffer\(/],
  timeout: [/setTimeout\(/, /setInterval\(/, /new Promise.*setTimeout/],
};
...
const SUGGESTIONS: Record<string, string> = {
  ...
  // Console
  'console.info(': 'Use logger.info() or logger.debug()',
      ...
      for (const pattern of patterns) {
        if (pattern.test(line)) {
          ...
          if (match) {
            ...
            const suggested =
              SUGGESTIONS[current] || `Consider Bun-native alternative for ${current}`;
...
function printReport(issues: MigrationIssue[]) {
  console.info('\n╔════════════════════════════════════════════════════════════════════════════╗');
  console.info('║                    BUN API MIGRATION REPORT                                ║');
  console.info('╚════════════════════════════════════════════════════════════════════════════╝\n');

  if (issues.length === 0) {
    console.info(`${ANSI.green}✅ No migration issues found!${ANSI.reset}\n`);
    return;
  }
  ...
  for (const [type, typeIssues] of Object.entries(byType)) {
    const color =
      type === 'console' ? ANSI.yellow : type === 'fs' || type === 'crypto' ? ANSI.cyan : ANSI.dim;
    console.info(`  ${color}${type.padEnd(15)}${ANSI.reset}: ${typeIssues.length} occurrences`);
  }
  console.info(`\n  ${ANSI.bold}Total:${ANSI.reset} ${issues.length} issues\n`);

  // Detailed report
  console.info(`${ANSI.bold}Details:${ANSI.reset}\n`);
      ...
      issue.type === 'console'
        ...
        : issue.type === 'fs'
          ...
          : issue.type === 'crypto'
            ? ANSI.blue
            : ANSI.dim;
    console.info(`  ${typeColor}[${issue.type}]${ANSI.reset} Line ${issue.line}: ${issue.current}`);
    console.info(`    ${ANSI.dim}→ ${issue.suggested}${ANSI.reset}`);
  }
...
async function main() {
  ...
  // Auto-fix
  if (shouldFix) {
    console.info(`${ANSI.bold}🔧 Applying auto-fixes...${ANSI.reset}\n`);
    ...
    for (const file of filesToFix) {
      ...
      if (fixes > 0) {
        console.info(`  ${ANSI.green}✓${ANSI.reset} ${file}: ${fixes} fixes`);
        totalFixes += fixes;
      }
    }
    ...
    console.info(
      `${ANSI.yellow}⚠️${ANSI.reset} Note: Some changes require manual review (fs, crypto, console)\n`
    );
  } else {
    console.info(`${ANSI.dim}Run with --fix to apply safe auto-fixes${ANSI.reset}\n`);
  }
}
