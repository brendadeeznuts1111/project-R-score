...
- **Task routing:**
  ...
  - Import cycles / deep relative imports → `bun run check:import-graph` (pre-commit when `lib/`/`scripts/` staged; ratchets may only go down — cycles currently pinned 15, `../../../` pinned 7; owner re-pin: `bun scripts/check-import-graph.ts --write-baseline`; known cycle clusters: `lib/telegram/seat-*`, `lib/channels/outbox` ↔ `lib/telegram/broadcast`, `lib/toc-ops`)
