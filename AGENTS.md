...
bindings) · `cloudflare-builds` (Workers Builds CI — not Pages deploy history) ·
`cloudflare-observability` (logs/metrics). Token SSOT: Proton Pass
...
| CLI flag allowlists / unknown `--*` guard        | [`docs/harness/cli-constants-flags.md`](docs/harness/cli-constants-flags.md) · [`lib/docs/ref-id-tool-flags.ts`](lib/docs/ref-id-tool-flags.ts) · `ALLOWED_LONG_REGISTRY` · **`BUN_STRIP_UNKNOWN`** · `bun run cli:flags:check` · `bun test tests/docs-ref-id-tool-exports.test.ts`                                                                                                                                                                     |
| Bun PR binary verify (`bunx bun-pr`)             | [`tools/bun-pr-verify.ts`](tools/bun-pr-verify.ts) · `bun run bun:pr:fetch` · `bun run bun:pr:verify -- <pr>` · [upstream contributing](https://bun.com/docs/project/contributing#download-release-build-from-pull-requests) · tenant [`docs/harness/tenants/bun-upstream-contributing.md`](docs/harness/tenants/bun-upstream-contributing.md) · allowlist `bun:pr:verify` (`proof` · `json`) |
