...
- **Task routing:**
  ...
  - Import cycles / deep relative imports → `bun run check:import-graph` (pre-commit when `lib/`/`scripts/` staged; ratchets may only go down — 9 strong + 5 weak cycles pinned, `../../../` pinned 7; owner re-pin: `bun scripts/check-import-graph.ts --write-baseline`; strong-cycle clusters: `lib/telegram/seat-*`, `lib/toc-ops`; weak cycles = lazy `import()` edge present)
