...
- **Task routing:**
  ...
  - Import cycles / deep relative imports → `bun run check:import-graph` (pre-commit when `lib/`/`scripts/` staged; ratchets may only go down — **0 strong** + 5 weak cycles pinned (telegram seat/registry clusters broken 2026-07-28 via `seat-intake.ts` / `seat-desk-forum-post.ts` / `flows/seat-telegram-id.ts` leaf extractions), `../../../` pinned 7; owner re-pin: `bun scripts/check-import-graph.ts --write-baseline`; weak cycles = lazy `import()` edge present)
