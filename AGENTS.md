...
| **Linker policy verification** | config | Bun ≥1.4 | `bun.lock` `configVersion` field | — | `install:verify` · `portal-cli doctor` (linker-config-version) | Implemented | [Default strategy](https://bun.com/docs/pm/cli/install#default-strategy) · [isolated installs](https://bun.com/docs/pm/isolated-installs) | `bun run portal-cli doctor` · `bun run install:verify` |
