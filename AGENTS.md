...
| **Memory optimization** | runtime | Bun ≥1.0 | `bun --smol …` | — | `portal-cli --smol vault health` (child spawn) | Implemented | [runtime · --smol](https://bun.com/docs/runtime#runtime-process-control) | `bun tools/portal-cli.ts --smol vault health` |
| **Watch mode** | runtime | Bun ≥1.0 | `bun --watch …` | — | `portal-cli --watch probe …` (child restart) | Implemented | [runtime · --watch](https://bun.com/docs/runtime#development-workflow) | `bun tools/portal-cli.ts --watch vault health` |
...
| **Force Bun runtime** | runtime | Bun ≥1.0 | `bun --bun` / `-b` | — | override node shebangs in children | Implemented | [runtime · --bun](https://bun.com/docs/runtime#runtime-process-control) | `bun tools/portal-cli.ts --bun pm ls` |
| **Debugger** | runtime | Bun ≥1.0 | `bun --inspect` / `--inspect-brk` | — | portal-cli → child bun | Implemented | [runtime · debugging](https://bun.com/docs/runtime#debugging) | `bun tools/portal-cli.ts --inspect vault health` |
| **Module preload** | runtime | Bun ≥1.0 | `bun --preload` / `-r` | — | inject module before portal-cli child entry | Implemented | [runtime · --preload](https://bun.com/docs/runtime#dependency-module-resolution) | `bun tools/portal-cli.ts --preload ./x.ts probe` |
...
| **Snapshot testing** | test | Bun ≥1.0 | `expect().toMatchSnapshot()` | — | `portal vault health` · report/inventory SSOT | Implemented | [bun test snapshots](https://bun.com/docs/test/snapshots) | `expect(report).toMatchSnapshot()` |
| **Update snapshots** | test | Bun ≥1.0 | `bun test --update-snapshots` | — | `portal vault health --update` | Implemented | same | `bun test tests/vault-health.test.ts --update-snapshots` |
...
| **Runtime flags catalog** | cli | Bun ≥1.4 | `--watch`, `--hot`, `--inspect`, `--smol`, `--no-install`, `--console-depth`, … (17 flags) | — | `config/runtime-flags.json` · `portal-cli runtime --list` | Implemented | [bun.sh/docs/runtime](https://bun.com/docs/runtime#general-execution-options) | `bun run portal-cli runtime --list` |
| **Runtime inspect** | cli | Bun ≥1.4 | `Bun.argv` + `Bun.inspect.table()` | — | `portal-cli runtime --effective` | Implemented | [runtime/utils](https://bun.com/docs/runtime/utils) | `bun run portal-cli runtime --effective` |
