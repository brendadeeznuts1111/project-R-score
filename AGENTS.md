...
| **Structured debug output** | dev | Bun ≥1.0 | `Bun.inspect(obj, { colors: true, depth: 4 })` | — | `snapshot run --debug` | Implemented | [Bun.inspect](https://bun.com/docs/runtime/utils#bun-inspect) | `console.log(Bun.inspect(manifest, { colors: true }));` |
| **Inspect table** | display | Bun ≥1.0 | `Bun.inspect.table(rows, cols?, { colors })` | — | `portal-cli pm graph` · `capabilities doctor` fail table | Implemented | [Bun.inspect.table](https://bun.com/docs/runtime/utils#bun-inspect-table-tabulardata-properties-options) | `console.log(Bun.inspect.table(failing, ["capability", "field", "range", "actual"]))` |
...
| **String width (Unicode)** | display | Bun ≥1.0 | `Bun.stringWidth(s)` | — | snapshot-core table pad · CLI columns | Implemented | [Bun.stringWidth](https://bun.com/docs/runtime/utils#bun-stringwidth) | `s + " ".repeat(Math.max(0, w - Bun.stringWidth(s)))` |
| **Strip ANSI** | display | Bun ≥1.0 | `Bun.stripANSI(text)` | — | doctor / CLI plain-text width · logs | Implemented | [Bun.stripANSI](https://bun.com/docs/runtime/utils#bun-stripansi) | `Bun.stripANSI("\u001b[31mred\u001b[0m")` |
| **Wrap ANSI text** | display | Bun ≥1.0 | `Bun.wrapAnsi(text, columns, opts?)` | — | doctor fail lines · long CLI help | Implemented | [Bun.wrapAnsi](https://bun.com/docs/runtime/utils#bun-wrapansi) | `Bun.wrapAnsi(line, 80, { hard: false })` |
| **file:// ↔ path** | io | Bun ≥1.0 | `Bun.fileURLToPath` · `Bun.pathToFileURL` | — | import.meta.url → abs path · path → URL | Implemented | [utils fileURL](https://bun.com/docs/runtime/utils#bun-fileurltopath) | `Bun.fileURLToPath(import.meta.url)` |
| **GZIP compress** | compress | Bun ≥1.0 | `Bun.gzipSync` · `Bun.gunzipSync` | — | artifact packs · oneliners proof | Available | [Bun.gzipSync](https://bun.com/docs/runtime/utils#bun-gzipsync) | `Bun.gunzipSync(Bun.gzipSync(buf))` |
| **DEFLATE compress** | compress | Bun ≥1.0 | `Bun.deflateSync` · `Bun.inflateSync` | — | wire payloads (future) | Available | [Bun.deflateSync](https://bun.com/docs/runtime/utils#bun-deflatesync) | `Bun.inflateSync(Bun.deflateSync(buf))` |
| **Zstandard compress** | compress | Bun ≥1.0 | `Bun.zstdCompressSync` · `Bun.zstdDecompressSync` | — | large registry bakes (future) | Available | [Bun.zstdCompressSync](https://bun.com/docs/runtime/utils#bun-zstdcompress-bun-zstdcompresssync) | `Bun.zstdDecompressSync(Bun.zstdCompressSync(buf, { level: 6 }))` |
| **High-precision timer** | perf | Bun ≥1.0 | `Bun.nanoseconds()` | — | bake:all step timing · doctor elapsed | Implemented | [Bun.nanoseconds](https://bun.com/docs/runtime/utils#bun-nanoseconds) | `const t0 = Bun.nanoseconds()` |
| **ReadableStream consumers** | io | Bun ≥1.0 | `Bun.readableStreamToText` · `ToBytes` · `ToJSON` · … | — | fetch body → text/json (snapshots) | Available | [readableStreamTo*](https://bun.com/docs/runtime/utils#bun-readablestreamto) | `await Bun.readableStreamToText(res.body)` |
...
| **Curated runtime flags catalog** | cli | Bun ≥1.0 | 14 documented runtime flags | — | `config/runtime-flags.json` · `portal-cli --help` | Implemented | [runtime options](https://bun.com/docs/runtime/index#general-execution-options) | `bun tools/portal-cli.ts --help` |
