...
export function runDemo() {
  ...
  for (const [tier, data] of Object.entries(SampleAccountAges)) {
    ...
    if (result.errors.length > 0) {
      logger.debug(`Validation errors for ${tier}`, { errors: result.errors.map(e => e.message) }, 'FUSION');
    }
  }
  
  logger.fusion('Testing utilities');
  
  // Test utilities
  const accountAge5 = FusionUtils.getAccountAgeFromDays(5);
  const accountAge45 = FusionUtils.getAccountAgeFromDays(45);
  const accountAge400 = FusionUtils.getAccountAgeFromDays(400);
  ...
  const newRate = FusionUtils.calculateEffectiveRate(0.6, 'new');
  const veteranRate = FusionUtils.calculateEffectiveRate(0.6, 'veteran');
  
  logger.debug('Effective rate calculations', {
    'new account': newRate,
    'veteran': veteranRate
  }, 'FUSION');
  
  const newCasualActions = FusionUtils.getActionsFor('new', 'casual').map(a => a.id);
  const veteranWhaleActions = FusionUtils.getActionsFor('veteran', 'whale').map(a => a.id);
  
  logger.debug('Action mappings', {
    'new + casual': newCasualActions,
    'veteran + whale': veteranWhaleActions
  }, 'FUSION');
  
  // Test database
  logger.fusion('Testing database');
  const db = new FusionDatabase();
  const allAges = db.getAllAccountAges();
  logger.debug('Account ages retrieved from database', { 
    ages: allAges.map((a: any) => a.tier),
    count: allAges.length 
  }, 'FUSION');
  
  // Test caching (if Redis available)
  logger.fusion('Testing cache');
  FusionCache.cacheAccountAge('new', SampleAccountAges.new).then(() => {
    logger.debug('Successfully cached new account age', { tier: 'new' }, 'FUSION');
  });
  
  logger.info('Demo completed successfully', {}, 'FUSION');
}
