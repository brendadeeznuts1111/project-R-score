...
async function autoAssignTicket(ticket: Ticket): Promise<boolean> {
  ...
  if (bestBarber && bestScore >= 50) {
    ...
    logger.tickets(`Auto-assigned ticket to barber`, {
      ticketId: ticket.id,
      barberName: bestBarber.name,
...
async function seedData(): Promise<void> {
  ...
  logger.tickets('Barbers seeded successfully', { count: 3 }, 'SETUP');
}
...
logger.tickets('Barbershop Ticket System started successfully', {
  port: server.port,
  endpoints: {
    createTicket: '/ticket/create',
    barberLogin: '/barber/login',
    barberComplete: '/barber/complete',
    pendingTickets: '/tickets/pending',
    managerClear: '/manager/clear',
    endOfDay: '/manager/end-of-day',
