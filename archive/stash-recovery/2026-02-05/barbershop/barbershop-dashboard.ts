...
function pushError(entry: AppErrorEntry) {
  errorLog.push(entry);
  ...
  <body>
    ...
    <div class="card">
      ...
      <ul>
        ...
        <li><a href="/docs/runtime-config"><code>/docs/runtime-config</code></a> - Parsed runtime JSONC config</li>
        <li><a href="/docs/archive/list"><code>/docs/archive/list</code></a> - Archive file list (JSON)</li>
        <li><a href="/docs/archive.tar"><code>/docs/archive.tar</code></a> - Docs bundle archive (tar)</li>
        <li><a href="/docs/archive.tar.gz"><code>/docs/archive.tar.gz</code></a> - Docs bundle archive (gzip)</li>
      </ul>
    </div>
...
const server = serve({
  ...
  routes: {
    ...
    '/docs/manifest.json': () =>
      Response.json(manifestData, { headers: responseHeaders('application/json; charset=utf-8') }),
    '/docs/readme': async () => {
    ...
    '/docs/runtime-config': () => jsonResponse({ ok: true, config: runtimeConfig }),
    '/docs/archive/list': handled('/docs/archive/list', async () => {
      const files = await docsArchiveMap();
      return jsonResponse({ ok: true, files: Object.keys(files), count: Object.keys(files).length });
    }),
    '/docs/archive.tar': handled('/docs/archive.tar', async () => {
    ...
    '/docs/archive.tar.gz': handled('/docs/archive.tar.gz', async () => {
    ...
    '/ops/fetch-check': handled('/ops/fetch-check', async (req, requestId) => {
      const url = new URL(req.url);
      const target = url.searchParams.get('url') || 'https://example.com';
      if (!isPublicHttpUrl(target)) {
        throw new HttpError(400, 'INVALID_TARGET_URL', 'Only public http/https URLs are allowed.', { target });
      }
      const method = (url.searchParams.get('method') || 'GET').toUpperCase();
      const allowedMethods = new Set(['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'HEAD', 'OPTIONS']);
      if (!allowedMethods.has(method)) {
        throw new HttpError(400, 'INVALID_METHOD', `Unsupported method: ${method}`, { method });
      }
      ...
      if (headersRaw) {
        try {
          ...
          throw new HttpError(400, 'INVALID_HEADERS_JSON', 'Invalid headers JSON');
        }
      }
      ...
      if (bodyJsonRaw != null) {
        try {
          ...
          throw new HttpError(400, 'INVALID_BODY_JSON', 'Invalid body_json value');
        }
      } else {
    ...
    '/ops/runtime': handled('/ops/runtime', (req) => {
    ...
    '/ops/r2-status': handled('/ops/r2-status', async () => {
    ...
    '/health': handled('/health', async () => {
      const [pending, assigned, completed] = await Promise.all([
        redis.smembers('tickets:pending'),
        redis.smembers('tickets:assigned'),
        redis.smembers('tickets:completed')
      ]);
      const status = r2MirrorState.lastError ? 'degraded' : 'ok';
      return jsonResponse(
        {
          ok: status === 'ok',
          status,
          ...
          r2: {
            mode: r2MirrorConfig.mode,
            configured: r2MirrorConfig.enabled,
            connected: r2MirrorState.connected
          }
        },
        status === 'ok' ? 200 : 503
      );
    }),
    '/ops/status': handled('/ops/status', async () => {
      const [pending, assigned, completed] = await Promise.all([
        redis.smembers('tickets:pending'),
        redis.smembers('tickets:assigned'),
        redis.smembers('tickets:completed')
    ...
    '/ops/lifecycle': handled('/ops/lifecycle', (req) => {
      const url = new URL(req.url);
      const key = url.searchParams.get('key');
      if (key !== LIFECYCLE_KEY) {
        throw new HttpError(401, 'UNAUTHORIZED', 'Unauthorized lifecycle key');
      }
    ...
    '/admin/data': handled('/admin/data', async () => {
      ...
      return jsonResponse({ ok: true, connections, telemetry, financials });
    }),
    '/admin/orders': handled('/admin/orders', async () => {
    ...
    '/barbers': handled('/barbers', async () => {
      ...
      return jsonResponse({ ok: true, barbers });
    }),
    
    // BARBER ENDPOINTS
    '/barber': () => textResponse(BARBER_DASHBOARD_V2, 'text/html; charset=utf-8'),
    '/barber/login': handled('/barber/login', async (req) => {
      const body = await parseJsonBody(req) as { code?: string };
      const code = String(body.code ?? '').trim();
      if (!code) {
        throw new HttpError(400, 'MISSING_BARBER_CODE', 'Missing barber code');
      }
      const barberId = await redis.get(`barber:code:${code.toUpperCase()}`);
      if (!barberId) throw new HttpError(401, 'INVALID_BARBER_CODE', 'Invalid barber code');
      ...
      return jsonResponse({ ok: true, success: true, barber, tickets });
    }),
    '/barber/stats': handled('/barber/stats', async (req) => {
      const url = new URL(req.url);
      const barberId = url.searchParams.get('barberId');
      if (!barberId) throw new HttpError(400, 'MISSING_BARBER_ID', 'Missing barberId');
      ...
      return jsonResponse({ ok: true, barberId, ticketsCompleted, tipsShared, ordersSeen });
    }),
    
    // API ENDPOINTS (from previous implementation)
    '/checkout/bundle': handled('/checkout/bundle', async (req) => {
      const body = await parseJsonBody(req) as {
        items?: Array<Record<string, unknown>>;
        tip?: { mode?: string; value?: number };
        customerName?: string;
        paymentId?: string;
      };
      const items = Array.isArray(body.items) ? body.items : [];
      if (items.length === 0) {
        throw new HttpError(400, 'MISSING_ITEMS', 'No checkout items provided');
      }
    ...
    '/ticket/create': handled('/ticket/create', async (req) => {
      const body = await parseJsonBody(req) as {
        customerName?: string;
      ...
      if (!body.customerName) {
        throw new HttpError(400, 'MISSING_CUSTOMER_NAME', 'customerName is required');
      }
      if (!Array.isArray(body.services) || body.services.length === 0) {
        throw new HttpError(400, 'MISSING_SERVICES', 'services must be a non-empty array');
      }
      if (!Number.isFinite(Number(body.totalAmount))) {
        throw new HttpError(400, 'INVALID_TOTAL_AMOUNT', 'totalAmount must be numeric');
      }
    ...
    '/tickets/pending': handled('/tickets/pending', async () => {
      const ids = await redis.smembers('tickets:pending');
      return jsonResponse({ ok: true, count: ids.length });
    }),
    '/barber/complete': handled('/barber/complete', async (req) => {
      const body = await parseJsonBody(req) as { ticketId?: string };
      const ticketId = String(body.ticketId ?? '').trim();
      if (!ticketId) {
        throw new HttpError(400, 'MISSING_TICKET_ID', 'ticketId is required');
      }
      ...
      return jsonResponse({ ok: true, success: true, ticketId });
    }),
    '/action': handled('/action', async (req) => {
      ...
      if (!(profilePicture instanceof Blob)) {
        throw new HttpError(400, 'MISSING_PROFILE_PICTURE', 'Must upload a profile picture.');
      }
      ...
      return jsonResponse({ ok: true, success: true, name: safeName, file, path, ip });
    }),
    '/ws/dashboard': (req) => {
  ...
  fetch(req) {
    const requestId = makeRequestId();
    return jsonError(404, 'NOT_FOUND', 'Route not found', requestId, { path: new URL(req.url).pathname });
  },
  error(error) {
    const requestId = makeRequestId();
    const message = error instanceof Error ? error.message : String(error);
    ...
    return jsonError(
      ...
      NODE_ENV === 'development' ? message : 'Unhandled server error',
      requestId
    );
  },
  websocket: {
...
logger.dashboard('Server started successfully', {
serverName: SERVER_NAME,
host: SERVER_HOST,
port: server.port,
baseUrl: PUBLIC_BASE_URL,
adminUrl: `${PUBLIC_BASE_URL}/admin`,
clientUrl: `${PUBLIC_BASE_URL}/client`,
barberUrl: `${PUBLIC_BASE_URL}/barber`,
websocketUrl: `${PUBLIC_BASE_URL.replace(/^http/, 'ws')}/ws/dashboard`,
adminKey: 'godmode123',
barberCodes: ['JB', 'MS', 'CK']
});
...
function shutdown(signal: string) {
  ...
  logger.dashboard(`Graceful shutdown via ${signal}`, { signal, serverName: SERVER_NAME });
  process.exit(0);
}
