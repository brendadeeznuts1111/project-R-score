...
class BenchmarkHelper {
  ...
  printComparison(comparison: ComparisonResult): void {
    const { baseline, comparison: comparisonData, improvement, faster } = comparison;

    console.log(`\n⚡ Performance Comparison`);
    console.log(`   Baseline: ${baseline.name} - ${baseline.avgTimeMs.toFixed(4)}ms`);
    console.log(`   Comparison: ${comparisonData.name} - ${comparisonData.avgTimeMs.toFixed(4)}ms`);
