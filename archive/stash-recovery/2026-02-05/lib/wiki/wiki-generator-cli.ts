...
on:
  push:
    paths: ['lib/docs/constants/utils.ts']
jobs:
  update-wiki:
...
1. **Update Source**: Modify \`lib/docs/constants/utils.ts\`
  ...
  async function createWikiFilesWithR2(
    ...
    try {
      ...
      if (config.format === 'html' || config.format === 'all') {
        const htmlContent = generateHTMLWikiWithR2(
          wikiData,
          analytics,
          searchIndex,
          r2Data,
      ...
      if (config.format === 'json' || config.format === 'all') {
        const jsonContent = generateJSONWikiWithR2(
          wikiData,
          analytics,
          searchIndex,
          r2Data,
...
`);
  ...
  async function main(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `   📄 ${config.outputDir}/bun-utilities-wiki.json - API-ready with search index`
      );
      ...
      if (config.r2Integration) {
        ...
        console.log(
          `   📄 ${config.outputDir}/bun-utilities-wiki-r2.json - API-ready with R2 data`
        );
      ...
      console.log('\n💡 Next Steps:');
      console.log(
        `   1. Open ${config.outputDir}/bun-utilities-wiki.html to view interactive wiki`
      );
      ...
      console.error('\n🔧 Troubleshooting:');
      console.error('   • Check if lib/docs/constants/utils.ts exists');
