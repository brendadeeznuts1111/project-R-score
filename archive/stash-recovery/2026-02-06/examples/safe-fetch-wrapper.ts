...
export const safeFetchAPI = {
  ...
  // POST JSON
  async postJSON(url: string, data: unknown, options: Omit<SafeFetchOptions, 'method' | 'headers'> = {}): Promise<unknown> {
    // BETTER FIX - Completely isolate method
    const { method, ...cleanOptions } = options || {} as Omit<SafeFetchOptions, 'method' | 'headers'>;
  ...
  // POST Form
  async postForm(url: string, data: URLSearchParams | Record<string, string>, options: Omit<SafeFetchOptions, 'method' | 'headers'> = {}): Promise<unknown> {
    // BETTER FIX - Completely isolate method
    const { method, ...cleanOptions } = options || {} as Omit<SafeFetchOptions, 'method' | 'headers'>;
  ...
  // Upload File
  async uploadFile(url: string, file: File | Blob, fieldName: string = 'file', options: Omit<SafeFetchOptions, 'method' | 'headers'> = {}): Promise<unknown> {
    // BETTER FIX - Completely isolate method
    const { method, ...cleanOptions } = options || {} as Omit<SafeFetchOptions, 'method' | 'headers'>;
