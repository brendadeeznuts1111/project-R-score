...
//  Native macOS Status Bar Widget
//
//  Usage: swift BunStatusWidget.swift
//
...
class StatusBarController: NSObject, NSApplicationDelegate {
    ...
    override init() {
        ...
        if let button = statusItem.button {
            button.image = NSImage(systemSymbolName: "gearshape.fill", accessibilityDescription: "Bun TOML Secrets Editor")
    ...
    private func updateStatusItem(_ status: WidgetStatus) {
        ...
        Last Update: \(Date(timeIntervalSince1970: Double(status.lastUpdate) ?? 0).formatted(date: .abbreviated, time: .shortened))
        """
    ...
    private func createStatusIcon(color: NSColor) -> NSImage {
        let image = NSImage(systemSymbolName: "gearshape.fill", accessibilityDescription: "Status")!
...
class StatusPopoverViewController: NSViewController {
    ...
    private func updateUI(_ status: WidgetStatus) {
        ...
        lastUpdateLabel.stringValue = "🕐 Last Update: \(date.formatted(date: .abbreviated, time: .shortened))"
    }
}
