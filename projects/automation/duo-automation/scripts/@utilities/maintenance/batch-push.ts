...
async function batchPush(dir: string, phoneFile: string) {
    ...
    console.info(`📦 Starting batch push from ${dir} to phones in ${phoneFile}...`);
    
    try {
        const files = readdirSync(dir).filter(f => f.endsWith('.mp4'));
        const phones = (await Bun.file(phoneFile).text()).split('\n').filter(p => p.trim() !== '');

        console.info(`Found ${files.length} videos and ${phones.length} phones.`);
        ...
        for (const phone of phones) {
            for (const file of files) {
                ...
                console.info(`Pushing ${file} to ${phone}...`);
                await Bun.sleep(100); // Simulate network/io
                console.info(`✅ Pushed ${file} to ${phone} in ${(Bun.nanoseconds() - start) / 1e6}ms`);
            }
        }
    } catch (e: any) {
