/**
 * FactoryWager SDK Integration Module
 * 
 * Provides seamless integration with FactoryWager cloud services including:
...
export interface FactoryWagerConfig {
  apiKey: string;
  region: 'us' | 'eu' | 'asia';
  baseUrl: string;
  timeout: number;
...
/**
 * FactoryWager SDK Client
 */
export class FactoryWagerSDK {
  private config: FactoryWagerConfig;
  private apiKey: string;

  constructor(apiKey: string, region: FactoryWagerConfig['region'] = 'us') {
    this.apiKey = apiKey;
    this.config = {
      apiKey,
      region,
      baseUrl: `https://api.factory-wager.${region}.v1`,
      timeout: 30000
    };
  }
...
// Export instances
export const factory-wagerSDK = (apiKey: string) => new FactoryWagerSDK(apiKey);
