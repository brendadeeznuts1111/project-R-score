...
const PORT = parseInt(process.env.ENHANCED_DASHBOARD_PORT || '3010');
...
const dashboardServer = {
  ...
  // Built-in routes with static file buffering for speed
  ...
  async fetch(req: Request, server: any) {
    ...
    // API v1 routes - handle built-in endpoints
    if (url.pathname.startsWith('/api/v1')) {
      return Response.json({ error: "API v1 not implemented" }, { status: 501 });
    }
