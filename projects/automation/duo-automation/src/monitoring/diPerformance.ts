// src/monitoring/diPerformance.ts - Optimized for Bun v1.3.6
import { performance } from 'perf_hooks';

// Use Bun's optimized performance when available
const perf = typeof Bun !== 'undefined' ? Bun.performance : performance;
...
class DiPerformanceMonitor {
  ...
  record(functionName: string, startTime: number, error?: Error) {
    const metric: DiMetric = {
      ...
      resolutionMs: Math.round(perf.now() - startTime),
