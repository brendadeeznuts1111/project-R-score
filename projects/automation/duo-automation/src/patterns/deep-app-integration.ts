...
import { CashAppResolver } from './cashapp-resolver.js';
import type { CashAppProfile as ProductionCashAppProfile } from './cashapp-resolver.js';
import { FactoryWagerPhoneIntelligenceSystem } from '../integrations/factory-wager-phone-intelligence.js';
...
export class MultiAppOrchestrator {
  ...
  private duoPlus = FactoryWagerPhoneIntelligenceSystem.create({
    apiKey: (globalThis as any).Bun?.env?.FACTORY_WAGER_API_KEY || 'test-key',
    endpoint: 'https://api.factory-wager.com/v1',
    timeout: 5000,
    retries: 3
  });
  ...
  async getUnifiedProfile(phone: string, options: { dryRun?: boolean, mockLevel?: string, webhookUrl?: string } = {}): Promise<UnifiedProfile> {
    ...
    const duoPlusDevice: FactoryWagerDevice | null = duoPlusResult.duoPlusData ? {
  ...
  private performCrossValidation(cashApp: ProductionCashAppProfile | null, duoPlus: any): CrossValidationResult {
    ...
    // SYMMETRIC VALIDATION: We validate CashApp against FactoryWager AND FactoryWager against CashApp
    if (cashApp && duoPlus) {
      ...
      // FactoryWager returns carrier info directly. CashApp profile doesn't always have it, 
      ...
      if (duoPlusCarrier) {
        // If we have a verified carrier from FactoryWager, it's a foundation signal
...
export class EnhancedPhoneIntelligenceSystem {
  ...
  async assessDeepRisk(phone: string): Promise<{
    overallRisk: string;
    ...
    try {
      const result = await this.processEnhanced(phone);
      const risks: Array<{ factor: string; severity: string; recommendation: string }> = [];
      
      // Analyze various risk factors
      if (result.identityGraph.syntheticScore > 0.6) {
        risks.push({
          factor: 'SYNTHETIC_IDENTITY',
          severity: 'HIGH',
          recommendation: 'Block and investigate'
        });
      }
      
      if (result.multiApp.trustScore < 40) {
        risks.push({
      ...
      if (result.multiApp.crossValidation.consistency < 0.5) {
        risks.push({
          factor: 'CROSS_VALIDATION_FAILURE',
          severity: 'HIGH',
      ...
      return {
        overallRisk,
        actionRequired: overallRisk !== 'LOW',
        risks
      };
    } catch (error) {
      return {
        ...
        risks: [{
          ...
          severity: 'HIGH',
          recommendation: 'Retry assessment or use alternative method'
        }]
      };
    }
  }
