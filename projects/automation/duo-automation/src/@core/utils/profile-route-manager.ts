...
// Inline shortcut functionality (replacing missing keyboard-shortcuts-lite)
const shortcuts = {
...
export class ProfileRouteManager {
  ...
  switchProfile(profileId: string): void {
    ...
    // Security check: Require authentication for admin access
    if (profileId === 'admin') {
      const isAuthenticated = this.checkAdminAuthentication();
      if (!isAuthenticated) {
        console.error('❌ Admin access denied: Authentication required');
        return;
      }
    }
  ...
  private checkAdminAuthentication(): boolean {
    ...
    // For now, check for admin session or environment variable
    return process.env.ADMIN_SESSION === 'true' || 
           process.env.USER_ROLE === 'admin' ||
           Bun.env.ADMIN_AUTH === 'true';
  }
  ...
  private addActivity(message: string, type: 'success' | 'warning' | 'error' | 'info' = 'info'): void {
    ...
    feed.insertBefore(item, feed.firstChild || null);
