...
import { safeFilename, encodeContentDisposition, safeEvidenceFilename, safeArchiveFilename, safeReportFilename } from "../native/safeFilename.bun.ts";
...
export async function uploadUserReport(
  ...
  try {
    ...
    // Generate secure filename using Bun-native safe filename
    const filename = isPremium 
      ? safeFilename(`premium-export-${Date.now()}.json`)
      : safeReportFilename(scope, userId);
    
    await deps.s3Write(`exports/${scope}/${userId}/report.json`, buffer, {
      contentType: "application/json",
      contentDisposition: encodeContentDisposition(filename, "attachment"),
