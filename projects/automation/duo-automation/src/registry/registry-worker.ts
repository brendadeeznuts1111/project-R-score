...
// Middleware
app.use('*', cors({
  origin: ['https://factory-wager-registry.utahj4754.workers.dev', 'https://registry.factory-wager-registry.utahj4754.workers.dev', 'https://npm.factory-wager-registry.utahj4754.workers.dev'],
...
app.use('*', async (c, next) => {
  ...
  c.header('X-Registry-Domain', 'registry.factory-wager-registry.utahj4754.workers.dev');
  await next();
});
...
app.get('/', (c) => {
  return c.json({
    ...
    description: 'Premium npm-compatible registry for FactoryWager components',
    version: '1.0.0',
    organization: 'Factory Wager',
    website: 'https://factory-wager-registry.utahj4754.workers.dev',
    support: 'mailto:registry@factory-wager-registry.utahj4754.workers.dev',
    registry: 'https://registry.factory-wager-registry.utahj4754.workers.dev',
    endpoints: {
      health: '/health',
      packages: '/@factory-wager/:package',
      download: '/@factory-wager/:package/-/:filename',
      search: '/-/v1/search'
    },
    domains: {
      registry: 'https://registry.factory-wager-registry.utahj4754.workers.dev',
      npm: 'https://npm.factory-wager-registry.utahj4754.workers.dev',
      packages: 'https://packages.factory-wager-registry.utahj4754.workers.dev',
      factory-wager: 'https://factory-wager.factory-wager-registry.utahj4754.workers.dev'
    }
  });
});

// Get package metadata
app.get('/@factory-wager/:package', async (c) => {
  const packageName = c.req.param('package');
  
  try {
    // Get latest version metadata
    const latestKey = `@factory-wager/${packageName}/latest/package.json`;
    ...
    // Get all versions
    const versionsKey = `@factory-wager/${packageName}/versions.json`;
...
app.get('/@factory-wager/:package/:version', async (c) => {
  const packageName = c.req.param('package');
  const version = c.req.param('version');
  
  try {
    const key = `@factory-wager/${packageName}/${version}/package.json`;
    ...
    metadata.dist = {
      tarball: `https://registry.factory-wager.com/@factory-wager/${packageName}/-/${tarballName}`,
...
app.get('/@factory-wager/:package/-/:filename', async (c) => {
  const packageName = c.req.param('package');
  const filename = c.req.param('filename');
  
  try {
    const key = `@factory-wager/${packageName}/${filename}`;
...
// Publish package
app.put('/@factory-wager/:package', async (c) => {
  const packageName = c.req.param('package');
  ...
  try {
    ...
    await c.env.PACKAGES_BUCKET.put(
      `@factory-wager/${packageName}/${tarballName}`,
      uploadData.tarball
    );
    
    // Store package metadata
    await c.env.METADATA_BUCKET.put(
      `@factory-wager/${packageName}/${version}/package.json`,
      JSON.stringify(uploadData.metadata)
    );
    
    // Update latest symlink
    await c.env.METADATA_BUCKET.put(
      `@factory-wager/${packageName}/latest/package.json`,
      JSON.stringify(uploadData.metadata)
    );
    ...
    return c.json({
      ...
      package: packageName,
      version: version,
      url: `https://registry.factory-wager.com/@factory-wager/${packageName}/${version}`
    });
  } catch (error) {
...
async function getAllPackages(bucket: R2Bucket): Promise<Array<{
  ...
  try {
    ...
    const list = await bucket.list({
      prefix: '@factory-wager/',
      delimiter: '/'
    });
    
    for (const prefix of list.prefixes) {
      const packageName = prefix.replace('@factory-wager/', '').replace('/', '');
