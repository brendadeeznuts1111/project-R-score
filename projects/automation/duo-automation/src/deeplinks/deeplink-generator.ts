...
export class DeepLinkGenerator {
  ...
  generateDisputeDeepLink(dispute: Dispute, action?: string): string {
    const base = `factory-wager://dispute`;
  ...
  generateQRDisputeDeepLink(
    ...
    return `factory-wager://dispute/qr/${encodedData}`;
  }
  ...
  parseDeepLink(deepLink: string): ParsedDeepLink {
    ...
    // Handle factory-wager:// URLs by adding a dummy host for URL parsing
    let urlToParse = decoded;
    if (decoded.startsWith('factory-wager://')) {
      urlToParse = decoded.replace('factory-wager://', 'http://factory-wager/');
    }
  ...
  generateWebFallbackLink(deepLink: string): string {
    ...
    return `https://factory-wager.com/deeplink/redirect?target=${encodedDeepLink}&platform=web`;
  }
  ...
  generateSecureDisputeLink(dispute: Dispute, expiresInHours: number = 24): SecureDeepLink {
    ...
    const link = `factory-wager://dispute/secure/${dispute.id}?token=${token}&expires=${expiresAt}`;
    ...
    return {
      ...
      token,
      oneTimeUse: true,
      webFallback: `https://factory-wager.com/disputes/secure/${dispute.id}?token=${token}` 
    };
  }
  ...
  generateAndroidIntentURI(deepLink: string): string {
    ...
    const fallbackUrl = encodeURIComponent('https://factory-wager.com/fallback');
    return `intent://factory-wager/launch#Intent;scheme=factory-wager;package=com.factory-wager.app;S.browser_fallback_url=${fallbackUrl};end`;
  }
...
export const examples = {
  // Simple dispute view
  simple: 'factory-wager%3A%2F%2Fdispute%2FDSP-12345',
  
  // Dispute with action
  withAction: 'factory-wager%3A%2F%2Fdispute%2FDSP-12345%2Fadd-evidence%3Faction%3Dupload',
  
  // QR dispute with encoded data
  qrDispute: 'factory-wager%3A%2F%2Fdispute%2Fqr%%3D',
  
  // Secure one-time link
  secure: 'factory-wager%3A%2F%2Fdispute%2Fsecure%2FDSP-12345%3Ftoken%3DeyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9%26expires%3D1690560000000'
};
