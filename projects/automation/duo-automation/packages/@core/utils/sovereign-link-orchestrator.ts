...
export class SovereignLinkOrchestrator {
  ...
  static async registerModules() {
    console.log("💎 Sovereign Batch Linker: Registering modules...");
    try {
      ...
      for (const mod of modules) {
        ...
        console.log(`📦 Linking workspace: ${mod}...`);
      ...
      console.log("✅ All Sovereign modules registered locally.");
      return modules;
    } catch (e) {
  ...
  static async linkToTarget(targetDir: string, modules: string[]) {
    console.log(`🛰️ Linking modules into target: ${targetDir}...`);
    
    for (const mod of modules) {
      // Assuming @duoplus namespace for Sovereign v4.0
      const scopedName = `@duoplus/${mod}`;
      console.log(`🔗 Linking ${scopedName}...`);
