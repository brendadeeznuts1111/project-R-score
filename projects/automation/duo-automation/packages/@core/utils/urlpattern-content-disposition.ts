...
const INLINE_EXTENSIONS = new Set(['json', 'txt', 'html', 'pdf']);
...
export function getContentDispositionByPath(path: string): {
  ...
  if (!classified) {
    ...
    return {
      type: INLINE_EXTENSIONS.has(ext) ? 'inline' : 'attachment',
      filename
    };
  }
...
// Batch upload with pattern-aware disposition and concurrency control
export async function bulkUploadWithDisposition(
  ...
  const results: Array<PromiseSettledResult<any>> = new Array(items.length);
  ...
  const workers = Array(Math.min(concurrency, items.length)).fill(null).map(async () => {
    while (queue.length > 0) {
      ...
      completed++;
      if (options.reportProgress && completed % 10 === 0) {
...
if (import.meta.main) {
  ...
  const manager = new R2ContentManager(process.env.R2_BUCKET!);
