...
async function publishPackages() {
  console.info("🛠️ Starting implementation of CI/CD pipeline...");
  ...
  try {
    ...
    for (const pkg of packages) {
      ...
      console.info(`  -> Running 'bun pm pack' in ${pkgPath}`);
      ...
      if (tarballMatch) {
        ...
        console.info(`  ✅ Artifact created: ${destPath}`);
        ...
        if (untrustedCode === 0) {
            console.info(`  🛡️ Security check passed (No untrusted dependencies)`);
        }
      } else {
