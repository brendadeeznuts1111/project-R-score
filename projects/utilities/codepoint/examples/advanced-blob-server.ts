...
function createTransformedBlob(): { blob: Blob; etag: string } {
  const originalSvg = `<svg width="200" height="200" xmlns="http://www.w3.org/2000/svg">
  ...
  // Simulate transformation (resize and enhance)
  const transformedSvg = originalSvg
    .replace('width="100" height="100"', 'width="200" height="200"')
    .replace("BLOB", "ENHANCED")
    .replace('font-size="14"', 'font-size="24"');
 ...
 */
function handleConditionalRequest(request: Request, etag: string): Response | null {
