...
class BuildOrganizer {
      ...
      try {
        ...
        for (const file of files) {
          try {
            ...
            if (stats.isFile) {
              // For safety, we'll just count files instead of deleting
              cleanedFiles++;
  ...
  private async organizeMDXFiles(): Promise<void> {
    // 🚀 BUN 1.1.X OPTIMIZATION: Create archive directory using Bun's file operations
    const archiveDir = join(process.cwd(), 'docs-archive');
    try {
      await Bun.file(join(archiveDir, '.gitkeep')).exists();
    } catch (error) {
      // Directory doesn't exist, create it
      await Bun.write(join(archiveDir, '.gitkeep'), '');
    }
    ...
    for (const filePath of problematicFiles) {
      ...
      try {
        ...
        // 🚀 BUN 1.1.X OPTIMIZATION: Create directory using Bun's file operations
        try {
          await Bun.file(dirname(archivePath)).exists();
        } catch (error) {
          await Bun.write(join(dirname(archivePath), '.gitkeep'), '');
        }
