...
export class RegulatoryFiling extends ValueObject {
  ...
  getRequiredDocuments(): string[] {
    ...
    const requirements: Record<FilingType, string[]> = {
      [FilingType.FORM_1099]: ['transaction_records', 'payee_information', 'tax_id_verification'],
      ...
      [FilingType.FORM_941]: ['payroll_records', 'employee_information', 'tax_withholdings'],
      // Add more filing type requirements...
    };

    return requirements[this._filingType] || ['financial_records', 'supporting_documentation'];
  }
  ...
  private validateFiling(params: RegulatoryFilingParams): void {
    ...
    if (!Object.values(RegulatoryBody).includes(params.regulatoryBody)) {
      throw new DomainError('Valid regulatory body is required', 'INVALID_REGULATORY_BODY');
    }

    if (!params.period || !(params.period instanceof AccountingPeriod)) {
      throw new DomainError('Valid accounting period is required', 'INVALID_PERIOD');
    }
    ...
    if (params.dueDate <= new Date()) {
      throw new DomainError('Due date must be in the future', 'INVALID_DUE_DATE');
    }
    ...
    if (params.thresholdAmount !== undefined && params.thresholdAmount < 0) {
      throw new DomainError('Threshold amount cannot be negative', 'INVALID_THRESHOLD');
    }
  }
}
