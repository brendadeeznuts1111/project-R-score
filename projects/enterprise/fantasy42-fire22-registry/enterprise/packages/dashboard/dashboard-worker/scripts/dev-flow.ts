...
class DevFlow {
  ...
  private async searchInDirectory(
    ...
    try {
      ...
      const globPattern = options.type
        ? `**/*.{${this.getExtensionsForType(options.type)
            .map(ext => ext.slice(1))
            .join(',')}}`
        : `**/*.{ts,tsx,js,jsx,json,md,css,html,sql,toml,yaml,yml}`;
  ...
  private async findTests(options: FlowOptions): Promise<void> {
    ...
    if (options.pattern) {
      ...
      // 🚀 BUN 1.1.X OPTIMIZATION: Enhanced fs.glob for test file discovery
      const files = await Array.fromAsync(fs.glob(testGlob, {
  ...
  private async findConfig(): Promise<void> {
    ...
    for (const pattern of configPatterns) {
      // 🚀 BUN 1.1.X OPTIMIZATION: Enhanced fs.glob for configuration files
  ...
  private async showPatterns(): Promise<void> {
    ...
    const patternFiles = await Array.fromAsync(fs.glob('src/patterns/*.ts', {
      exclude: ['**/node_modules/**']
    }));
  ...
  private async quickHealth(): Promise<void> {
    ...
    const distFiles = await Array.fromAsync(fs.glob('dist/*', {
      exclude: ['**/node_modules/**']
    }));
    ...
    const dbFiles = await Array.fromAsync(fs.glob('*.db', {
      exclude: ['**/node_modules/**']
    }));
    if (dbFiles.length > 0) {
