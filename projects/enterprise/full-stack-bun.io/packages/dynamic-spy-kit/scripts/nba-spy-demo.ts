...
console.info('🏀 LIVE NBA SPYING - 25K MARKETS\n');
...
console.info('📡 Simulating live odds updates...\n');
...
for (let i = 0; i < 100; i++) {
	...
	if (i < 5) {
		console.info(`  ${i + 1}. ${newOdds} ← SPY #${i} FIRED!`);
	}
}
...
console.info(`📊 Spy throughput: ${throughput.toLocaleString()} markets/sec ⚡`);
console.info(`📈 Total updates: ${stats.updates24h.toLocaleString()}`);
