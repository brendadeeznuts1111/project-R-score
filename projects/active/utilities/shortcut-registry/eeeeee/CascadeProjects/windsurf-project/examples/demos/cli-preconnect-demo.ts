...
console.info("🚀 CLI Preconnect Demonstration");
...
async function simulateStartup() {
  console.info("🏁 Simulating application startup...");
  ...
  for (const service of SERVICES) {
    ...
    try {
      ...
      console.info(`   ✅ ${service.name}: ${time.toFixed(2)}ms`);
    } catch (error) {
      const time = performance.now() - start;
      console.info(`   ❌ ${service.name}: Failed (${time.toFixed(2)}ms)`);
    }
  }
  ...
  console.info(`   Average first request: ${avgTime.toFixed(2)}ms`);
...
async function demonstrateRuntimePreconnect() {
  ...
  if (!targetService) {
    console.info("❌ No services available for preconnect demo");
    return { preconnectTime: 0, requestTime: 0 };
  }
  
  console.info(`Preconnecting to ${targetService.name}...`);
  ...
  try {
    ...
    console.info(`✅ Preconnect completed in ${preconnectTime.toFixed(2)}ms`);
    ...
    console.info(`📡 Request after preconnect: ${requestTime.toFixed(2)}ms`);
    console.info(`🎯 Status: ${response.status}`);
    
    return { preconnectTime, requestTime };
  } catch (error) {
    console.info(`❌ Preconnect failed:`, error);
    return { preconnectTime: 0, requestTime: 0 };
  }
}
...
async function testConnectionReuse() {
  ...
  console.info(`Making ${requestCount} requests to test connection reuse...`);
  ...
  for (let i = 0; i < requestCount; i++) {
    ...
    try {
      ...
      if (i < 3) {
        console.info(`   Request ${i + 1}: ${time.toFixed(2)}ms`);
      }
    } catch (error) {
  ...
  if (times.length > 0) {
    ...
    console.info(`   🚀 Reuse improvement: ${improvement.toFixed(1)}%`);
...
async function demonstrateHighConcurrency() {
  ...
  console.info(`Launching ${concurrency} concurrent requests...`);
  ...
  console.info(`⚡ Throughput: ${requestsPerSecond.toFixed(0)} req/sec`);
...
function showCLIExamples() {
  ...
  console.info("# Basic preconnect:");
  console.info("bun --fetch-preconnect https://api.github.com ./app.ts");
  console.info("");
  console.info("# Multiple preconnects:");
  console.info("bun --fetch-preconnect https://api.github.com \\");
  console.info("   --fetch-preconnect https://cdn.jsdelivr.net \\");
  console.info("   --fetch-preconnect https://auth0.com ./app.ts");
  console.info("");
  console.info("# With high concurrency:");
  console.info("BUN_CONFIG_MAX_HTTP_REQUESTS=2048 \\");
  console.info("  bun --fetch-preconnect https://api.github.com ./high-load-app.ts");
  console.info("");
  console.info("# Debug connection pool:");
  console.info("BUN_DEBUG=fetch bun --fetch-preconnect https://api.github.com ./app.ts");
}

// Main demonstration
async function main() {
  try {
    console.info(`🔧 Max concurrent requests: ${process.env.BUN_CONFIG_MAX_HTTP_REQUESTS || '256'}`);
    console.info(`🌐 Target services: ${SERVICES.length}`);
    console.info("");
    ...
    console.info("=" .repeat(40));
    console.info(`🚀 Startup time: ${startupResults.totalTime.toFixed(2)}ms`);
    console.info(`🔗 Preconnect time: ${preconnectResults.preconnectTime.toFixed(2)}ms`);
    console.info(`🔄 Connection reuse: ${reuseResults.improvement.toFixed(1)}% improvement`);
    console.info(`⚡ High concurrency: ${concurrencyResults.requestsPerSecond.toFixed(0)} req/sec`);
    
    // Show CLI examples
    showCLIExamples();
    
    console.info("\n🎯 CLI Preconnect Demo Complete!");
    console.info("💡 Remember: Use --fetch-preconnect for production optimization");
