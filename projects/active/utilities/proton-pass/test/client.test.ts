...
describe('env', () => {
  it('throws when PAT is missing', () => {
    const previous = process.env.PROTON_PASS_PERSONAL_ACCESS_TOKEN;
    delete process.env.PROTON_PASS_PERSONAL_ACCESS_TOKEN;
    delete Bun.env.PROTON_PASS_PERSONAL_ACCESS_TOKEN;
    expect(() => getPat()).toThrow(MissingPersonalAccessTokenError);
    if (previous !== undefined) {
      process.env.PROTON_PASS_PERSONAL_ACCESS_TOKEN = previous;
    }
  });

  it('returns the configured CLI path', () => {
    const previous = process.env.PROTON_PASS_CLI_PATH;
    process.env.PROTON_PASS_CLI_PATH = '/custom/pass-cli';
    expect(getCliPath()).toBe('/custom/pass-cli');
    if (previous !== undefined) {
      process.env.PROTON_PASS_CLI_PATH = previous;
    } else {
      delete process.env.PROTON_PASS_CLI_PATH;
    }
  });
