...
class ProfileComparator {
	...
	generateReport(comparison) {
		const report = [
			"# 📊 Performance Profile Comparison Report",
			...
			...comparison.improvements.map(
				(m) =>
					`✅ **${m.functionName}**: ${m.percentage.toFixed(2)}% faster (${m.totalTime.toFixed(2)}ms → ${m.totalTime.toFixed(2)}ms)`,
			),
			"",
			"## ⚠️ Performance Regressions",
			...comparison.regressions.map(
				(m) =>
					`❌ **${m.functionName}**: ${m.percentage.toFixed(2)}% slower (${m.totalTime.toFixed(2)}ms → ${m.totalTime.toFixed(2)}ms)`,
			),
			"",
			"---",
...
`);
	...
	if (!baselinePath || !currentPath) {
		console.error("❌ Both --baseline and --current paths are required");
		process.exit(1);
	}
