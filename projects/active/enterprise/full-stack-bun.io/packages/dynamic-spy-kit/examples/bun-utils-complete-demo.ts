...
function demoVersionEnv() {
	...
	console.info("1. VERSION & ENVIRONMENT");
	console.info("=".repeat(60));
	...
	console.info(`   Bun.env.PATH: ${(Bun.env.PATH || "").slice(0, 50)}...`);

	// Check if main entry
	const isMain = import.meta.path === Bun.main;
	console.info(`\n🔍 Entry Point Check:`);
	console.info(`   import.meta.path === Bun.main: ${isMain}`);
}
...
async function demoSleep() {
	console.info("\n" + "=".repeat(60));
	console.info("2. SLEEP FUNCTIONS");
	console.info("=".repeat(60));
	...
	await Bun.sleep(100);
	console.info(`   Slept for ${(performance.now() - start).toFixed(2)}ms`);

	// Sync sleep
	console.info(`\n⏱️ Bun.sleepSync(50) - blocking pause...`);
	const syncStart = performance.now();
	Bun.sleepSync(50);
	console.info(`   Slept for ${(performance.now() - syncStart).toFixed(2)}ms`);

	// Sleep until date
	console.info(`\n⏱️ Bun.sleep(Date) - sleep until specific time...`);
	const futureDate = new Date(Date.now() + 50);
	await Bun.sleep(futureDate);
	console.info(`   Woke up at target time!`);
}
...
function demoWhich() {
	console.info("\n" + "=".repeat(60));
	console.info("3. WHICH - FIND EXECUTABLES");
	console.info("=".repeat(60));
	...
	for (const exe of executables) {
		...
		console.info(`   ${exe.padEnd(8)} → ${path || "not found"}`);
	}
	...
	console.info(`\n🔍 Custom PATH search:`);
	console.info(`   bun with custom PATH → ${customBun || "not found"}`);
}
...
function demoUUIDv7() {
	console.info("\n" + "=".repeat(60));
	console.info("4. UUID v7 - MONOTONIC & SORTABLE");
	console.info("=".repeat(60));
	...
	console.info(`   ${uuid2}`);
	console.info(`   ${uuid3}`);
	console.info(`   ✅ Monotonically increasing (sortable!)`);
	...
	console.info(`   buffer:    Uint8Array(${buffer.length}) [${buffer.slice(0, 4).join(", ")}...]`);

	// Custom timestamp
	const customTime = Bun.randomUUIDv7("hex", Date.now() - 86400000);
	console.info(`\n⏱️ Custom timestamp (yesterday):`);
	console.info(`   ${customTime}`);
}
...
async function demoPeek() {
	console.info("\n" + "=".repeat(60));
	console.info("5. PEEK - SYNCHRONOUS PROMISE INSPECTION");
	console.info("=".repeat(60));

	// Resolved promise
	const resolved = Promise.resolve("hello");
	console.info(`\n📋 Resolved Promise:`);
	console.info(`   peek(resolved) = "${peek(resolved)}"`);
	console.info(`   peek.status(resolved) = "${peek.status(resolved)}"`);

	// Pending promise
	const pending = new Promise(() => {});
	console.info(`\n📋 Pending Promise:`);
	console.info(`   peek(pending) === pending: ${peek(pending) === pending}`);
	console.info(`   peek.status(pending) = "${peek.status(pending)}"`);

	// Rejected promise
	const rejected = Promise.reject(new Error("test error"));
	rejected.catch(() => {}); // Handle to avoid unhandled rejection
	console.info(`\n📋 Rejected Promise:`);
	console.info(`   peek.status(rejected) = "${peek.status(rejected)}"`);

	// Non-promise value
	console.info(`\n📋 Non-Promise Value:`);
	console.info(`   peek(42) = ${peek(42)}`);
}
...
function demoDeepEquals() {
	...
	console.info(`\n📋 Object Comparison:`);
	console.info(`   obj1 = { a: 1, b: { c: 2 } }`);
	console.info(`   obj2 = { a: 1, b: { c: 2 } }`);
	console.info(`   obj3 = { a: 1, b: { c: 3 } }`);
	console.info(`   Bun.deepEquals(obj1, obj2) = ${Bun.deepEquals(obj1, obj2)}`);
	console.info(`   Bun.deepEquals(obj1, obj3) = ${Bun.deepEquals(obj1, obj3)}`);
	...
	console.info(`\n📋 Strict Mode:`);
	console.info(`   a = { entries: [1, 2] }`);
	console.info(`   b = { entries: [1, 2], extra: undefined }`);
	console.info(`   Bun.deepEquals(a, b) = ${Bun.deepEquals(a, b)}`);
	console.info(`   Bun.deepEquals(a, b, true) = ${Bun.deepEquals(a, b, true)}`);
}
...
function demoEscapeHTML() {
	console.info("\n" + "=".repeat(60));
	console.info("7. ESCAPE HTML - 480MB/s-20GB/s");
	console.info("=".repeat(60));
	...
	console.info(`\n📋 HTML Escaping:`);
	console.info(`   Input:  ${unsafe}`);
	console.info(`   Output: ${escaped}`);
	...
	console.info(`\n📊 Benchmark:`);
	console.info(`   100 iterations of 50KB input`);
...
function demoStringWidth() {
	console.info("\n" + "=".repeat(60));
	console.info("8. STRING WIDTH - 6,756x FASTER THAN NPM");
	console.info("=".repeat(60));

	// Basic examples from docs
	console.info(`\n📋 Basic Usage:`);
	console.info(`   Bun.stringWidth("hello") = ${Bun.stringWidth("hello")}`);
	console.info(`   Bun.stringWidth("\\u001b[31mhello\\u001b[0m") = ${Bun.stringWidth("\u001b[31mhello\u001b[0m")}`);
	console.info(`   Bun.stringWidth("\\u001b[31mhello\\u001b[0m", { countAnsiEscapeCodes: true }) = ${Bun.stringWidth("\u001b[31mhello\u001b[0m", { countAnsiEscapeCodes: true })}`);
	...
	console.info(`   ${"Description".padEnd(20)} ${"String".padEnd(25)} Width  WithANSI`);
	console.info(`   ${"-".repeat(20)} ${"-".repeat(25)} ${"-----"} ${"--------"}`);
	for (const { str, desc } of examples) {
		...
		console.info(`   ${desc.padEnd(20)} ${displayStr.padEnd(25)} ${width.toString().padStart(5)} ${withAnsi.toString().padStart(8)}`);
	}

	// Ambiguous width option
	console.info(`\n📋 Ambiguous Width (emoji handling):`);
	const ambiguous = "⚡";
	console.info(`   "⚡" ambiguousIsNarrow=true:  ${Bun.stringWidth(ambiguous, { ambiguousIsNarrow: true })}`);
	console.info(`   "⚡" ambiguousIsNarrow=false: ${Bun.stringWidth(ambiguous, { ambiguousIsNarrow: false })}`);
	...
	for (const item of items) {
		...
		console.info(`   ${item.name}${padding} : ${item.score}`);
	}
	...
	const time = performance.now() - start;
	console.info(`   ${iterations.toLocaleString()} iterations in ${time.toFixed(2)}ms`);
...
function demoFileURL() {
	console.info("\n" + "=".repeat(60));
	console.info("9. FILE URL CONVERSION");
	console.info("=".repeat(60));
...
function demoCompression() {
	console.info("\n" + "=".repeat(60));
	console.info("10. COMPRESSION - GZIP/DEFLATE/ZSTD");
	console.info("=".repeat(60));
	...
	console.info(`\n📦 GZIP:`);
	console.info(`   Compressed:   ${gzipped.length} bytes (${((1 - gzipped.length / data.length) * 100).toFixed(1)}% smaller)`);
	console.info(`   Decompressed: ${gunzipped.length} bytes ✅`);

	// DEFLATE
	const deflated = deflateSync(data);
	const inflated = inflateSync(deflated);
	console.info(`\n📦 DEFLATE:`);
	console.info(`   Compressed:   ${deflated.length} bytes (${((1 - deflated.length / data.length) * 100).toFixed(1)}% smaller)`);
	console.info(`   Decompressed: ${inflated.length} bytes ✅`);
	...
	console.info(`\n📦 ZSTD:`);
	console.info(`   Compressed:   ${zstdCompressed.length} bytes (${((1 - zstdCompressed.length / data.length) * 100).toFixed(1)}% smaller)`);
	console.info(`   Decompressed: ${zstdDecompressed.length} bytes ✅`);

	// Compression levels
	console.info(`\n📊 ZSTD Compression Levels:`);
	for (const level of [1, 6, 19]) {
		const compressed = Bun.zstdCompressSync(data, { level });
		console.info(`   Level ${level.toString().padStart(2)}: ${compressed.length} bytes`);
	}
}
...
function demoInspect() {
	console.info("\n" + "=".repeat(60));
	console.info("11. INSPECT - CONSOLE.LOG AS STRING");
	console.info("=".repeat(60));
	...
	console.info(`   Uint8Array:   ${Bun.inspect(uint8)}`);
	
	const uint16 = new Uint16Array([256, 512, 1024]);
	console.info(`   Uint16Array:  ${Bun.inspect(uint16)}`);
	
	const float32 = new Float32Array([1.5, 2.75, 3.14159]);
	console.info(`   Float32Array: ${Bun.inspect(float32)}`);
	
	const buffer = Buffer.from("hello");
	console.info(`   Buffer:       ${Bun.inspect(buffer)}`);
	...
	console.info(`\n📋 Bun.inspect() - Binary:`);
	console.info(`   ArrayBuffer: ${Bun.inspect(arrayBuffer)}`);
	console.info(`   DataView:    ${Bun.inspect(dataView)}`);
	...
	console.info(Bun.inspect.table(nbaStats));

	// Table with colors option
	console.info(`\n📋 Bun.inspect.table(data, { colors: true }):`);
	console.info(Bun.inspect.table(nbaStats, { colors: true }));
	...
	console.info(`\n📋 Market Odds Table (Basic):`);
	console.info(Bun.inspect.table(marketOdds));
	...
	console.info(`\n📊 Comprehensive Market Data Table:`);
	console.info(`   Formula: Index = 100 × (Prob_FIRST - Prob_LAST) / ((Prob_FIRST + Prob_LAST) / 2)`);
	console.info(Bun.inspect.table(displayTable));
	...
	console.info(Bun.inspect.table(summaryStats));
	...
	if (alerts.length > 0) {
		...
		console.info(Bun.inspect.table(alerts));
	}
	...
	console.info(`   ${Bun.inspect(market)}`);
	console.info(`\n   ${Bun.inspect(arb)}`);
}
...
function demoNanoseconds() {
	console.info("\n" + "=".repeat(60));
	console.info("12. NANOSECONDS - HIGH PRECISION TIMING");
	console.info("=".repeat(60));
	...
	console.info(`   Start: ${start.toLocaleString()} ns`);
	console.info(`   End:   ${end.toLocaleString()} ns`);
	console.info(`   Duration: ${durationNs.toLocaleString()} ns`);
	console.info(`            = ${durationUs.toFixed(2)} µs`);
	console.info(`            = ${durationMs.toFixed(4)} ms`);
}
...
async function demoReadableStream() {
	console.info("\n" + "=".repeat(60));
	console.info("13. READABLE STREAM UTILITIES");
	console.info("=".repeat(60));
	...
	console.info(`   readableStreamToText: ${text.slice(0, 50)}...`);

	// To JSON
	const json = await Bun.readableStreamToJSON(blob.stream());
	console.info(`   readableStreamToJSON: markets=${json.markets}`);

	// To ArrayBuffer
	const arrayBuffer = await Bun.readableStreamToArrayBuffer(blob.stream());
	console.info(`   readableStreamToArrayBuffer: ${arrayBuffer.byteLength} bytes`);

	// To Bytes (Uint8Array)
	const bytes = await Bun.readableStreamToBytes(blob.stream());
	console.info(`   readableStreamToBytes: Uint8Array(${bytes.length})`);

	// To Blob
	const newBlob = await Bun.readableStreamToBlob(blob.stream());
	console.info(`   readableStreamToBlob: ${newBlob.size} bytes`);

	// To Array (chunks)
	const arr = await Bun.readableStreamToArray(blob.stream());
	console.info(`   readableStreamToArray: ${arr.length} chunks`);
}
...
function demoResolveSync() {
	console.info("\n" + "=".repeat(60));
	console.info("14. RESOLVE SYNC - MODULE RESOLUTION");
	console.info("=".repeat(60));
	...
	try {
		...
		console.info(`   ./bun-utils-complete-demo.ts → ${selfPath.slice(-50)}`);
	} catch {
		console.info(`   ./bun-utils-complete-demo.ts → (not found)`);
	}

	// Resolve node module
	try {
		const bunPath = Bun.resolveSync("bun", process.cwd());
		console.info(`   bun → ${bunPath}`);
	} catch {
		console.info(`   bun → (built-in)`);
	}
}
...
function demoStripANSI() {
	console.info("\n" + "=".repeat(60));
	console.info("15. STRIP ANSI - 6-57x FASTER THAN NPM");
	console.info("=".repeat(60));
	...
	console.info(`\n📋 ANSI Stripping:`);
	console.info(`   Input:  "${colored}"`);
	console.info(`   Output: "${Bun.stripANSI(colored)}"`);
	console.info(`\n   Input:  "${formatted}"`);
	console.info(`   Output: "${Bun.stripANSI(formatted)}"`);
...
function demoBunJSC() {
	console.info("\n" + "=".repeat(60));
	console.info("16. BUN:JSC - SERIALIZE/DESERIALIZE");
	console.info("=".repeat(60));
	...
	console.info(`\n📋 Serialize:`);
	console.info(`   Object: ${JSON.stringify(obj).slice(0, 50)}...`);
	console.info(`   Serialized: ArrayBuffer(${serialized.byteLength})`);

	// Deserialize
	const deserialized = deserialize(serialized);
	console.info(`\n📋 Deserialize:`);
	console.info(`   Restored: ${JSON.stringify(deserialized).slice(0, 50)}...`);
	console.info(`   Equal: ${Bun.deepEquals(obj, deserialized)}`);

	// Memory estimation
	console.info(`\n📋 Memory Estimation:`);
	console.info(`   Object: ${estimateShallowMemoryUsageOf(obj)} bytes`);
	console.info(`   Buffer(1MB): ${estimateShallowMemoryUsageOf(Buffer.alloc(1024 * 1024))} bytes`);
	console.info(`   Array(100): ${estimateShallowMemoryUsageOf(new Array(100))} bytes`);
}
...
function demoOpenInEditor() {
	console.info("\n" + "=".repeat(60));
	console.info("17. OPEN IN EDITOR");
	console.info("=".repeat(60));

	console.info(`\n📋 Bun.openInEditor() API:`);
	console.info(`   // Opens current file in default editor`);
	console.info(`   Bun.openInEditor(import.meta.url);`);
	console.info(`\n   // With options`);
	console.info(`   Bun.openInEditor(file, {`);
	console.info(`     editor: "vscode",  // or "subl"`);
	console.info(`     line: 10,`);
	console.info(`     column: 5`);
	console.info(`   });`);
	console.info(`\n   // Configure in bunfig.toml:`);
	console.info(`   [debug]`);
	console.info(`   editor = "code"`);
}
...
async function main() {
	...
	console.info("\n" + "=".repeat(60));
	console.info("✅ BUN UTILS CHEAT SHEET");
	console.info("=".repeat(60));
	console.info(`
