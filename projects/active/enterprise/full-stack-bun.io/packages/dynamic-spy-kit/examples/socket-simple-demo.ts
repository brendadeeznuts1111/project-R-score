...
async function main() {
	console.info("🔌 Simple Socket Information Demo\n");
	...
	try {
		...
		console.info(`${s.localAddress}:${s.localPort} → ${s.remoteAddress}:${s.remotePort} (${s.remoteFamily})`);
		// Example output: 10.0.0.24:59876 → 104.21.8.212:443 (IPv4)
		
		// Access individual properties
		console.info("\nIndividual Properties:");
		console.info(`  Local:  ${s.localAddress}:${s.localPort} (${s.localFamily})`);
		console.info(`  Remote: ${s.remoteAddress}:${s.remotePort} (${s.remoteFamily})`);
		
		s.end();
	} catch (error) {
		// Fallback to HTTP connection if TLS fails
		console.info("TLS connection failed, trying HTTP...\n");
