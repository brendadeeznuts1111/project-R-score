// @see https://bun.com/reference/bun/mmap
/**
...
export class MMapCache {
  ...
  async loadMarkets(): Promise<string> {
    try {
      ...
      // Bun.mmap maps a non-empty file path. It does not accept byte offsets.
      const markets = Bun.mmap('cache/12k-markets.bin');
  ...
  private validateMmapOptions(
    ...
    if ('offset' in options && typeof options.offset !== 'number') {
   ...
   */
  safeMmap(buffer: ArrayBuffer, options: { offset?: number; size?: number } = {}): Uint8Array {
    const validated = this.validateMmapOptions(buffer, options);
    return new Uint8Array(buffer, validated.offset, validated.size);
  }
}
