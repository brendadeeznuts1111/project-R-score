...
console.info('🚀 ${config.packageName} initialized with Bun isolated installs');
`;
    }
...
switch (command) {
  ...
  default:
    console.info('Usage: bun orchestration.ts [build:linked|build:standalone|install:isolated|test:all]');
    console.info('');
...
if (import.meta.main) {
  ...
  try {
    switch (command) {
      ...
      default:
        console.info('Usage: bun enhanced-workspace-isolator.ts [isolate|create]');
        console.info(
