...
console.info("Bun 1.3.6 Performance Benchmarks");
...
console.info("📦 Response.json (SIMD FastStringifier)");
console.info(`   Response.json():        ${r1.elapsed.toFixed(2)}ms (${(r1.opsPerMs * 1000).toFixed(0)} ops/s)`);
console.info(`   stringify + Response(): ${r2.elapsed.toFixed(2)}ms (${(r2.opsPerMs * 1000).toFixed(0)} ops/s)`);
console.info(`   Ratio: ${(r1.elapsed / r2.elapsed).toFixed(2)}x`);
console.info();
...
console.info("🔍 Buffer.indexOf/includes (SIMD search - up to 2x faster)");
console.info(`   indexOf (found):      ${b1.elapsed.toFixed(2)}ms (${(b1.opsPerMs * 1000).toFixed(0)} ops/s)`);
console.info(`   indexOf (not found):  ${b2.elapsed.toFixed(2)}ms (${(b2.opsPerMs * 1000).toFixed(0)} ops/s)`);
console.info(`   includes (found):     ${b3.elapsed.toFixed(2)}ms (${(b3.opsPerMs * 1000).toFixed(0)} ops/s)`);
console.info(`   includes (not found): ${b4.elapsed.toFixed(2)}ms (${(b4.opsPerMs * 1000).toFixed(0)} ops/s)`);
...
console.info("🚀 Bun.spawnSync (fd limit optimization - Linux ARM64 fix)");
console.info(`   1000 spawns: ${s1.elapsed.toFixed(2)}ms`);
console.info(`   Per spawn:   ${(s1.elapsed / 1000).toFixed(3)}ms`);
console.info(`   💡 Fixed 30x slowdown on Linux systems with high FD limits`);
console.info();
...
console.info("🔢 Bun.hash.crc32 (hardware accelerated)");
console.info(`   1MB × 1000: ${c1.elapsed.toFixed(2)}ms`);
console.info(`   Throughput: ~${((1000 * 1000) / c1.elapsed / 1000).toFixed(0)} GB/s`);
console.info();
