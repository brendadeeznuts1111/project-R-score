...
console.info(`swap16(): ${swap16Time.toFixed(2)}µs per operation`);
...
console.info(`swap64(): ${swap64Time.toFixed(2)}µs per operation`);

// Calculate improvements
console.info('\n--- Performance Analysis ---');
console.info(`Expected (Bun v1.3.7): swap16 ≈ 0.56µs, swap64 ≈ 0.56µs`);
console.info(`Your results:          swap16 ≈ ${swap16Time.toFixed(2)}µs, swap64 ≈ ${swap64Time.toFixed(2)}µs`);

if (swap64Time < 1.0) {
  console.info('✅ swap64 is performing at optimized speeds (< 1µs)');
}

if (Math.abs(swap16Time - swap64Time) < 0.1) {
  console.info('✅ swap16 and swap64 have similar performance (as expected in v1.3.7+)');
}
...
try {
  ...
  console.info('✅ Correctly throws ERR_INVALID_BUFFER_SIZE for invalid buffer size');
}
