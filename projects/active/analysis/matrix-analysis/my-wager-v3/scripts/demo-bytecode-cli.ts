...
console.info('🔥 Bytecode Profiling CLI Demo');
console.info('=============================\n');

console.info('1. Show help:');
console.info('$ bun run cli/test.ts --help\n');
...
console.info('$ bun run cli/test.ts --profile-config --config=local\n');

// Profile config loading
await import('../cli/test.ts').then(m => m.testCommand(['--profile-config', '--config=local']));

console.info('\n3. Run with bytecode profiling:');
console.info('$ bun run cli/test.ts --profile --config=local\n');
...
console.info('\n4. Compare profiles:');
console.info('$ bun run cli/test.ts --compare-profiles --config=local\n');
