...
describe('Responsible Gambling Engine', () => {
  ...
  let originalGetUserLocation: (typeof ResponsibleGamblingEngine)['getUserLocation'];

  beforeEach(() => {
    // Reset any global state
    mock.restore();
    originalFetch = globalThis.fetch;
    originalGetUserLocation = ResponsibleGamblingEngine['getUserLocation'];
  ...
  afterEach(() => {
    ...
    ResponsibleGamblingEngine['getUserLocation'] = originalGetUserLocation;
    // Clean up env vars
  ...
  test('handles geographic compliance', async () => {
    ...
    ResponsibleGamblingEngine['getUserLocation'] = mockGetUserLocation;
  ...
  test('detects high-risk behavioral patterns', async () => {
    ...
    ResponsibleGamblingEngine['getUserActivity'] = mock(() =>
      Promise.resolve([
        ...
        { timestamp: Date.now() - 4000, amount: 400, type: 'loss' },
        { timestamp: Date.now() - 5000, amount: 200, type: 'loss' },
      ])
    );

    ResponsibleGamblingEngine['getBehavioralModel'] = mock(() =>
      Promise.resolve({
        predict: () => 0.95,
      })
    );
    ...
    expect(result.allowed).toBe(false);
    expect(
      result.restrictions?.find(r => r.reason === 'HIGH_RISK_BEHAVIOR_DETECTED')
    ).toBeDefined();
...
describe('Wagering Engine', () => {
  ...
  test('rejects wager with compliance violation', async () => {
    ...
    ResponsibleGamblingEngine.checkUser = mock(() =>
      Promise.resolve({
        allowed: false,
...
describe('Wagering Router', () => {
  ...
  test('handles OPTIONS requests for CORS', async () => {
    ...
    expect(response.headers.get('Access-Control-Allow-Origin')).toBe(
      'https://licensed-operator.example.com'
    );
  });
});
...
describe('Integration Tests', () => {
  test('full wager flow with all checks', async () => {
    ...
    // Simulate full flow
    const complianceResult = await ResponsibleGamblingEngine.checkUser(
