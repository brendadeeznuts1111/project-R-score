...
export class WebSocketCookieFix {
  ...
  private static auditWebSocketCookie(name: string, _value: string): void {
...
// Zero-cost export
export const { upgradeWithCookies, onWebSocketUpgrade } = feature(
  "WS_COOKIE_FIX"
)
  ? WebSocketCookieFix
  : {
      upgradeWithCookies: (r: Request, s: WebSocketServer): boolean =>
        s.upgrade(r),
      onWebSocketUpgrade: (): Map<string, string> => new Map(),
    };
