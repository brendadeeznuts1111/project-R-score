...
pub enum MarketType {
    ...
    Total,
    Btts,
    // Extended types for latency arbitrage framework
    TeamTotal,
    QuarterTotal,
    HalfTotal,
    PlayerProp,
...
impl std::fmt::Display for MarketType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            MarketType::TeamTotal => write!(f, "team_total"),
            MarketType::QuarterTotal => write!(f, "quarter_total"),
            MarketType::HalfTotal => write!(f, "half_total"),
            MarketType::PlayerProp => write!(f, "player_prop"),
            MarketType::AltLine => write!(f, "alt_line"),
            MarketType::Combo => write!(f, "combo"),
        }
    }
}
...
impl TimestampedOrderbook {
    ...
    /// Load current state
    #[inline(always)]
    pub fn load(&self) -> (PriceCents, PriceCents, SizeCents, SizeCents, TimestampNs) {
        let packed = self.packed.load(Ordering::Acquire);
        let timestamp = self.timestamp_ns.load(Ordering::Acquire);
        let (yes_ask, no_ask, yes_size, no_size) = unpack_orderbook(packed);
        (yes_ask, no_ask, yes_size, no_size, timestamp)
    }

    /// Store new state with timestamp
    #[inline(always)]
    pub fn store(&self, yes_ask: PriceCents, no_ask: PriceCents, yes_size: SizeCents, no_size: SizeCents, timestamp_ns: TimestampNs) {
    ...
    /// Update YES side only with timestamp
    #[inline(always)]
    pub fn update_yes(&self, yes_ask: PriceCents, yes_size: SizeCents, timestamp_ns: TimestampNs) {
    ...
    /// Update NO side only with timestamp
    #[inline(always)]
    pub fn update_no(&self, no_ask: PriceCents, no_size: SizeCents, timestamp_ns: TimestampNs) {
        let mut current = self.packed.load(Ordering::Acquire);
        loop {
...
impl Default for TimestampedOrderbook {
    fn default() -> Self {
        Self::new()
    }
}
...
pub enum Platform {
    Kalshi,
    Polymarket,
    // Extended providers for latency arbitrage
    DraftKings,
    FanDuel,
    BetMGM,
    Caesars,
...
impl std::fmt::Display for Platform {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Platform::DraftKings => write!(f, "DRAFTKINGS"),
            Platform::FanDuel => write!(f, "FANDUEL"),
            Platform::BetMGM => write!(f, "BETMGM"),
            Platform::Caesars => write!(f, "CAESARS"),
            Platform::PointsBet => write!(f, "POINTSBET"),
            Platform::Barstool => write!(f, "BARSTOOL"),
            Platform::ESPN => write!(f, "ESPN"),
        }
    }
}
