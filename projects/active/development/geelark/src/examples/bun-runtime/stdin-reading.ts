...
console.info("📥 Reading from Stdin Examples\n");

// Example 1: Using console as AsyncIterable (line-by-line)
console.info("1. Line-by-line reading with console AsyncIterable:");

// Check if we're running interactively (not with piped input)
if (process.stdin.isTTY) {
  console.info("Running interactive prompt from documentation:");
  ...
  console.info("Interactive mode not available when piped input is present");
  console.info("For interactive testing: bun run examples/stdin-reading.ts (no piping)\n");
}

// Example 2: Using Bun.stdin.stream() for chunked reading
console.info("2. Chunked reading with Bun.stdin.stream():");

console.info("Attempting to read from stdin stream...");
...
try {
  ...
  if (hasReadData) {
    console.info(`Successfully read ${chunkCount} total chunks`);
  } else {
    console.info("No data available on stdin - use: echo 'test' | bun run examples/stdin-reading.ts");
  }
} catch (error) {
...
async function demonstrateStdin() {
  console.info("3. Demonstration of stdin reading functionality:");

  // Show that Bun.stdin exists and has expected method
  console.info(`Bun.stdin available: ${!!Bun.stdin}`);
  console.info(`Bun.stdin.stream available: ${typeof Bun.stdin.stream === 'function'}`);

  // Show console is AsyncIterable
  console.info(`Console is async iterable: ${!!console[Symbol.asyncIterator]}`);

  console.info("\nTo test both methods:");
  console.info("- Line by line: Run and type interactively with Enter");
  console.info("- Chunked: Use 'echo \"test\" | bun run examples/stdin-reading.ts'");
}
