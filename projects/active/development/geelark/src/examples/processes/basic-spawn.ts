...
console.info("🚀 Basic Child Process Spawning Examples\n");

// Example 1: Simple command execution
console.info("1. Simple echo command:");
const proc1 = Bun.spawn(["echo", "Hello from a child process!"]);

// Wait for completion
await proc1.exited;
console.info(`Process exited with code: ${proc1.exitCode}\n`);

// Example 2: Command with multiple arguments
console.info("2. Multiple arguments:");
const proc2 = Bun.spawn(["echo", "First arg", "Second arg", "Third arg"]);
await proc2.exited;
console.info(`Exit code: ${proc2.exitCode}\n`);

// Example 3: Different executable (if available)
console.info("3. Using a system command:");
try {
  const proc3 = Bun.spawn(["pwd"]); // Print working directory
  await proc3.exited;
  console.info(`Exit code: ${proc3.exitCode}\n`);
} catch (error) {
  console.info(`Error running pwd: ${error.message}\n`);
}

// Example 4: Long-running process (with timeout)
console.info("4. Long-running process (with sleep simulation):");
...
try {
  ...
  const timeout = setTimeout(() => {
    proc4.kill();
    console.info("Process was killed due to timeout");
  }, 3000);

  await proc4.exited;
  clearTimeout(timeout);
  console.info(`Sleep completed in ${(Date.now() - start)}ms, exit code: ${proc4.exitCode}\n`);
} catch (error) {
  console.info(`Error: ${error.message}\n`);
}

console.info("✅ Basic spawning examples completed!");
