...
export class BunEnvSyncTool {
  ...
  public static async syncSecrets() {
    console.info("🔐  BunEnvSyncTool: Starting token synchronization...");
    ...
    if (!(await Bun.file(this.ENV_PATH).exists())) {
      ...
      console.info("✅  New rotating token generated.");
    }

    try {
      console.info("📤  Pushing NPM_TOKEN to Cloudflare Wrangler...");
      ...
      if (token) {
        ...
        console.info(`📡  Syncing: cd ${this.WORKER_DIR} && echo "${token}" | wrangler secret put NPM_TOKEN`);
        console.info("✅  Secret store synchronized with Edge.");
      }
    } catch (err) {
