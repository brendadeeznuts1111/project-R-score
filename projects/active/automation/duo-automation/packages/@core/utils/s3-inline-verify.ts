...
export async function verifyInlinePreview(embedUrl: string) {
  try {
    ...
    if (!resp.ok) {
       console.info(`❌ Fetch failed: ${resp.status} ${resp.statusText}`);
    ...
    console.info('✅ Inline Preview Headers:');
    console.info(`  Disposition: ${disposition || 'inline (default)'}`);
    ...
    try {
      ...
      console.info('📋 Parsed Metadata:', Bun.inspect.table(Object.entries(metadata).map(([k,v]) => ({ Key: k, Value: v })), { colors: true }));
      return { disposition, metadata, success: !disposition || disposition.includes('inline') };
    } catch (e) {
      console.info('📄 Content is not valid JSON, raw text excerpt:', metaText.slice(0, 100).replace(/\n/g, ' '));
...
if (import.meta.main) {
  ...
  if (!demoUrl) {
    ...
    demoUrl = bucketUrl ? `${bucketUrl}/test-verify.json` : 'https://pub-dc0e1ef5dd2245be81d6670a9b7b1550.r2.dev/test.json';
    console.info(`⚠️ No URL provided, using fallback: ${demoUrl}`);
  }
