#!/usr/bin/env bun
// cli/commands/matrix-enhanced.ts
...
class EnhancedMatrixCLI {
  ...
  private showColorHelp(): void {
    ...
    const examples = [
      { Command: 'bun run matrix-enhanced.ts --theme=empire', Description: 'Empire Pro v3.7 theme' },
      { Command: 'bun run matrix-enhanced.ts --scheme=status', Description: 'Status-based colors' },
      { Command: 'bun run matrix-enhanced.ts --empire --badges', Description: 'Empire Pro with badges' },
      { Command: 'bun run matrix-enhanced.ts --format=json --color', Description: 'JSON with colors' },
      { Command: 'bun run matrix-enhanced.ts --no-color', Description: 'Monochrome output' },
      { Command: 'bun run matrix-enhanced.ts --theme=duoplus --scheme=health', Description: 'DuoPlus health theme' }
    ];

    console.info(UnicodeTableFormatter.generateTable(examples, { maxWidth: 100 }));

    console.info(EmpireProDashboard.generateSection('MATRIX FLAGS', '📊'));
    
    const matrixFlags = [
      { flag: '--filter=<column:value>', description: 'Filter matrix by column value' },
      { flag: '--sort=<column>', description: 'Sort by column (asc/desc)' },
      { flag: '--limit=<number>', description: 'Limit number of rows' },
      { flag: '--export=<file>', description: 'Export to file (csv/json/html)' },
      { flag: '--empire', description: 'Enable Empire Pro features' },
      { flag: '--badges', description: 'Show status badges' },
      { flag: '--status', description: 'Focus on status information' },
      { flag: '--performance', description: 'Focus on performance metrics' }
    ];

    const matrixTable = matrixFlags.map(flag => ({
      Flag: flag.flag,
      Description: flag.description
    }));

    console.info(UnicodeTableFormatter.generateTable(matrixTable, { maxWidth: 100 }));
  ...
  private colorizeValue(key: string, value: string): string {
    ...
    if (this.options.theme === 'duoplus') {
      ...
      if (lowerKey.includes('performance') || lowerKey.includes('speed')) {
  ...
  private extractStatus(row: any): string {
    // Extract status from various possible columns
  ...
  private exportData(data: any[], format: string): void {
    if (!this.flags['--export']) return;

    const filename = this.flags['--export'];
    let content: string;
    ...
    switch (format) {
      ...
      case 'csv':
        ...
        break;
      case 'html':
        content = this.generateHTMLTable(data);
        break;
      default:
        content = JSON.stringify(data, null, 2);
    }
    ...
    console.info(UnicodeTableFormatter.colorize(`✅ Exported to ${filename}`, DesignSystem.status.operational));
  }
  ...
  private generateHTMLTable(data: any[]): string {
    ...
    const css = `
      <style>
        body { font-family: Arial, sans-serif; background: ${DesignSystem.background.primary}; color: ${DesignSystem.text.primary}; }
        table { border-collapse: collapse; width: 100%; }
        th, td { border: 1px solid ${DesignSystem.background.tertiary}; padding: 12px; text-align: left; }
        th { background: ${DesignSystem.background.secondary}; color: ${DesignSystem.text.accent.blue}; }
        .operational { color: ${DesignSystem.status.operational}; }
        .degraded { color: ${DesignSystem.status.degraded}; }
        .downtime { color: ${DesignSystem.status.downtime}; }
        .maintenance { color: ${DesignSystem.status.maintenance}; }
      </style>
    `;

    const headerRow = headers.map(h => `<th>${h}</th>`).join('');
    const dataRows = data.map(row => 
      headers.map(h => `<td class="${this.getCSSClass(row[h])}">${row[h]}</td>`).join('')
    ).map(row => `<tr>${row}</tr>`).join('');

    return `<!DOCTYPE html>
<html>
<head>
  <title>Empire Pro v3.7 Matrix Export</title>
  ${css}
</head>
<body>
  <h1>Empire Pro v3.7 Matrix Export</h1>
  <table>
  ...
  private getCSSClass(value: string): string {
    ...
    if (lower.includes('operational') || lower.includes('healthy')) return 'operational';
    if (lower.includes('degraded') || lower.includes('warning')) return 'degraded';
    if (lower.includes('downtime') || lower.includes('critical')) return 'downtime';
    if (lower.includes('maintenance') || lower.includes('updating')) return 'maintenance';
  ...
  async run(): Promise<void> {
    ...
    try {
      // Use native test data instead of external markdown parsing
      ...
      console.info(UnicodeTableFormatter.colorize(`📂 Generated ${data.length} test entries with native UnicodeTableFormatter`, DesignSystem.text.accent.blue));

      // Apply color scheme
      const coloredData = this.applyColorScheme(data);

      // Display matrix with colors using our native formatter
      ...
      this.exportData(data, this.options.format);
...
// Main execution
async function main() {
  const cli = new EnhancedMatrixCLI(process.argv.slice(2));
  await cli.run();
}
