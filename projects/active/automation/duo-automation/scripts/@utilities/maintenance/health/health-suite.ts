...
class HealthSuite {
  async runCheck() {
    console.info("🏥 Bun Secrets Health Check");
    ...
    for (const secretName of criticalSecrets) {
      ...
      console.info(value ? `✅ ${secretName}: Available` : `❌ ${secretName}: MISSING`);
    }
  }

  async runMonitor() {
    console.info('\u001b[1m\u001b[35m🔍 AUTOMATED HEALTH MONITOR\u001b[0m');
    const mgr = new BunR2AppleManager();
    const storageStats = await mgr.getMetrics();
    console.info(`📦 STORAGE: ${storageStats.metrics.localMirroredFiles} local files`);
    
    const phoneSystem = await phoneIntelligenceSystem.getMetrics();
    console.info(`⚡ PERFORMANCE: ${phoneSystem.throughput.toFixed(0)} items/s`);
  }
  ...
  async runTrace() {
    ...
    console.info("\x1b[32m[EMPIRE PRO] Live Health Tracer Active\x1b[0m");
    const subsystems = ['STORAGE', 'PROXY', 'AUTH', 'ORCHESTRATION'];
    setInterval(() => {
      const sub = subsystems[Math.floor(Math.random() * subsystems.length)];
      console.info(`${new Date().toLocaleTimeString()} | ${sub.padEnd(15)} | HEALTHY`);
    }, 1200);
  }
}
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      console.info("Usage: bun run scripts/maintenance/health/health-suite.ts [check|monitor|trace]");
  }
}
