...
export class PatternValidator {
  ...
  static generateLSPInfo<T extends BasePattern>(pattern: T): LSPPatternInfo {
    return {
      id: (pattern as unknown as { id?: string }).id || '§Unknown:0',
      perf: (pattern as unknown as { perfBudget?: string }).perfBudget || '<unknown>',
      roi: (pattern as unknown as { roi?: string }).roi || '0x',
      semantics: (pattern as unknown as { semantics?: string[] }).semantics || [],
      description: `${(pattern as unknown as { category?: string }).category || 'Pattern'} pattern for ${pattern.constructor?.name || 'unknown'}`,
