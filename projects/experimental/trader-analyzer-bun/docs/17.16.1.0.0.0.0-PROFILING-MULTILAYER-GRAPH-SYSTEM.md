...
├── ProfilingMultiLayerGraphSystem (underlying system)
│   ├── PerformanceMonitor (CPU profiling)
│   ├── MultiLayerCorrelationGraph (graph structure)
│   └── Database (SQLite for persistence)
├── Markets (Market17[]) - Market data storage
└── Profiles (Map<string, Profile17>) - Session profiles
```
