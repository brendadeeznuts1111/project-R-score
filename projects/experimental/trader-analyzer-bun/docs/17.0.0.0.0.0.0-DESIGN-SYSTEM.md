...
## 🔄 Lived Workflow: The Closed-Loop Pipeline

### Scenario: Latency Spike Detection (HBMO-017)

#### Step 1: Trigger → Operational Telemetry
...
#### Step 2: Immediate Reflex → Context Preservation

**Action**: Engineer hits `Ctrl-Space ,` (or `Ctrl-Space n`) → switches to pre-focused "Analytics" pane
...
**Key Point**: **Zero context switch** — engineer never leaves Tmux session

---

#### Step 3: Instant Visual Correlation → Deterministic Radiance
...
**Result**: Engineer sees **exact same semantic meaning** across terminal → dashboard → visualization
...
**Core Principle**: One semantic token → One meaning → All media
...
#[fg=colour196]ERR:#{error_count}     → --color-risk-critical
#[fg=colour220]WARN:#{warn_count}     → --color-risk-warning
#[fg=colour51]INFO:#{info_count}      → --color-risk-info
```

**Cross-Medium Consistency**:
- Terminal log: `[WARN]` → Yellow (`colour220`)
...
**Terminal Integration**: Tmux uses dark theme (`colour234` background) → Dashboard automatically uses dark theme → **Zero visual jarring**
...
**Terminal Integration**: Terminal uses monospace → Dashboard uses `JetBrains Mono` → **Consistent typography**
...
**Color Alignment**:
- `background_color: #1a1a1a` → Matches `--bg-primary`
- `theme_color: #667eea` → Matches `--accent-purple`
...
- Terminal command → Registry browser → API endpoint

**See**: `docs/17.0.0.0.0.0.0-REGISTRY-BROWSER-INTEGRATION.md` for complete documentation

---

## 🔗 Integration with Terminal Environment (11.4.x)
...
**Result**: Engineer sees red → understands critical → takes action → **Zero context switch**
...
**Result**: Engineer sees yellow spike → understands warning → checks latency → **Instant correlation**
...
**Result**: Engineer sees graph → understands node states → **Visual matches data**
...
Same semantic token → Same meaning → All media (terminal, dashboard, logs)

### 2. **Zero Context Switch**
Engineer never leaves Tmux session → Dashboard accessible from terminal → Seamless workflow

### 3. **Perceptual Clarity**
Every photon hitting retina carries maximum signal → Colors convey meaning → Typography ensures readability

### 4. **Cross-Medium Consistency**
Terminal colors match dashboard colors → Same semantic language → Reduced cognitive load

### 5. **Unified Nervous System**
Keypress → Token → Photon → Decision → Keypress → **One coherent system**

---

## 📋 Implementation Checklist

### Phase 1: Semantic Token System ✅
...
- [x] Ensure terminal → dashboard color matching
