...
async function main() {
	console.info("🔗 HTML Link Extractor Demo\n");
	...
	console.info(`Found ${allLinks.length} links:`);
	allLinks.forEach((link, i) => {
		console.info(`  ${i + 1}. ${link.url}`);
		console.info(`     Text: "${link.text}"`);
		console.info(`     External: ${link.isExternal}`);
		if (link.title) console.info(`     Title: ${link.title}`);
		if (link.target) console.info(`     Target: ${link.target}`);
		if (link.rel) console.info(`     Rel: ${link.rel}`);
	});
	...
	console.info(`Found ${externalLinks.length} external links:`);
	externalLinks.forEach((link, i) => {
		console.info(`  ${i + 1}. ${link.url} - "${link.text}"`);
	});
	...
	console.info(`Found ${internalLinks.length} internal links:`);
	...
	console.info(`Found ${filteredLinks.length} matching links:`);
	...
	console.info(`Found ${domains.size} unique domains:`);
	Array.from(domains).forEach((domain, i) => {
		console.info(`  ${i + 1}. ${domain}`);
	});
	...
	for (const [domain, links] of grouped.entries()) {
		console.info(`\n  ${domain}:`);
		links.forEach((link) => {
			console.info(`    - ${link.url} - "${link.text}"`);
		});
	}
	...
	console.info(`Found ${responseLinks.length} links from Response`);

	// Demo 8: Limit number of links
	console.info("\n8. Limit Number of Links (max 3):");
	console.info("-".repeat(60));
	const limitedLinks = extractLinksSync(HTML_EXAMPLE, { maxLinks: 3 });
	console.info(`Found ${limitedLinks.length} links (limited to 3):`);
