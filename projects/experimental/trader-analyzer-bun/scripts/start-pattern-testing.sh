...
tmux send-keys -t "$SESSION_NAME:pattern-dev.2" 'tail -f logs/engine.log 2>/dev/null | grep -E "HBAPI-00[2-4]" || echo "No log file found"' Enter
...
tmux send-keys -t "$SESSION_NAME:pattern-benchmark.4" 'tail -f logs/engine.log 2>/dev/null | grep -E "pattern_missed|execution_failed" || echo "No errors found"' Enter
