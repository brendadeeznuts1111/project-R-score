...
export class ProfileAnalyzer {
  ...
  private analyzeLayerPerformance(profile: CPUProfile): LayerPerformance {
    ...
    profile.nodes.forEach((node) => {
      ...
      if (
        functionName.includes('Layer1') ||
        functionName.includes('DirectCorrelation')
      ) {
  ...
  private calculateFunctionTime(
    ...
    // Simplified time calculation based on hit count
    // In production, would use actual time deltas
    return node.hitCount * 0.1; // Placeholder
  }
...
if (import.meta.main) {
  ...
  if (!profilePath) {
    console.error('Usage: bun run analyze-profile.ts <path-to-cpuprofile>');
    process.exit(1);
  }
