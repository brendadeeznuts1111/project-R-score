...
const healthReport = `
# 🩺 Initial Template Health Report
...
# 📐 GOLDEN_FILE_STANDARD.md: Obsidian Vault Governance & Interoperability
...
├── Phase-0-Name/                # Phase 0 Documentation
│   └── Sub-Area/
│       ├── EXECUTION-LOG.md    # Execution log (if applicable)
│       └── GUIDE.md            # Practical guide (if applicable)
├── Phase-1-Name/                # Phase 1 Documentation
├── Standards/                   # Standards & Guidelines (if applicable)
└── Tools/                       # Tools & Utilities (if applicable)
```
...
|-------|------|--------|---------------|-------|------------------|
| Phase 0 | Item 1 | ✅ Complete | [[Link\|Log]] | [[Link\|Guide]] | 6–400× |
...
- `speed_improvement`: `string` - Performance multiplier in standardized format (e.g., "6–400× faster", "183×+ faster")
...
- `throughput`: `string` - Throughput improvement (e.g., "2× throughput", "50% increase")
- `memory_reduction`: `string` - Memory reduction percentage (e.g., "-60%", "4× less")
...
performance_metrics:
  ...
  speed_improvement: "6–400× faster (183×+ average)"
...
- `OWNER_TEAM` → `OWNER_TEAM` (no change)
...
**Usage Examples**:
- Performance optimizations → `Worker Pink` (semantic: "offload", "performance")
- Action/execution components → `Command CH1` (semantic: "action", "execution")
- Data processing → `Data Orange` (semantic: "data", "communication", "flow")
- Core critical systems → `Core Blue` (semantic: "core", "critical", "attention")
