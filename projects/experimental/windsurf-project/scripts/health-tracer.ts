...
async function main() {
  ...
  console.info("--- Monitoring subsystem health and failover status ---\n");
  ...
  setInterval(() => {
    ...
    console.info(`${timestamp} | ${sub.padEnd(15)} | ${status.color}${status.label.padEnd(10)}\x1b[0m | Memory: ${rss}MB`);
    
    if (statusIdx > 0) {
      console.info(`   \x1b[33m⚠️ ALERT:\x1b[0m Abnormal latency detected in ${sub} cluster.`);
    }
  }, 1200);
