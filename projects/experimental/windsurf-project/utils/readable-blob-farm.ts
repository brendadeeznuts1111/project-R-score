...
function getS3Client(): S3Client {
  if (!s3Client) {
    ...
    s3Client = new S3Client({
      ...
      bucket: process.env.S3_BUCKET || config.getEndpoint('storage').r2.bucket || 'duoplus-storage',
      endpoint: process.env.S3_ENDPOINT || config.getEndpoint('storage').r2.endpoint || '',
...
export async function readableToInlineBlob(key: string, readable: Readable) {
  ...
  if (process.env.R2_PUBLIC_DOMAIN) {
    ...
    const bucket = process.env.S3_BUCKET || config.getEndpoint('storage').r2.bucket || 'duoplus-storage';
