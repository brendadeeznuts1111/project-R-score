...
const testConfig = {
  ...
  grafana: {
    ...config.grafana,
    url: 'config.getEndpoint('dashboard').local',
    apiKey: 'test-api-key'
  },
  notifications: {
...
describe('Dashboard Integration Tests', () => {
  ...
  beforeAll(() => {
    ...
    config.getEndpoint('storage').r2.bucket = 'test-bucket';
    config.getEndpoint('storage').r2.endpoint = 'https://test.r2.cloudflarestorage.com';
    config.getEndpoint('analytics').grafana.url = testConfig.grafana.url;
  ...
  describe('Configuration Validation', () => {
    ...
    test('should detect missing required configuration', () => {
      const originalBucket = config.getEndpoint('storage').r2.bucket;
      delete config.getEndpoint('storage').r2.bucket;
      const validation = validateConfig();
      expect(validation.valid).toBe(false);
      expect(validation.errors).toContain('R2 bucket is required');
      config.getEndpoint('storage').r2.bucket = originalBucket;
    });
  });
  ...
  describe('CLI Commands', () => {
    ...
    test('should validate environment variables for deploy', async () => {
      ...
      // Restore
      if (originalBucket) config.getEndpoint('storage').r2.bucket = originalBucket;
    });
  });
