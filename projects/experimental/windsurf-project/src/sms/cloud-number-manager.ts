...
/**
 * Enhanced Cloud Number Manager with DuoPlus Integration
 * 
 * Integrates with DuoPlus cloud numbers for seamless phone verification:
 * - Direct number purchase via DuoPlus API
 * - Automatic assignment to Apple ID accounts
 * - Cost optimization and monitoring
 * - Renewal and lifecycle management
 */

import { DuoPlusSDK, CloudNumberManager } from './duoplus-sdk.js';
...
export class EnhancedCloudNumberManager {
  ...
  async generateCostReport() {
    ...
    return `
📊 DuoPlus Cloud Numbers Cost Report
=====================================
...
`;
  ...
  async fetchMessages(): Promise<any[]> {
    try {
      const response = await axios.get(`${(this.legacyManager as any).baseUrl}/messages`, {
        headers: {
          'Authorization': `Bearer ${(this.legacyManager as any).apiKey}`,
          'Content-Type': 'application/json'
        },
        params: {
          phoneNumber: (this.legacyManager as any).phoneNumber,
          limit: 10
        }
      });
  ...
  async markMessageAsRead(messageId: string): Promise<void> {
    try {
      await axios.post(`${(this.legacyManager as any).baseUrl}/messages/${messageId}/read`, {}, {
        headers: {
          'Authorization': `Bearer ${(this.legacyManager as any).apiKey}`,
          'Content-Type': 'application/json'
        }
  ...
  async getAvailableNumbers(): Promise<any[]> {
    try {
      const response = await axios.get(`${(this.legacyManager as any).baseUrl}/numbers`, {
        headers: {
          'Authorization': `Bearer ${(this.legacyManager as any).apiKey}`,
          'Content-Type': 'application/json'
        }
  ...
  async rentNumber(country = 'US', type = 'non-voip'): Promise<any> {
    try {
      const response = await axios.post(`${(this.legacyManager as any).baseUrl}/numbers/rent`, {
        country,
        type
      }, {
        headers: {
...
export const cloudNumberManager = new EnhancedCloudNumberManager(process.env.DUOPLUS_API_KEY || 'demo-key');
