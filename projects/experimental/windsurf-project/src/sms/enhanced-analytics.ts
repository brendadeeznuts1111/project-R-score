 ...
 * 
 * Comprehensive monitoring for the DuoPlus-enhanced Apple ID system:
...
export class EnhancedAnalyticsManager {
  ...
  private evaluateCondition(condition: string, metrics: SystemMetrics): boolean {
    try {
      ...
      evalCondition = evalCondition.replace(/accounts\.(\w+)/g, (match, prop) => (context.accounts as any)[prop]);
      evalCondition = evalCondition.replace(/devices\.(\w+)/g, (match, prop) => (context.devices as any)[prop]);
      evalCondition = evalCondition.replace(/costs\.(\w+)/g, (match, prop) => (context.costs as any)[prop]);
      evalCondition = evalCondition.replace(/performance\.(\w+)/g, (match, prop) => (context.performance as any)[prop]);
      evalCondition = evalCondition.replace(/rpa\.(\w+)/g, (match, prop) => (context.rpa as any)[prop]);
...
<html>
<head>
    <title>DuoPlus Apple ID System Dashboard</title>
    ...
    <style>
        ...
        .alert-high { background: #f8d7da; border: 1px solid #f5c6cb; color: #721c24; }
        .alert-medium { background: #fff3cd; border: 1px solid #ffeaa7; color: #856404; }
    </style>
</head>
<body>
    <h1>🚀 DuoPlus Apple ID System Dashboard</h1>
