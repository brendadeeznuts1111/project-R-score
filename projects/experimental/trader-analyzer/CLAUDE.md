...
**Bun 1.3.4+ Web APIs**:
- `URLPattern` - Declarative URL pattern matching for routing (like regex for URLs)
- `jest.useFakeTimers()` - Fake timers for testing time-sensitive code (10x faster tests)
- `fetch()` proxy headers - Custom proxy headers for authenticated proxy access
...
- `http.Agent` connection pooling - Fixed in v1.3.4 (80-95% reduction in connection overhead)
