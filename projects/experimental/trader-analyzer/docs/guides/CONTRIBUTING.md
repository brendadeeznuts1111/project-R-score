...
### 2.4. [SETUP.MONOREPO.RG] Monorepo Workflows

Use `bun --filter` to work with specific packages:
...
**Team Commands**: Use `source scripts/team-aliases.sh` for convenient shortcuts.

See [`docs/guides/BUN-FILTER-WORKFLOWS.md`](./BUN-FILTER-WORKFLOWS.md) for complete guide.
...
#### 5.1.4. [STYLE.CONSTANTS.RG] Constants Usage

**Always use centralized constants** for URLs, paths, and configuration values:
...
- `BUN_DOCS_URLS` - Bun documentation URLs (see `src/utils/rss-constants.ts`)
- `RSS_API_PATHS` - API endpoint paths
- `DATABASE_PATHS` - Database file paths
- `RSS_DASHBOARD_PATHS` - Dashboard HTML paths
- `WORKSPACE_PATHS` - Workspace directory paths
...
**See Also**: [`docs/CONSTANTS-USAGE.md`](../CONSTANTS-USAGE.md) for complete constants guide
