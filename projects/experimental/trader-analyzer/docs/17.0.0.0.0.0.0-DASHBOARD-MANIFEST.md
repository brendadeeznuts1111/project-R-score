...
- `background_color`: `#1a1a1a` → Matches `--bg-primary: oklch(25% 0.05 250)` ≈ `#1a1a1a`
- `theme_color`: `#667eea` → Matches `--accent-purple: #667eea` (Dashboard department color)

---

## 🎨 Design System Integration (`_design_system`)
...
- `background_primary` → `--bg-primary: oklch(25% 0.05 250)`
- `theme_color` → `--accent-purple: #667eea`
- `risk_critical` → `--color-risk-critical: #ef4444`
- `risk_warning` → `--color-risk-warning: #f59e0b`
...
## 🔄 Closed-Loop Pipeline Integration
...
"closed_loop_pipeline": {
  ...
  "workflow": "Terminal log → Dashboard visualization → Same semantic token → Zero context switch",
...
**Example Flow**:
1. Terminal log: `[WARN] HBMO-017` → Yellow (`colour220`)
2. Dashboard alert: `border-left: 6px solid var(--color-risk-warning)` → Yellow (`#f59e0b`)
3. **Same semantic token** → **Same meaning** → **Zero context switch**

---

## 🎯 Philosophy Integration (`_philosophy`)
...
"_philosophy": {
  ...
  "closed_loop_pipeline": "Keypress → Token → Photon → Decision → Keypress",
  "zero_context_switch": "Engineer never leaves tmux session - dashboard always accessible",
  "deterministic_radiance": "Same semantic token → Same meaning → All media"
}
```
...
### Design System Colors → Manifest Colors
...
## 🔧 MCP Error Codes Integration (`_mcp_error_codes`)

### Error Code Reference Dashboard

**Version**: 6.1.1.2.2.8.1.1.2.6  
**Status**: ✅ Integrated  
**Location**: `dashboard/index.html` → MCP Monitoring Dashboard → Error Codes Tab
...
2. **View Details**: Click "View Details →" link to open full documentation
...
**Version**: 17.0.1.0.0.0.0  
**Status**: ✅ Integrated  
**Philosophy**: **"Same semantic token → Same meaning → All media"**
