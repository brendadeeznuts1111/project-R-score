...
# Note: Shows what would be published without actually publishing
# Correct description: "Perform a dry run without making changes"
# (Some Bun versions may show incorrect description "Don't install anything")
bun publish --dry-run
...
**`--dry-run` Flag**:
- ✅ **Correct Description**: "Perform a dry run without making changes"
- ❌ **Incorrect Description** (older versions): "Don't install anything"
- **What it does**: Displays files that would be published without actually publishing to the registry
- **Fixed in**: Bun v1.3.4+
- **See**: [`BUN-PUBLISH-HELP-VERIFICATION.md`](./BUN-PUBLISH-HELP-VERIFICATION.md) for version compatibility
...
Persistently patch packages in `node_modules` with git-friendly `.patch` files.

**Usage**:
```bash
# Prepare package for patching (always do this first!)
bun patch <package>

# Commit patch (after making changes)
...
**Workflow**:
1. **Prepare**: `bun patch <package>` - Creates unlinked copy in `node_modules`
2. **Edit**: Make changes to files in `node_modules/<package>/`
3. **Test**: Verify changes work correctly
4. **Commit**: `bun patch --commit <package>` - Generates `.patch` file

**What Happens**:
- Generates `.patch` file in `patches/` directory
- Updates `package.json` with `patchedDependencies` entry
- Updates lockfile
- Patch is automatically applied on future `bun install`

**Important**: Always run `bun patch <pkg>` first! This ensures the package is unlinked from Bun's global cache. Editing without preparing first will modify the global cache.
...
**See**: [Bun Patch](./BUN-PATCH.md) - Complete patching guide with examples and best practices.
...
- [Bun Patch](./BUN-PATCH.md) - Persistent package patching guide
...
bun patch --commit <package>     # Commit patch
