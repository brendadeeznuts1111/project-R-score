...
async function verifySQLiteVersion(): Promise<void> {
  console.info("🔍 Verifying SQLite version...\n");
  ...
  try {
    ...
    console.info(`✅ SQLite version: ${sqliteVersion}`);
    ...
    if (isSupported) {
      console.info(`✅ SQLite 3.51.0+ features available\n`);
    } else {
      console.info(`⚠️  SQLite version ${sqliteVersion} detected. Bun 1.3.3+ includes SQLite 3.51.0.\n`);
    }

    // Display PRAGMA settings
    console.info("📊 Current PRAGMA settings:");
    ...
    console.info(`   journal_mode: ${journalMode.journal_mode}`);
    console.info(`   synchronous: ${synchronous.synchronous}`);
    console.info(`   foreign_keys: ${foreignKeys.foreign_keys}`);
    console.info(`   cache_size: ${cacheSize.cache_size} pages\n`);

    // Test WAL mode (note: in-memory databases use "memory" mode)
    console.info("🧪 Testing WAL mode...");
    db.exec("PRAGMA journal_mode = WAL");
    const walMode = db.query("PRAGMA journal_mode").get() as { journal_mode: string };
    if (walMode.journal_mode === "wal") {
      console.info(`✅ WAL mode enabled successfully\n`);
    } else if (walMode.journal_mode === "memory") {
      console.info(`ℹ️  In-memory database uses "memory" journal mode (expected)\n`);
    } else {
      console.info(`⚠️  WAL mode not available (current: ${walMode.journal_mode})\n`);
    }

    // Performance test
    console.info("⚡ Performance test (1000 inserts)...");
    db.exec(`
    ...
    console.info(`   Duration: ${durationMs.toFixed(2)}ms`);
    console.info(`   Throughput: ~${opsPerSec.toLocaleString()} ops/sec\n`);

    // Cleanup
    db.exec("DROP TABLE IF EXISTS test_perf");
    db.close();

    console.info("✅ SQLite verification complete!");
  } catch (error) {
