	...
	const telegramInfo = teamInfo.supergroup 
		? `
			<div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #ccc;">
				<strong>💬 Telegram:</strong> ${escapeHtml(teamInfo.supergroup)}
	...
	const telegramLink = teamInfo.telegramHandle
		? `
			...
			<a href="https://t.me/${teamInfo.telegramHandle.replace('@', '')}" class="telegram-link">
				📱 ${escapeHtml(teamInfo.telegramHandle)}
			</a>
		`
		: '';
...
<body>
	<div class="container">
		<h1>
			📦 ${escapeHtml(pkg.name)} <span class="${getBadgeClass(pkg.version)}">v${escapeHtml(pkg.version)}</span>
		</h1>
