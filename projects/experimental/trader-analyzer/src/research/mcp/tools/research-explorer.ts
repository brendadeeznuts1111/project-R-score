			...
			execute: async (args: { sport: string; hours?: number }) => {
				...
				return {
					content: [
						{
							text:
								`🔬 Discovered ${patterns.length} patterns for ${args.sport.toUpperCase()}\n\n` +
								patterns
									.map(
										(p: ResearchPattern) =>
      ...
      `);
		...
		},
		{
			...
			execute: async (args: { sport?: string; hours?: number }) => {
				...
				const text =
					`🔍 URL Anomaly Patterns Discovered\n\n` +
					patterns
						.map(
							(p) =>
								`${p.pattern_name}\n` +
			...
			execute: async (args: { bookmaker: string; hours?: number }) => {
				...
				return {
					content: [
						{
							text:
								`📊 False Steam Rate for ${args.bookmaker}\n\n` +
			...
			execute: async (args: { startDate: string; endDate: string }) => {
				...
				return {
					content: [
						{
							text:
								`✅ Historical Data Correction Complete\n\n` +
