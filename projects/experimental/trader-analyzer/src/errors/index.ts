...
export class NexusError extends Error {
	 ...
	 * Provides concise, domain-specific console output respecting --console-depth settings
	 */
	[Bun.inspect.custom](depth: number, options: any): string {
		if (depth < 0) {
			return '[NexusError]'; // Concise when truncated
		}
		// Show error code and message for shallow inspection
		if (depth === 0) {
			return `[NexusError:${this.code}]`;
		}
		// Full details for deep inspection
		return `[NexusError:${this.code}] ${this.message}${this.details ? ` ${JSON.stringify(this.details)}` : ''}`;
	}
}
