...
class AgentRankingsSystem {
  ...
  async generateRankings(state?: string): Promise<AgentRanking[]> {
    ...
    try {
      ...
      console.error(`❌ Failed to generate rankings: ${error.message}`);
      return [];
    }
  }
...
*Generated by Agent Rankings System v2.8 • Live data from Datapipe API*
`;
...
${rankings.slice(0, 3).map((agent, i) =>
  ...
  private generateHeatmap(rankings: AgentRanking[]): string {
    ...
    const tiers = [
      { name: 'Elite', minProfit: 5000, color: '🔥' },
      { name: 'Pro', minProfit: 1000, color: '⭐' },
      { name: 'Solid', minProfit: 100, color: '✅' },
      { name: 'Developing', minProfit: -100, color: '📈' },
      { name: 'Challenging', minProfit: -1000, color: '⚠️' },
      { name: 'Critical', minProfit: -10000, color: '🚨' }
    ];
...
📊 *Overall:* ${rankings.length} agents, $${totalProfit.toLocaleString()} total profit
