...
async function cleanLogs() {
  try {
    console.info(`🧹 Reading logs from: ${logFile}`);
    ...
    console.info(`✅ Clean logs written to: ${outputFile}`);
    console.info(`📊 Cleaned ${rawLog.length} chars → ${clean.length} chars (${Math.round((1 - clean.length/rawLog.length) * 100)}% reduction)`);
