...
describe("Official Bun Documentation Compliance", () => {
  ...
  test("should implement sleep utilities exactly as documented", async () => {
    ...
    console.info(`✅ Bun.sleep(ms): ~${end1 - start1}ms (matches official docs)`);
    ...
    console.info(`✅ Bun.sleep(Date): ~${end2 - start2}ms (matches official docs)`);
    ...
    console.info(`✅ Bun.sleepSync(ms): ~${end3 - start3}ms (matches official docs)`);
  });
  ...
  test("should implement UUID v7 exactly as documented", () => {
    ...
    console.info(`✅ Bun.randomUUIDv7(): "${id1}" (matches official docs)`);
    
    // ✅ Bun.randomUUIDv7(encoding) - Exactly as documented
    const id2 = Bun.randomUUIDv7("base64");
    expect(id2).toMatch(/^[A-Za-z0-9+/]+={0,2}$/);
    console.info(`✅ Bun.randomUUIDv7("base64"): "${id2}" (matches official docs)`);
    ...
    console.info(`✅ Bun.randomUUIDv7("buffer"): ${id3.length} bytes (matches official docs)`);
    ...
    console.info(`✅ Bun.randomUUIDv7(timestamp): "${id4}" (matches official docs)`);
  });
  ...
  test("should implement peek utilities exactly as documented", () => {
    ...
    console.info(`✅ Bun.peek(resolved): "${Bun.peek(resolvedPromise)}" (matches official docs)`);
    
    // ✅ Bun.peek.status(promise) - Exactly as documented
    expect(Bun.peek.status(resolvedPromise)).toBe("fulfilled");
    console.info(`✅ Bun.peek.status(resolved): "${Bun.peek.status(resolvedPromise)}" (matches official docs)`);
    
    // ✅ Bun.peek(non-promise) - Exactly as documented
    expect(Bun.peek(42)).toBe(42);
    console.info(`✅ Bun.peek(non-promise): ${Bun.peek(42)} (matches official docs)`);
    ...
    console.info(`✅ Bun.peek(rejected): Error message (matches official docs)`);
  });
  ...
  test("should implement deepEquals exactly as documented", () => {
    ...
    console.info(`✅ Bun.deepEquals(): Basic comparison (matches official docs)`);
    ...
    console.info(`✅ Bun.deepEquals(obj1, obj2, true): Strict mode (matches official docs)`);
  });
  ...
  test("should implement string utilities exactly as documented", () => {
    ...
    console.info(`✅ Bun.stringWidth("hello"): ${Bun.stringWidth("hello")} (matches official docs)`);
    ...
    console.info(`✅ Bun.stringWidth(ANSI, options): ${Bun.stringWidth(ansiText, { countAnsiEscapeCodes: true })} (matches official docs)`);
    
    // ✅ Bun.escapeHTML(value) - Exactly as documented
    const html = '<script>alert("xss")</script>';
    const escaped = Bun.escapeHTML(html);
    expect(escaped).toBe('&lt;script&gt;alert(&quot;xss&quot;)&lt;/script&gt;');
    console.info(`✅ Bun.escapeHTML(): HTML escaped (matches official docs)`);
    ...
    console.info(`✅ Bun.stripANSI(): "${Bun.stripANSI(coloredText)}" (matches official docs)`);
  });
  ...
  test("should implement path utilities exactly as documented", () => {
    ...
    console.info(`✅ Bun.fileURLToPath(): "${filePath}" (matches official docs)`);
    ...
    console.info(`✅ Bun.pathToFileURL(): "${convertedURL}" (matches official docs)`);
  });
  ...
  test("should implement compression utilities exactly as documented", () => {
    ...
    console.info(`✅ Bun.gzipSync()/gunzipSync(): ${originalBuffer.length}→${compressed.length} bytes (matches official docs)`);
    ...
    console.info(`✅ Bun.deflateSync()/inflateSync(): ${originalBuffer.length}→${deflated.length} bytes (matches official docs)`);
  });
  ...
  test("should implement inspection utilities exactly as documented", () => {
    ...
    console.info(`✅ Bun.inspect(): Object serialization (matches official docs)`);
    ...
    console.info(`✅ Bun.inspect.table(): Table formatting (matches official docs)`);
  });
  ...
  test("should implement high-precision timing exactly as documented", () => {
    ...
    console.info(`✅ Bun.nanoseconds(): ${ns} nanoseconds (matches official docs)`);
  });
  ...
  test("should demonstrate script execution detection exactly as documented", () => {
    ...
    console.info(`✅ import.meta.path === Bun.main: ${isDirectExecution} (matches official docs)`);
