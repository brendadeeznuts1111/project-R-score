...
async function basicReadableStream() {
  console.info('🌊 Basic ReadableStream Reading:');
  ...
  for await (const chunk of stream) {
    ...
    if (chunkCount <= 3) {
      console.info(`  Chunk ${chunkCount}: ${chunk.byteLength} bytes`);
      console.info(`    First 20 bytes: ${Array.from(chunk.slice(0, 20)).map(b => b.toString(16).padStart(2, '0')).join(' ')}`);
  ...
  console.info(`File size matches bytes read: ${totalBytes === file.size}`);
}
...
async function streamWithTransformation() {
  ...
  console.info('Processing README.md as stream with line counting...');
  ...
  console.info(`Lines: ${lineCount}`);
  console.info(`Words: ${wordCount}`);
  console.info(`Characters: ${characterCount}`);
}
...
async function streamWithGetReader() {
  ...
  console.info('Processing server.ts with manual reader...');
  ...
  try {
    while (true) {
      ...
      if (done) {
        console.info('Stream reading complete');
        break;
      }
      ...
      // Process the chunk
      if (chunkCount <= 2) {
        console.info(`  Chunk ${chunkCount}: ${value.byteLength} bytes`);
...
async function streamProcessingAndWriting() {
  ...
  console.info('Processing package.json stream and writing to stream-output.json...');
  ...
  console.info(`Content matches: ${outputContent === inputContent}`);
  
  // Clean up
  try {
    await fs.promises.unlink("./stream-output.json");
    console.info('Cleaned up output file');
  } catch (error: any) {
...
async function streamWithAsyncGenerators() {
  ...
  console.info('Processing dashboard.html with async generator...');
  ...
  for await (const result of processStream(file)) {
    ...
    if (chunkCount <= 2) {
      console.info(`  Processed chunk ${chunkCount}:`);
      console.info(`    Original size: ${result.originalSize} bytes`);
      console.info(`    Transformed preview: ${result.content}`);
    } else if (chunkCount === 3) {
      console.info('  ... (additional chunks omitted for brevity)');
    }
  }
...
async function streamWithErrorHandling() {
  ...
  try {
    ...
    for await (const chunk of stream) {
      console.info('This should not be printed');
    }
  } catch (error: any) {
    console.info(`Caught expected error: ${error.message}`);
  }
  ...
  try {
    ...
    console.info(`Caught processing error: ${error.message}`);
  }
}
...
async function streamPerformanceComparison() {
  ...
  console.info(`Stream read ${streamBytes} bytes`);
  console.info(`Content sizes match: ${fullContent.length === streamBytes}`);
}

// Main execution
async function main() {
  console.info('🚀 ReadableStream Examples');
  console.info('========================');
