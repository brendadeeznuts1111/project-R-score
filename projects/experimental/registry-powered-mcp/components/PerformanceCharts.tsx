...
export const PerformanceCharts: React.FC = () => {
  const chartContext = `Performance Data (24h):
${data.map(d => `${d.name}: ${d.reqs} requests, ${d.latency}ms latency`).join('\n')}

Peak: ${Math.max(...data.map(d => d.reqs))} requests at ${data.find(d => d.reqs === Math.max(...data.map(x => x.reqs)))?.name}
Avg Latency: ${(data.reduce((a, b) => a + b.latency, 0) / data.length).toFixed(1)}ms`;
