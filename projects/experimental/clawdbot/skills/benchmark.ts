...
console.info("Bun JSON Serialization Benchmark");
console.info("═".repeat(50));
console.info(`Bun version: ${Bun.version}`);
console.info(`Iterations: ${ITERATIONS}`);
console.info(`Payload size: ~${JSON.stringify(largePayload).length} bytes`);
console.info("");

// Benchmark 1: JSON.stringify
console.info("1. JSON.stringify()");
const start1 = Bun.nanoseconds();
for (let i = 0; i < ITERATIONS; i++) {
  JSON.stringify(largePayload);
}
const time1 = (Bun.nanoseconds() - start1) / 1_000_000;
console.info(`   ${time1.toFixed(2)}ms total, ${(time1 / ITERATIONS).toFixed(3)}ms per op`);
...
console.info(`   ${time2.toFixed(2)}ms total, ${(time2 / ITERATIONS).toFixed(3)}ms per op`);
console.info(`   Speedup: ${(time1 / time2).toFixed(2)}x`);

// Benchmark 3: console.log with %j - SIMD FastStringifier
console.info("\n3. console.info('%j') [SIMD FastStringifier]");
...
for (let i = 0; i < ITERATIONS; i++) {
  console.info("%j", largePayload);
}
const time3 = (Bun.nanoseconds() - start3) / 1_000_000;
console.log = origLog;
console.info(`   ${time3.toFixed(2)}ms total, ${(time3 / ITERATIONS).toFixed(3)}ms per op`);
console.info(`   Speedup vs stringify: ${(time1 / time3).toFixed(2)}x`);
...
console.info(`   ${time4.toFixed(2)}ms total, ${(time4 / ITERATIONS).toFixed(3)}ms per op`);
console.info(`   Throughput: ${((testData.length * ITERATIONS) / (time4 / 1000) / 1_000_000).toFixed(2)} MB/s`);
...
console.info(`   ${time5.toFixed(2)}ms total, ${(time5 / ITERATIONS).toFixed(3)}ms per ${colors.length} colors`);

// Summary
console.info("\n" + "═".repeat(50));
console.info("Summary:");
console.info(`  JSON.stringify:    ${(time1 / ITERATIONS).toFixed(3)}ms/op (baseline)`);
console.info(`  Response.json():   ${(time1 / time2).toFixed(2)}x faster`);
console.info(`  console.info(%j):   ${(time1 / time3).toFixed(2)}x faster`);
console.info(`  CRC32 throughput:  ${((testData.length * ITERATIONS) / (time4 / 1000) / 1_000_000).toFixed(0)} MB/s`);
