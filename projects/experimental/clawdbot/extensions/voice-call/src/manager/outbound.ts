...
export async function initiateCall(
  ...
  try {
    ...
    if (mode === "notify" && initialMessage) {
      ...
      console.info(`[voice-call] Using inline TwiML for notify mode (voice: ${pollyVoice})`);
    }
...
export async function speakInitialMessage(
  ...
  if (!initialMessage) {
    console.info(`[voice-call] speakInitialMessage: no initial message for ${call.callId}`);
    return;
  }
  ...
  console.info(`[voice-call] Speaking initial message for call ${call.callId} (mode: ${mode})`);
  ...
  if (mode === "notify") {
    ...
    console.info(`[voice-call] Notify mode: auto-hangup in ${delaySec}s for call ${call.callId}`);
    setTimeout(async () => {
      const currentCall = ctx.activeCalls.get(call.callId);
      if (currentCall && !TerminalStates.has(currentCall.state)) {
        console.info(`[voice-call] Notify mode: hanging up call ${call.callId}`);
