...
export function registerHooksCli(program: Command): void {
    ...
    .action(async (raw: string, opts: { link?: boolean }) => {
      ...
      if (fs.existsSync(resolved)) {
        if (opts.link) {
          ...
          // Batch entry updates to avoid O(n) nested spreads
          const entries = { ...next.hooks?.internal?.entries };
          for (const hookName of probe.hooks) {
            entries[hookName] = {
              ...(entries[hookName] as object | undefined),
              enabled: true,
            };
          }
          next = {
            ...
            hooks: {
              ...next.hooks,
              internal: { ...next.hooks?.internal, entries },
            },
          };
  ...
  hooks
    ...
    .action(async (id: string | undefined, opts: HooksUpdateOptions) => {
      ...
      // Filter valid npm hooks upfront to enable parallelization
      const validHooks: Array<{
        hookId: string;
        record: (typeof installs)[string];
        installPath: string;
      }> = [];
      ...
      if (opts.dryRun) {
        // Parallel dry-run: read versions + probe registry concurrently
        const results = await Promise.all(
          validHooks.map(async ({ hookId, record, installPath }) => {
            const [currentVersion, probe] = await Promise.all([
              readInstalledPackageVersion(installPath),
              installHooksFromNpmSpec({
                spec: record.spec!,
                mode: "update",
                dryRun: true,
                expectedHookPackId: hookId,
                logger: { info: () => {}, warn: () => {} }, // Suppress per-hook logs in parallel
              }),
            ]);
            return { hookId, currentVersion, probe };
          }),
        );

<<<<<<< HEAD
        for (const { hookId, currentVersion, probe } of results) {
||||||| de33bc70e
        if (opts.dryRun) {
...
>>>>>>> upstream/main
      ...
      // Sequential updates to avoid npm rate limits
      let nextCfg = cfg;
      let updatedCount = 0;

      for (const { hookId, record, installPath } of validHooks) {
