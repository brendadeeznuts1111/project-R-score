...
export function startGatewayMaintenanceTimers(params: {
  ...
  const dedupeCleanup = setInterval(() => {
    ...
    if (params.dedupe.size > DEDUPE_MAX) {
      // Delete oldest entries without full sort - O(k*n) is better than O(n log n) when k is small
      const toDelete = params.dedupe.size - DEDUPE_MAX;
      for (let i = 0; i < toDelete; i++) {
        ...
        if (oldestKey) params.dedupe.delete(oldestKey);
      }
    }
