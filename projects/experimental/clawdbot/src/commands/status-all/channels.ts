...
function sha256HexPrefix(value: string, len = 8): string {
  return new Bun.CryptoHasher("sha256").update(value).digest("hex").slice(0, len);
}
...
function summarizeTokenConfig(params: {
  ...
  if (hasBotOrAppTokenFields) {
    // Single pass to categorize accounts (avoids iterating enabled twice)
    const ready: typeof enabled = [];
    const partial: typeof enabled = [];
    ...
    // Single pass to collect both sources (avoids iterating ready twice)
    const botSourceList: string[] = [];
    const appSourceList: string[] = [];
    for (const a of ready) {
      botSourceList.push(a.snapshot.botTokenSource ?? "none");
      appSourceList.push(a.snapshot.appTokenSource ?? "none");
    }
    const botSources = summarizeSources(botSourceList);
    const appSources = summarizeSources(appSourceList);
