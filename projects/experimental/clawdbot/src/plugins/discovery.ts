...
const INDEX_CANDIDATES = ["index.ts", "index.js", "index.mjs", "index.cjs"] as const;

/** Find first existing index file in a directory (avoids creating all paths upfront) */
function findIndexFile(dir: string): string | undefined {
...
function readPackageManifest(dir: string): PackageManifest | null {
  ...
  // Single syscall: try to read, catch if not found (avoids existsSync + readFileSync)
  try {
...
function discoverFromPath(params: {
  ...
  if (stat.isDirectory()) {
    ...
    const indexFile = findIndexFile(resolved);
    if (indexFile && isExtensionFile(indexFile)) {
      addCandidate({
        candidates: params.candidates,
        seen: params.seen,
