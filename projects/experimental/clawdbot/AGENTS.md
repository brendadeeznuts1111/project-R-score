...
- Restart: stop old gateway and run:
  ```bash
  # Kill existing gateway with logging
  if pkill -9 -f clawdbot-gateway; then
    echo "Stopped existing gateway"
  else
    echo "No existing gateway process found"
  fi
  
  # Start new gateway with proper logging
  LOG_DIR="${TMPDIR:-/tmp}/clawdbot"
  mkdir -p "$LOG_DIR"
  nohup clawdbot gateway run --bind loopback --port 18789 --force > "$LOG_DIR/clawdbot-gateway.log" 2>&1 &
  echo "Gateway started, log: $LOG_DIR/clawdbot-gateway.log"
  ```
- Verify: `clawdbot channels status --probe`, `ss -ltnp | rg 18789`, `tail -n 120 "${TMPDIR:-/tmp}/clawdbot/clawdbot-gateway.log"`.
...
## Skills API Server (`skills/`)

The `skills/` directory contains a standalone Skill Management System with an API server and dashboard.

### Structure
- `skills/src/api-server.ts` - Main API server with embedded dashboard (~2500 LOC)
- `skills/src/skills.ts` - Skill registry and management
- `skills/src/packager.ts` - Skill packaging utilities
- `skills/src/version-manager.ts` - Version management
- `skills/skills/` - Individual skill implementations (weather, etc.)
...
- `GET /api/metrics` - Execution metrics and system stats
- `GET /api/logs` - Server logs (filterable)
- `POST /api/skills/:id/execute` - Execute a skill
- `DELETE /api/logs` - Clear logs

### Dashboard Features
- Real-time metrics with 5s polling
- System health tension ring (circular SVG indicator)
- Sparkline charts for executions/errors over 24h
- Log viewer with level/category filtering
- Skill status grid
- Terminal session tracking
- Quick actions (refresh, export metrics, emergency stop)

### Development Notes
- API key: Use `dev-key` for local development or set `SKILL_API_KEYS` env var
- Debug mode: Set `DEBUG=1` for verbose logging
- The dashboard is a single embedded HTML page in `api-server.ts`
- Keep the server running in background: `bun run src/api-server.ts &`
||||||| de33bc70e
