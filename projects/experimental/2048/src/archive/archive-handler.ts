...
export async function buildOddsDataset(
  ...
  if (process.env.NODE_ENV !== "production") {
    console.info("%j", manifest); // 3x faster JSON serialization in Bun v1.3.6
  }
...
async function main() {
  ...
  console.info(`🔢 Algorithms: ${algorithms.join(", ")}`);
  console.info(`💾 Sizes: ${sizes.map((s) => `${s}B`).join(", ")}`);
  console.info("");
  ...
  if (result.success) {
    ...
    console.info(`   Original: ${(result.originalSize / 1024).toFixed(2)} KB`);
    console.info(
      `   Ratio: ${((result.size / result.originalSize) * 100).toFixed(1)}%`,
    );
    console.info(`   CRC32: 0x${result.crc32}`);
    console.info(`   Entries: ${result.entries}`);
    console.info(`   Duration: ${result.duration.toFixed(2)}ms`);
  } else {
    console.info(`❌ Archive creation failed: ${result.error}`);
    process.exit(1);
  }
}
