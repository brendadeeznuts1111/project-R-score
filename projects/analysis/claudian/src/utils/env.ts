...
function getExtraBinaryPaths(): string[] {
  ...
  if (isWindows) {
    ...
    // Bun locations (primary)
    if (appData) {
      paths.push(path.join(appData, 'bun', 'bin'));
      paths.push(path.join(appData, 'bun'));
    }

    // Node.js / npm locations (fallback)
    if (appData) {
 ...
 * Note: Also searches bun installation paths since bun bundles node.
 */
 ...
 * where binaries like bun, node, python, etc. are typically installed.
 *
 ...
 *                  that directory is added to PATH. This handles nvm, fnm, volta, bun, etc.
 *                  where package manager globals are installed alongside node.
 */
export function getEnhancedPath(additionalPaths?: string, cliPath?: string): string {
  ...
  // This handles nvm, fnm, volta, asdf, bun, etc. where package manager globals are installed
  ...
  // search common locations for Node.js (including bun installations)
