...
```bash
bun run test -- --selectProjects unit        # Run unit tests
bun run test -- --selectProjects integration # Run integration tests
bun run test:coverage -- --selectProjects unit # Unit coverage
```
...
| CLI detection | `/usr/local/bin`, `~/.local/bin`, etc. | `%LOCALAPPDATA%\Claude`, `%APPDATA%\bun`, etc. |
...
- Run `bun run typecheck`, `bun run lint`, `bun run test`, `bun run build` after editing
