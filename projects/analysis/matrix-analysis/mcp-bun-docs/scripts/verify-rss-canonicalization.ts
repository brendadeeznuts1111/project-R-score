...
async function main(): Promise<void> {
	console.info("Tier-1380 RSS Canonicalization Verification\n");
	...
	for (const url of [BUN_CHANGELOG_RSS, BUN_SH_RSS]) {
		...
		if (res.ok) {
			...
			console.info(`  First 200 chars width: ${w} cols`);
		}
		console.info("");
	}
	...
	for (const url of DEAD_PATHS) {
		...
		console.info(url);
		console.info(`  HTTP ${res.status}  ${expect404 ? "✓ (404 expected)" : "✗"}`);
		console.info("");
	}
	...
	const safe = width <= 89;
	console.info("Typical RSS item preview (Col-89 check):");
	console.info(`  "${preview}"`);
	console.info(`  Width: ${width} cols  ${safe ? "✓ under 89" : "✗ over 89"}`);
