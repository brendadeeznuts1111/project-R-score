...
concepts stay in vocabulary + `concept:audit`. **Domain** = desk / business
lane (e.g. partner loop), not a single entity type. For `BM-*` catalog gaps
also set **Tenant** / **Owner** —
[ISSUE-ROUTING.md](../../docs/harness/ISSUE-ROUTING.md).

| Field | Value | Rule |
|-------|-------|------|
| **Domain** | `partner` · `control` · `trading` · `identity` · `knowledge` · `platform` | Required — desk loop / chrome `domainLanes` |
| **Tenant** | e.g. `bookmakers` · or `n/a` | When a residual table owns the entity (bookmaker ≠ partner) |
| **Owner** | desk lane or tenant id · or `n/a` | When the fix lane differs from Tenant (e.g. BM-5 → `platform`) |
