...
```bash
# Kill running app
pkill -x "Clawdbot" || echo "Warning: No running Clawdbot process found"

# Move old version to system temp directory
BACKUP_DIR="${TMPDIR:-/tmp}/Clawdbot-backup-$(date +%Y%m%d-%H%M%S)"
mv /Applications/Clawdbot.app "$BACKUP_DIR.app" || echo "Warning: Could not backup existing app"
