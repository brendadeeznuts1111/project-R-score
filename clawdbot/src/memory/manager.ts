...
export type { MemorySource } from "./manager-embeddings.js";
...
>>>>>>> upstream/main
  ...
  async probeEmbeddingAvailability(): Promise<{ ok: boolean; error?: string }> {
    try {
      await embedBatchWithRetry(this.createEmbeddingContext(), ["ping"]);
      return { ok: true };
    } catch (err) {
  ...
  private isSessionFileForAgent(sessionFile: string): boolean {
    return isSessionFileForAgent(sessionFile, this.agentId);
  }
  ...
  private async syncSessionFiles(params: {
    ...
    const files = await this.listSessionFilesForAgent();
    ...
    const tasks = files.map((absPath) => async () => {
      ...
      const entry = await this.buildSessionEntryForFile(absPath);
      if (!entry) {
        if (params.progress) {
          params.progress.completed += 1;
          params.progress.report({
  ...
  private async runSafeReindex(params: {
    ...
    try {
      ...
      pruneEmbeddingCacheIfNeeded(this.createEmbeddingContext());
  ...
  private async listSessionFilesForAgent(): Promise<string[]> {
    return listSessionFiles(this.agentId);
  }

  private sessionPathForFile(absPath: string): string {
    return sessionPathForFile(absPath);
  }

  private async buildSessionEntryForFile(absPath: string): Promise<SessionFileEntry | null> {
    return buildSessionEntry(absPath, (msg) => log.debug(msg));
  }

  /**
   * Create embedding context for use with extracted embedding functions.
   */
  private createEmbeddingContext(): EmbeddingContext {
    return {
      db: this.db,
...
>>>>>>> upstream/main
  ...
  private async indexFile(
    ...
    const ctx = this.createEmbeddingContext();
