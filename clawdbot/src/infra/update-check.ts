...
async function exists(p: string): Promise<boolean> {
  if (isBun) return Bun.file(p).exists();
  try {
    await fs.access(p);
    return true;
  } catch {
...
async function detectPackageManager(root: string): Promise<PackageManager> {
  try {
    ...
    const raw = isBun ? await Bun.file(pkgPath).text() : await fs.readFile(pkgPath, "utf-8");
  ...
  // Parallel file existence checks (faster than readdir for known files)
  const [hasPnpm, hasBunLock, hasNpm] = await Promise.all([
...
export async function checkGitUpdateStatus(params: {
  ...
  const [branchRes, upstreamRes, dirtyRes] = await Promise.all([
    ...
    runCommandWithTimeout(["git", "-C", root, "status", "--porcelain"], { timeoutMs }).catch(
      () => null,
    ),
  ]);
