...
| `1.3.14` | [`bun-1.3.14.test.ts`](./bun-1.3.14.test.ts) |
[blog](https://bun.com/blog/bun-v1.3.14) | | `1.3.12` |
[`bun-1.3.12.test.ts`](./bun-1.3.12.test.ts) |
[blog](https://bun.com/blog/bun-v1.3.12) |

## New version checklist

1. `bun run regression:new -- X.Y.Z`.
2. Read https://bun.com/blog/bun-vX.Y.Z — split Features / Performance /
   Bugfixes.
3. Replace scaffold / `TODO` skips with one `fixTest` (or `fixSkip`) per bullet.
4. Run `bun test tests/regression/bun-X.Y.Z.test.ts`; document skips in the file
   header.
