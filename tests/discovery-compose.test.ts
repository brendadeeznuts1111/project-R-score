...
describe('discovery-compose', () => {
  test(
    'runCombinedDiscovery merges harness and public planes',
    async () => {
  ...
  test(
    'discover:compose:check passes on current tree',
    async () => {
      const report = await runCombinedDiscovery({ skipUnused: true });
      expect(
        combinedReportPasses(report, { harnessMinSeverity: 'error', publicMinSeverity: 'error' })
      ).toBe(true);
    },
    { timeout: 30_000 }
  );
