...
/**
 * Gets the package manager global prefix directory.
 * Returns null if package manager is not available or prefix cannot be determined.
 */
function getPackageManagerGlobalPrefix(): string | null {
  // Check package manager prefix environment variable first (set by some configurations)
  if (process.env.npm_config_prefix) {
    return process.env.npm_config_prefix;
  }

  // Check common custom package manager prefix locations on Windows
  if (process.platform === 'win32') {
    // Custom global paths are often configured via package manager config
    // Check %APPDATA%\bun first (default Windows bun global)
    const appDataBun = process.env.APPDATA
      ? path.join(process.env.APPDATA, 'bun')
      : null;
    if (appDataBun && fs.existsSync(appDataBun)) {
      return appDataBun;
    }
  }
...
/**
 * Builds the list of paths to search for cli.js in package manager's node_modules.
 */
function getPackageManagerCliJsPaths(): string[] {
  const homeDir = os.homedir();
  const isWindows = process.platform === 'win32';
  const cliJsPaths: string[] = [];

  if (isWindows) {
    // Default bun global path on Windows
    cliJsPaths.push(
      path.join(homeDir, 'AppData', 'Roaming', 'bun', 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js')
    );

    // Package manager prefix from environment/config
    const pmPrefix = getPackageManagerGlobalPrefix();
    if (pmPrefix) {
      cliJsPaths.push(
        path.join(pmPrefix, 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js')
      );
    }

    // Common custom package manager global directories on Windows
    const programFiles = process.env.ProgramFiles || 'C:\\Program Files';
    const programFilesX86 = process.env['ProgramFiles(x86)'] || 'C:\\Program Files (x86)';

    // Check common nodejs installation paths with custom package manager global
    cliJsPaths.push(
    ...
    // Unix/macOS package manager global paths
    cliJsPaths.push(
      path.join(homeDir, '.bun', 'lib', 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js'),
      '/usr/local/lib/node_modules/@anthropic-ai/claude-code/cli.js',
      '/usr/lib/node_modules/@anthropic-ai/claude-code/cli.js'
    );

    // Check package manager prefix for custom global paths on Unix
    if (process.env.npm_config_prefix) {
      cliJsPaths.push(
...
export function findClaudeCLIPath(pathValue?: string): string | null {
  ...
  if (isWindows) {
    ...
    const cliJsPaths = getPackageManagerCliJsPaths();
  ...
  // Platform-specific search paths for native binaries and package manager symlinks
  const commonPaths: string[] = [
    ...
    // Package manager global bin symlinks (created by bun install -g)
    path.join(homeDir, '.bun', 'bin', 'claude'),
  ];

  // Also check package manager prefix bin directory
  const pmPrefix = getPackageManagerGlobalPrefix();
  if (pmPrefix) {
    commonPaths.push(path.join(pmPrefix, 'bin', 'claude'));
  }
