#!/usr/bin/env bun
// @bun v1.3.7+
// Standalone Performance Suite v2.1 - No external dependencies
...
async function parseRSS(url) {
	try {
		...
		const text = await fetch(url, {
			headers: { "User-Agent": "Tier-1380-Scanner/2.1" },
		}).then((r) => r.text());
...
// ─── Simple CSS Analyzer (No LightningCSS) ────────
/**
 * Analyze CSS file without external dependencies
...
async function analyzeCSS(file) {
	...
	try {
		...
		// Simple minification without LightningCSS
		const minified = code
		...
		const saved = ((1 - minified.length / code.length) * 100).toFixed(1);
