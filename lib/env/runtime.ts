...
export class RuntimeEnv {
  static read(): RuntimeEnvState {
    ...
    const allowFallbackRaw = String(process.env.PLAYGROUND_ALLOW_PORT_FALLBACK || '')
      .trim()
      .toLowerCase();
    const allowFallback =
      allowFallbackRaw === '1' ||
      allowFallbackRaw === 'true' ||
      allowFallbackRaw === 'yes' ||
      allowFallbackRaw === 'on'
        ? true
        : allowFallbackRaw === '0' ||
            allowFallbackRaw === 'false' ||
            allowFallbackRaw === 'no' ||
            allowFallbackRaw === 'off'
          ? false
          : false;
    ...
    const runtimeStaleMsRaw = Number.parseInt(
      process.env.PLAYGROUND_RUNTIME_STALE_MS || '15000',
      10
    );
    const wsBroadcastMsRaw = Number.parseInt(process.env.PLAYGROUND_WS_BROADCAST_MS || '1000', 10);
    const portRange =
      String(process.env.PLAYGROUND_PORT_RANGE || '').trim() ||
      (allowFallback
        ? `${config.port}-${Math.min(config.port + 20, 65535)}`
    ...
    return {
      ...
      runtimeOrigins,
      runtimeStaleMs:
        Number.isFinite(runtimeStaleMsRaw) &&
        runtimeStaleMsRaw >= 1000 &&
        runtimeStaleMsRaw <= 300_000
          ? runtimeStaleMsRaw
          : 15000,
      wsBroadcastMs:
