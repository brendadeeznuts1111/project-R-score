...
export class RuntimeEnv {
  static read(): RuntimeEnvState {
    ...
    const allowFallbackRaw = String(process.env.PLAYGROUND_ALLOW_PORT_FALLBACK || "").trim().toLowerCase();
    const allowFallback =
      allowFallbackRaw === "1" ||
    ...
    const portRange =
      ...
      (allowFallback ? `${config.port}-${Math.min(config.port + 20, 65535)}` : `${config.port}-${config.port}`);
    return {
      host: config.host,
      port: config.port,
      base: config.base,
