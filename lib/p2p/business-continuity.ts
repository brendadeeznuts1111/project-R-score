...
export interface BusinessSpecialty {
  type: 'barbershop' | 'coffee' | 'gym' | 'restaurant' | 'bookstore' | 'retail' | 'service' | 'other';
...
export class BusinessContinuity {
  ...
  static async registerBusinessIdentity(businessData: BusinessIdentity): Promise<string> {
    const businessId = Bun.hash(businessData.name + businessData.alias).toString(16).substring(0, 16);
    ...
    await redis.hmset(`business:${businessId}`, [
      ...
      'current', businessData.endDate ? 'false' : 'true',
      'contact', businessData.contact,
      'location', businessData.location || '',
      'paymentHandles', JSON.stringify(businessData.paymentHandles),
      'branding', businessData.branding ? JSON.stringify(businessData.branding) : '',
      'specialty', businessData.specialty ? JSON.stringify(businessData.specialty) : ''
    ]);
  ...
  private static async notifyCustomersOfChange(
    ...
    for (const key of customerKeys.slice(0, 100)) { // Limit to 100 for demo
