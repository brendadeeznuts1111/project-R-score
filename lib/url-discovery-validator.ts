...
class URLDiscoveryValidator {
  ...
  static async discoverURLs(): Promise<void> {
    ...
    const knownURLs = [
      ...
      { url: 'https://github.com/oven-sh/bun/issues', source: 'documentation-constants', type: 'github' },
      { url: 'https://github.com/oven-sh/bun/pulls', source: 'documentation-constants', type: 'github' },
    ...
    const foundURLs = [
      ...
      { url: 'https://bun.sh/docs/cli/runtime/networking/fetch', source: 'fetch-service', type: 'docs' },
      ...
      { url: 'https://bun.sh/docs/cli/api/http-server#metrics', source: 'trader-analyzer', type: 'docs' },
  ...
  static async generateReport(): Promise<void> {
    ...
    console.log(`   Success Rate: ${((results.valid / (results.total - results.skipped)) * 100).toFixed(1)}%`);
