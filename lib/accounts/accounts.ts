...
export class AccountSystem {
  private db: Database;
  private tenantId: string; // brand-ok

  constructor(tenantId: string = "operations", dbPath?: string) {
    this.tenantId = tenantId;
    if (dbPath) {
      this.db = new Database(dbPath);
    } else {
      this.db = new Database(`data/accounts-${tenantId}.db`, { create: true });
    }
  ...
  async create(
    node: Omit<TreeNode, "id" | "createdAt" | "status" | "totalLiquidity" | "totalAccounts" | "promotedAt" | "lastPlayAt">
    ...
    return {
      ...node, id, createdAt, status, promotedAt: null, lastPlayAt: null,
      totalLiquidity: 0, totalAccounts: 0, email: node.email, oidcSubject: node.oidcSubject,
    };
  }

  getById(id: string): TreeNode | null {
    const row = this.db.query("SELECT * FROM tree_nodes WHERE id = $id").get({ $id: id }) as Record<string, unknown> | null;
    return row ? this.deserialize(row) : null;
  }

  getByTelegram(telegramId: string): TreeNode | null {
    const row = this.db.query("SELECT * FROM tree_nodes WHERE telegram_id = $t").get({ $t: telegramId }) as Record<string, unknown> | null;
    return row ? this.deserialize(row) : null;
  }
  ...
  async canPromote(nodeId: string): Promise<{ eligible: boolean; reason?: string }> {
    ...
    if (node.type !== "agent") return { eligible: false, reason: "Only agents can become partners" };
  ...
  getTree(nodeId: string, depth = 0): TreeNode[] {
    ...
    while (queue.length > 0) {
      ...
      const children = this.db.query("SELECT id FROM tree_nodes WHERE parent_id = $p AND status != 'suspended'").all({ $p: id }) as { id: string }[];
  ...
  getNodesForExpert(expertId: string): TreeNode[] {
  ...
  private async notify(telegramId: string, lines: string[]): Promise<void> {
    const token = Bun.env.TELEGRAM_BOT_TOKEN;
    if (!token) return;
    await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ chat_id: telegramId, text: lines.join("\n"), parse_mode: "Markdown" }),
    });
  }
}
