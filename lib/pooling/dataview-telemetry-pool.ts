...
export class DataViewTelemetryPool extends TelemetryPool {
  ...
  async queryDataViewSessions(member: string = '*'): Promise<any[]> {
    ...
    try {
      ...
      const deserializedResults = results
        .map(row => {
          try {
  ...
  async batchInsertDataViewProfiles(
    ...
    try {
      ...
      const totalDataSize = profiles.reduce(
        (sum, p) => sum + DataViewProfileSerializer.getEstimatedSize(p.profile),
        0
      );

      // Enhanced metrics with GC impact tracking
      this.metrics.recordMetric(
        'batch_insert_dv_profiles',
        latency,
        totalDataSize,
        this.dvPool.length
      );
      ...
      if (gcBefore.available) {
        ...
        console.info(
          `   📊 Throughput: ${((profiles.length / latency) * 1000).toFixed(0)} profiles/sec`
        );
      }
  ...
  async getDataViewPoolStats(): Promise<any> {
    ...
    try {
      const profileCount = db.prepare('SELECT COUNT(*) as count FROM dv_profiles').get() as {
        count: number;
      };
      const totalSize = db.prepare('SELECT SUM(data_size) as size FROM dv_profiles').get() as {
        size: number;
      };
      const metricsCount = db.prepare('SELECT COUNT(*) as count FROM dv_metrics').get() as {
        count: number;
      };
  ...
  async exportDataViewData(): Promise<{ profiles: Uint8Array; metrics: Uint8Array }> {
    ...
    try {
      // Export profiles
      const profiles = db
        .prepare('SELECT binary_data FROM dv_profiles ORDER BY timestamp DESC')
        .all() as { binary_data: Uint8Array }[];
