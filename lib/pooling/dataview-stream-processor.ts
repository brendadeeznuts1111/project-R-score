...
export class DataViewStreamProcessor {
  ...
  *createZeroCopyChunks(largeBuffer: Uint8Array, chunkSize: number): Generator<Uint8Array> {
    ...
    console.info(
      `   🚀 Throughput: ${(((totalBytesProcessed / (endTime - startTime)) * 1000) / 1024 / 1024).toFixed(2)}MB/sec`
    );
  }
  ...
  async processLargeDataStream(
    ...
    console.info(
      `   💾 Memory delta: ${((endMemory.heapUsed - startMemory.heapUsed) / 1024 / 1024).toFixed(2)}MB`
    );
    console.info(
      `   🚀 Throughput: ${(((totalBytes / (endTime - startTime)) * 1000) / 1024 / 1024).toFixed(2)}MB/sec`
    );
  }
