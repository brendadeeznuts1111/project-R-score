    ...
    const bunDocs = [
      {
        ...
        description: 'Hash passwords using Argon2id, bcrypt, or scrypt with Bun\'s built-in password hashing API',
      ...
      },
      {
        ...
        codeExample: 'const server = Bun.serve({ port: 3000, fetch(req) { return new Response("Hello!"); } });'
      },
      {
      ...
      },
      {
        ...
        codeExample: 'const db = new Bun.Database(":memory:"); const query = db.query("SELECT * FROM users");'
      },
      {
  ...
  private async handleHttpRequest(request: JSONRPCRequest): Promise<JSONRPCResponse> {
    try {
      ...
      if (request.method === 'tools/list') {
        const tools = await this.server.request({ method: 'tools/list' }, { clientCapabilities: {} });
        return {
          jsonrpc: '2.0',
          id: request.id,
          result: tools,
