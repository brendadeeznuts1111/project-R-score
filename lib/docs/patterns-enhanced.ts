...
export const DOC_PATTERNS = {
  ...
  isSecretsUrl: (url: string): boolean => 
    url.includes('/runtime/secrets'),
  ...
  isValidBunUrl: (url: string): boolean => {
    try {
      ...
      return parsed.hostname.startsWith('bun.') && 
  ...
  generateBreadcrumbs: (url: string): Array<{ text: string; url: string }> => {
    ...
    for (const part of pathParts.slice(1)) { // Skip 'docs'
      currentPath += `/${part}`;
      breadcrumbs.push({
  ...
  cleanHash: (hash: string): string => {
    return hash.replace(/^#/, '').toLowerCase().replace(/[^a-z0-9-]/g, '-');
  },
...
export const DOC_VALIDATION = {
  ...
  validateSecretsDocs: (domain: 'sh' | 'com' = 'com'): {
    valid: boolean;
    missing: string[];
  ...
  validateAllLinks: async (urls: string[]): Promise<{
    valid: string[];
    invalid: string[];
