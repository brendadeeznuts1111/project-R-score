...
export const CRITICAL_PROOF_PATHS: readonly ProofPath[] = [
  ...
  },
  {
    ...
    claim:
      'Operations platform uses unified lib/operations schema, single DB factory, transactional play publish + telegram outbox',
    kinds: ['unit'],
    gateClass: 'human-only',
    gateRef: 'none',
    evidence: [
    ...
    freshRerun:
      'bun test tests/operations-schema.test.ts tests/play-dispatcher.test.ts tests/account-service.test.ts',
    freshRerunKind: 'claim',
  ...
  },
  {
    ...
    claim:
      'Per-tenant Telegram webhook validates secret token, routes commands, publishes channel events',
    kinds: ['unit'],
    gateClass: 'human-only',
    gateRef: 'none',
    evidence: [
