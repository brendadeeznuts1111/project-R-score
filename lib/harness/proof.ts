...
export const CRITICAL_PROOF_PATHS: readonly ProofPath[] = [
  ...
  },
  {
    ...
    gateClass: 'continuous',
    gateRef: 'ci:core',
    evidence: ['bun run proof:install', 'bun run install:verify'],
  ...
  },
  {
    ...
    gateClass: 'human-only',
    gateRef: 'none',
    evidence: ['bun run search:bench:gate'],
  ...
  // Behavior / intervention lives on CiRunbook; fail-closed coverage is ci-deploy-runbooks.
  {
    ...
    owner: 'scripts/ci-core.ts',
  },
  {
    id: 'typescript-ci-gate',
    claim: 'Local ownership of type-check:ci / type-check:full is cataloged',
    kinds: ['boundary'],
    gateClass: 'human-only',
    gateRef: 'none',
    evidence: [
    ...
    freshRerun: 'bun run docs:ci-deploy',
    freshRerunKind: 'catalog',
    owner: 'package.json · type-check:ci',
  },
  {
