...
| Fetch / content wire    | [`fetch-headers.ts`](fetch-headers.ts) · [`fetch-preconnect.ts`](fetch-preconnect.ts) · [`outbound-policy.ts`](outbound-policy.ts) · [`ip-authority.ts`](ip-authority.ts) · [`content-type.ts`](content-type.ts) · [`form-upload.ts`](form-upload.ts) · [`data-etag.ts`](data-etag.ts) · [`data-source.ts`](data-source.ts) · [`sha256.ts`](sha256.ts) | Outbound/inbound HTTP primitives                 |
...
Mega consumer: `scripts/serve-public.ts`.
...
| [`outbound-policy.ts`](outbound-policy.ts)                         | Exact-origin, method, credential-header, redirect, and timeout policy for outbound requests                                           |
| [`ip-authority.ts`](ip-authority.ts)                               | IPv4/IPv6 URI authority + opt-in DNS → IP connect → TLS SNI → HTTP Host proof                                                         |
