...
export function getFetchAPIDocsURL(options?: Omit<DocumentationURLOptions, 'provider' | 'category' | 'path'>): string {
...
export function getOptimalSyscall(useCase: string, options?: {
...
export function getAllDocumentationURLs(): Record<string, string> {
  ...
  return {
    ...
    internalWiki: ENTERPRISE_DOCUMENTATION_BASE_URLS[DocumentationProvider.INTERNAL_WIKI]?.BASE || 'https://wiki.internal.example.com'
  };
}
...
export const docs = {
  ...
  domains: {
    ...
    github: (path?: string) => `https://github.com/oven-sh/bun${path ? `/${path.replace(/^\//, '')}` : ''}`
  },
  ...
  routing: {
    ...
    getBestURL: (topic: string, userType: 'developers' | 'beginners' | 'educators' | 'all_users' = 'developers') => {
      return docsURLBuilder.buildTopicURL(topic, userType);
    },
    
    // Get alternative URLs for a topic
    getAlternatives: (topic: string, userType: 'developers' | 'beginners' | 'educators' | 'all_users' = 'developers') => {
