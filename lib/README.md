...
Inventory SSOT for agents — **indexes, no moves**. Domain folders stay where they
are; spine modules stay at `lib/<name>.ts`.
...
Every `lib/*.ts` module is listed. Grouped for scan; paths stay flat under `lib/`.
