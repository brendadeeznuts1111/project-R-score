...
Single hub for Bun CLI execution options, console depth / stdin, and the
utilities master table. The reviewed production pin is Bun **1.3.14**; the
executing operator runtime is observed separately and may be a canary. Wrapper
and declaration type channels are governed independently by
...
`bun.sh/docs/…` is the same Mintlify site as `bun.com/docs/…` — prefer
**bun.com** (repo SSOT / `bun-doc-refs`).

**Harness defaults:**
`import { stringWidth, stripANSI, wrapAnsi, sliceAnsi } from 'bun'` for TTY
...
`console.table` / `console.dir` / `util.inspect`
...
Canonical docs:
[General Execution Options](https://bun.com/docs/runtime#general-execution-options)
([`bun run` CLI](https://bun.com/docs/cli/run)). The executing command's
`bun run --help` is runtime authority; release-stamped probes preserve the
reviewed 1.3.14 contract while tip-only catalog entries remain observations
until promotion. Curated harvest for portal-cli:
...
**`--interactive` (docs gap):**
[General Execution Options](https://bun.com/docs/runtime#general-execution-options)
lists a Node-compat REPL (`node:repl`; with `-e`, eval then REPL — raw JS, not
TypeScript). Distinct from `bun repl` (Bun-native). **Not** printed by the
reviewed 1.3.14 runtime's `bun run --help`; treat as unverified until the
executing command help shows it.
...
- Child processes: pass `--console-depth=${getConsoleDepth()}` (or rely on
  bunfig).
- Do not put lifecycle `pre*` / `post*` on `check:harness-*` (steals stdin).
- Portal harvest / shortcodes stay in `runtime-flags.json` — this section is the
  full `bun run` surface.
...
This repo: `bun test tests/console-depth.test.ts` · claim
...
From the [reference index](https://bun.com/reference) (sidebar / cards). Prefer
the **Bun** module for harness code; use `bun:*` for specialized surfaces; treat
`node:*` as compatibility (harness bans `node:http` / `node:https` under `lib/`
/ `scripts/` / `tools/` / …).
...
Paths under `/reference/bun/<submodule>` match the live types site (not
`bun-jsc` hyphen forms).
