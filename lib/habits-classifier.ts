 ...
 */
export async function storeHabits(userId: string, habits: HabitsData, ttlSeconds = 86400): Promise<void> {
  const key = `habits:${userId}`;
  await redis.set(key, JSON.stringify(habits), 'EX', ttlSeconds);
  
  // Publish classification event
  await redis.publish('HABITS_CLASSIFIED', JSON.stringify({
    userId,
    habits,
...
if (import.meta.main) {
  ...
  for (const test of testCases) {
    ...
    console.log(`Tier: ${habits.tier.padEnd(12)} | Txns: ${habits.txnCount} | Avg: $${habits.avgTxn.toFixed(2)} | Bonus on $100: $${bonus.bonus.toFixed(2)}`);
  }
}
