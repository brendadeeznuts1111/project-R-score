// @see https://bun.com/docs/runtime/file-io#reading-files-bun-file — Bun.file (types only; I/O in partner-ops-registry)
/**
 * Partners-ops **projection** — pure-ish types, classifiers, validation, and
 * registry assembly from already-loaded sources (no seat/handshake disk reads).
 *
 * Bake I/O (load events, SQLite ledger, write partners-ops.json) lives in
 * `partner-ops-registry.ts` which composes this module.
 *
 * @see ./partner-ops-registry.ts
...
import type { PartnerLedgerRow } from '../partner-profile/ledger';
...
export type BookTypeWire =
  'crypto' | 'pph' | 'legal-us' | 'sweepstakes' | 'exchange' | 'offshore' | 'legal';
export type DepositMethodKey =
  'venmo' | 'crypto' | 'wire' | 'credit' | 'cashapp' | 'paypal' | 'zelle' | 'apple_pay' | 'unknown';
...
/** Wire sources already loaded (disk/SQLite I/O is the registry facade). */
export type PartnersOpsProjectSources = {
  seatRows: SeatDeskRow[];
  handshakeRows: HandshakeRow[];
  taxonomy: { bookRegistry?: BookRegistryRow[] } | null;
  events: PartnerOpsEvent[];
  ledgerSnapshots: Map<string, PartnerLedgerSnapshot>;
  /** Partner CODE (upper) → forum metadata (chatId + topics). */
  forumByCode: Map<
    string,
    {
      chatId?: string | number | null; // brand-ok — telegram chat wire
      topicsThreadMap?: Record<string, number | null | undefined>;
    }
  >;
...
/**
 * Project partners-ops v2 registry from in-memory sources (no disk reads).
 * Callers load seat/handshake/taxonomy/events/ledger/forum then invoke this.
 */
export function projectPartnersOpsRegistry(
  sources: PartnersOpsProjectSources
): PartnersOpsRegistry {
  const {
    seatRows,
    handshakeRows,
  ...
  for (const row of seatRows) {
    ...
    const forumMeta = forumByCode.get(code);
