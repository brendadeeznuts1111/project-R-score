...
/**
 * Shared synthetic `[data-fw-limit]` HTML → LimitObservation[] (Tier 4 books).
 *
 * @see docs/WIRE_BOUNDARY.md
 * @see docs/harness/tenants/partner-limits.md
 */

import { asStateCode, tryStateCode, type SportsbookId } from './domain.ts';
...
export type FwLimitHtmlMode = Extract<LimitObservation['mode'], 'html_fixture' | 'html_live'>;

export type ParseFwLimitHtmlOptions = {
  sportsbook: SportsbookId;
  agent: string;
  observedAt: string;
  mode: FwLimitHtmlMode;
...
function attrsToObservation(
  attrs: RawLimitAttrs,
  options: ParseFwLimitHtmlOptions
  ...
  const book = String(options.sportsbook);
  ...
  return {
    ...
    sourceRef: `scrape:html/${book}-${jurisdiction.toLowerCase()}-${sport}-${market}-${structure}-${phase}-${stamp}`,
...
/**
 * Parse synthetic `[data-fw-limit]` HTML at the wire boundary.
 * Empty / malicious input → empty array (fail closed).
 */
export async function parseFwLimitHtml(
  html: string,
  options: ParseFwLimitHtmlOptions
