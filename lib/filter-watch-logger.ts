...
class FilterWatchLogger {
  ...
  async stopSession(sessionId: string): Promise<void> {
    ...
    console.info(
      `🛑 Stopped watch session: ${sessionId} (duration: ${Date.now() - session.startTime}ms)`
    );
  }
  ...
  getSessionStats(): {
    ...
    const averageDuration =
      sessions.length > 0
        ? sessions.reduce((sum, session) => sum + (Date.now() - session.startTime), 0) /
          sessions.length
        : 0;
...
if (process.argv[1] && process.argv[1].endsWith('filter-watch-logger.ts')) {
  ...
  setTimeout(() => {
    recordWatchChange(session.id, 'package_modified', 'app-web', {
      file: 'package.json',
      change: 'version bump',
    });
  }, 2000);
