...
export const SYSCALLS = {
  ...
  LINUX: {
    COPY_FILE_RANGE: { name: 'copy_file_range', risk: 1.0000005, desc: 'Zero-copy file clone' },
    SENDFILE: { name: 'sendfile', risk: 1.000001, desc: 'Kernel pipe transfer' },
    SPLICE: { name: 'splice', risk: 1.0000015, desc: 'Pipe-to-pipe zero-copy' },
    WRITE: { name: 'write', risk: 1.000002, desc: 'Standard buffered write' },
  },
  // macOS - fastest to slowest
  DARWIN: {
    CLONEFILE: { name: 'clonefile', risk: 1.0000003, desc: 'APFS copy-on-write' },
    FCOPYFILE: { name: 'fcopyfile', risk: 1.0000008, desc: 'Existing file CoW' },
    WRITE: { name: 'write', risk: 1.000002, desc: 'Standard write' },
  },
} as const;
...
export class Tier1380SyscallProfiler {
  ...
  async profileOperation(input: any, output: any): Promise<SyscallMetrics> {
  ...
  private detectSyscall(input: unknown, output: unknown): { syscall: string; baseRisk: number } {
    if (this.platform === 'linux') {
      const isBunFile = (obj: unknown): boolean =>
        obj instanceof Blob && typeof (obj as Record<string, unknown>).name === 'string';
      if (isBunFile(input) && isBunFile(output)) {
        return { syscall: 'copy_file_range', baseRisk: 1.0000005 };
      }
      if (isBunFile(input) && output === Bun.stdout) {
        return { syscall: 'sendfile', baseRisk: 1.000001 };
      }
      if (input instanceof Response) {
        return { syscall: 'splice', baseRisk: 1.0000015 };
      }
      return { syscall: 'write', baseRisk: 1.000002 };
    } else {
      // macOS
      const isBunFile = (obj: unknown): boolean =>
        obj instanceof Blob && typeof (obj as Record<string, unknown>).name === 'string';
      if (isBunFile(input) && isBunFile(output)) {
        ...
        return { syscall: 'clonefile/fcopyfile', baseRisk: 1.0000003 };
      }
      return { syscall: 'write', baseRisk: 1.000002 };
    }
  }
