...
export class BunWikiIntegration {
  ...
  async generateWikiPages(): Promise<WikiCategory[]> {
    ...
    for (const docCategory of docIndex.categories) {
      ...
      if (!docCategory?.name) {
        logWarning(
          'generateWikiPages',
          `Invalid category found, skipping: ${JSON.stringify(docCategory)}`
        );
        continue;
      }
   ...
   */
  private async convertToWikiCategory(
    docCategory: DocumentationCategory
  ): Promise<WikiCategory | null> {
    // Validate input
    if (!docCategory?.name) {
    ...
    return {
      ...
      pages: wikiPages,
      subcategories: docCategory.subcategories
        ? await this.convertSubcategories(docCategory.subcategories)
        : [],
    };
  }
   ...
   */
  private async convertSubcategories(
    docSubcategories: DocumentationCategory[]
