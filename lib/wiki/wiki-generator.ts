...
async function createWikiFiles(config: Partial<WikiConfig> = {}): Promise<void> {
  ...
  console.info('Generating wiki files...');
  ...
  await AtomicFileOperations.writeAtomic(`${outputDir}/.gitkeep`, '');
...
*Generated by Bun Wiki URL Generator*`;
  ...
  await AtomicFileOperations.writeAtomic(`${outputDir}/README.md`, readmeContent);
...
async function main(): Promise<void> {
  console.info('Bun Wiki URL Generator');
  console.info('='.repeat(50));

  const options = parseCLIArgs();

  if (options.category && options.utility) {
    console.info(`Generating utility page: ${options.category}.${options.utility}`);
    ...
    await AtomicFileOperations.writeAtomic(`./internal-wiki/${fileName}`, pageContent);
