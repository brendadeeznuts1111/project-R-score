...
async function executeFilteredScript(
  ...
  const executionMode = flags.parallel ? 'parallel' : flags.sequential ? 'sequential' : 'parallel';

  const results: Array<{ pkg: (typeof packages)[0]; success: boolean; output: string }> = [];

  if (executionMode === 'parallel') {
    const promises = matched.map(pkg => runPackageWithOutputLimit(pkg, script, maxLines, flags));
...
async function runOnce(file: string, args: string[], flags: BunCLIFlags): Promise<number> {
...
function buildCommand(base: string[], flags: BunCLIFlags): string[] {
  ...
  if (flags.define) {
    Object.entries(flags.define).forEach(([k, v]) => cmd.push('--define', `${k}:${v}`));
  }
    ...
    cmd:
      shell === 'bun'
        ? [
            ...
            'run',
            '-e',
            `require('child_process').execSync('${cmd} ${args.join(' ')}', {stdio: 'inherit'})`,
          ]
...
export function parseOfficialFlags(args: string[]): {
  flags: BunCLIFlags;
  command: string;
  args: string[];
} {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case '--filter':
        ...
        i++;
        break;
      case '--filter-output-lines':
        flags.filterOutputLines = parseInt(next);
        i++;
        break;
      case '--ws':
        flags.ws = true;
      ...
      case '--bun':
        ...
        break;
      case '--shell':
        flags.shell = next as 'bun' | 'system';
        i++;
        break;
      case '--smol':
        flags.smol = true;
      ...
      case '--inspect':
        ...
        break;
      case '--inspect-brk':
        flags.inspectBrk = true;
        break;
      case '-r':
      case '--preload':
        flags.preload = [...(flags.preload || []), next];
        i++;
        break;
      case '--no-install':
        ...
        break;
      case '--install':
        flags.install = next as 'auto' | 'fallback' | 'force';
        i++;
        break;
      case '--tsconfig':
        flags.tsconfig = next;
      ...
      case '--port':
        flags.port = parseInt(next);
        i++;
        break;
      case '--preconnect':
        flags.preconnect = [...(flags.preconnect || []), next];
        i++;
        break;
      case '--dns':
        flags.dns = next as 'verbatim' | 'ipv4first' | 'ipv6first';
        i++;
        break;
      case '--env-file':
        flags.envFile = [...(flags.envFile || []), next];
        i++;
        break;
      case '--cwd':
        flags.cwd = next;
