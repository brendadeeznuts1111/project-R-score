...
export class Tier1380PasswordSecurity {
  ...
  private static needsRehash(hash: PasswordHash): boolean {
    ...
    switch (hash.algorithm) {
      case 'argon2id':
        return (hash.metadata?.memoryCost || 0) < 65536 || 
               (hash.metadata?.timeCost || 0) < 3;
   ...
   */
  private static async storePasswordHash(
    userId: string, 
    hash: PasswordHash
  ): Promise<void> {
   ...
   */
  private static async addToPasswordHistory(
    userId: string, 
    hash: PasswordHash
  ): Promise<void> {
  ...
  private static calculatePasswordScore(password: string): number {
    ...
    const varietyScore = [hasUpper, hasLower, hasNumbers, hasSpecial]
    ...
    if (userId) {
      ...
      if (hash) {
        ...
        if (ageInDays > 90) {
          ...
          report.recommendations.push(`User ${userId}: Password expired ${Math.floor(ageInDays - 90)} days ago`);
        }
        
        // Check hash strength
        if (hash.algorithm === 'bcrypt' && (hash.metadata?.cost || 0) < 10) {
          report.weakPasswords++;
          report.recommendations.push(`User ${userId}: bcrypt cost factor too low (${hash.metadata?.cost})`);
        }
      }
    }
...
async function main() {
  ...
  switch (command) {
    case 'hash':
      ...
      if (!password) {
        console.error('Usage: bun run enterprise-password-security.ts hash <password> [argon2id|bcrypt]');
        process.exit(1);
      }
