...
expectTypeOf(classifyAssetGroup).parameters.toEqualTypeOf<[mimeType: string, isFirstRequest: boolean]>();
...
describe("DocumentationMapper", () => {
  describe("getProvider", () => {
    test("should identify bun.sh URLs as the official Bun provider", () => {
      expect(DocumentationMapper.getProvider("https://bun.sh/docs/api/fetch")).toBe(DocumentationProvider.BUN_OFFICIAL);
    });

    test("should identify oven-sh/bun GitHub URLs as the Bun GitHub provider", () => {
      expect(DocumentationMapper.getProvider("https://github.com/oven-sh/bun/issues/123")).toBe(DocumentationProvider.BUN_GITHUB);
    });

    test("should identify npmjs.com/package/bun as the Bun NPM provider", () => {
      expect(DocumentationMapper.getProvider("https://www.npmjs.com/package/bun")).toBe(DocumentationProvider.BUN_NPM);
    });

    test("should identify stackoverflow.com as Stack Overflow provider", () => {
      expect(DocumentationMapper.getProvider("https://stackoverflow.com/questions/123")).toBe(DocumentationProvider.STACK_OVERFLOW);
    });

    test("should fall back to COMMUNITY for unrecognized domains", () => {
      expect(DocumentationMapper.getProvider("https://some-blog.dev/bun-tips")).toBe(DocumentationProvider.COMMUNITY);
    });
  ...
  describe("getCategory", () => {
    test("should classify /docs/api paths as the API category", () => {
      expect(DocumentationMapper.getCategory("https://bun.sh/docs/api/fetch")).toBe(DocumentationCategory.API);
    });

    test("should classify /docs/cli paths as the CLI category", () => {
      expect(DocumentationMapper.getCategory("https://bun.sh/docs/cli/run")).toBe(DocumentationCategory.CLI);
    });

    test("should classify /docs/runtime paths as the RUNTIME category", () => {
      expect(DocumentationMapper.getCategory("https://bun.sh/docs/runtime/loaders")).toBe(DocumentationCategory.RUNTIME);
    });
    ...
    test("should fall back to REFERENCE for unrecognized paths", () => {
      expect(DocumentationMapper.getCategory("https://bun.sh/about")).toBe(DocumentationCategory.REFERENCE);
    });
  });
  ...
  describe("getUrlType", () => {
    ...
    test("should classify GitHub /issues/ paths as GITHUB_ISSUE type", () => {
      expect(DocumentationMapper.getUrlType("https://github.com/oven-sh/bun/issues/5")).toBe(UrlType.GITHUB_ISSUE);
    });

    test("should classify GitHub /pull/ paths as GITHUB_PULL_REQUEST type", () => {
      expect(DocumentationMapper.getUrlType("https://github.com/oven-sh/bun/pull/42")).toBe(UrlType.GITHUB_PULL_REQUEST);
    });

    test("should classify YouTube /watch paths as VIDEO_TUTORIAL type", () => {
      expect(DocumentationMapper.getUrlType("https://youtube.com/watch?v=abc")).toBe(UrlType.VIDEO_TUTORIAL);
    });

    test("should fall back to GITHUB_SOURCE for generic GitHub repository URLs", () => {
      expect(DocumentationMapper.getUrlType("https://github.com/oven-sh/bun")).toBe(UrlType.GITHUB_SOURCE);
    });
