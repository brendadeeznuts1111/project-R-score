...
// 5. registerTest
{
  ...
  results.push(bench("registerTest (2 variants)", () => {
...
{
  const m = new ABTestManager();
  results.push(bench("registerTest (3 variants, weighted)", () => {
    m.registerTest({ id: "bench-w", variants: ["a", "b", "c"], weights: [50, 30, 20] });
  }));
}

// 7. getVariant (first call — assigns)
results.push(bench("getVariant (assign new)", () => {
  const m = new ABTestManager();
...
// 8. getVariant (subsequent — reads cookie)
{
  ...
  results.push(bench("getVariant (cached read)", () => {
    m.getVariant("cached");
  }));
}

// 9. getVariant from pre-set cookie
results.push(bench("getVariant (from cookie header)", () => {
...
// 10. forceAssign
{
  ...
  results.push(bench("forceAssign", () => {
    m.forceAssign("fa", "b");
  }));
}
...
// 11. getAllAssignments
{
  ...
  results.push(bench("getAllAssignments (5 tests)", () => {
    m.getAllAssignments();
  }));
}
...
// 12. getSetCookieHeaders
{
  ...
  results.push(bench("getSetCookieHeaders (5 cookies)", () => {
    m.getSetCookieHeaders();
  }));
}

// 13. Full request cycle: parse → assign → serialize
results.push(bench("Full cycle (parse→assign→serialize)", () => {
...
results.push(bench("Legacy wrapper full cycle", () => {
...
// 15. clear
{
  ...
  m.getVariant("clr");
  results.push(bench("clear (single test)", () => {
    m.clear("clr");
  }));
}
...
console.log(`  Cached variant read: ${cachedRead?.["ns/op"]}ns/op (${cachedRead?.["ops/s"]} ops/s)`);
