...
declare module 'bun' {
  interface Env {
    ...
    /**
     * GitHub / Bun create env keys — names SSOT in `lib/github-repository-ref.ts` (`BUN_GITHUB_ENV`).
...
export type CacheFeature =
  'REDIS_CACHE' | 'MEMORY_CACHE' | 'DISTRIBUTED_CACHE' | 'NO_CACHE' | 'PERSISTENT_CACHE';

export type AuditFeature =
  'FULL_AUDIT' | 'SECURITY_AUDIT' | 'PERFORMANCE_AUDIT' | 'COMPLIANCE_AUDIT' | 'MINIMAL_AUDIT';

export type AuthFeature = 'AWS_SIGV4' | 'BASIC_AUTH' | 'OAUTH2' | 'MTLS' | 'API_KEY' | 'JWT_TOKEN';

export type StorageFeature =
  'R2_STORAGE' | 'S3_STORAGE' | 'LOCAL_STORAGE' | 'HYBRID_STORAGE' | 'BACKUP_STORAGE';

export type MonitoringFeature =
  'PROMETHEUS' | 'GRAFANA' | 'CLOUDWATCH' | 'SENTRY' | 'CUSTOM_MONITORING';
