...
import { scopedService, loadScopedSecrets } from '../../utils/secrets-loader';
import { BunR2AppleManager } from '../../src/storage/r2-apple-manager';
import { parseCliFilters, filterData } from '../../utils/cli-filter';
import { BULK_CONFIG, CLI_FILTER } from '../../config/constants';
...
async function checkSecrets() {
  ...
  log('\n🔐 Bun Secrets (Experimental):');
  
  const keys = ['R2_BUCKET', 'DUOPLUS_API_KEY', 'R2_ENDPOINT', 'R2_ACCESS_KEY_ID', 'R2_SECRET_ACCESS_KEY'];
  for (const key of keys) {
    const val = await secrets.get({ service: SERVICE, name: key });
    const exists = val || isSim;
    log(`  ${key}: ${exists ? '✅ ' + (val ? '***STORED*** (Keychain)' : '***SIMULATED***') : '⚠️ Missing - run utils/secrets-loader.ts'}`);
    if (exists) secretsScore++;
  }

  // Cross-platform scoping test (+2 points)
  const testScoped = await scopedService('test-team');
  let testCred = await secrets.get({ service: testScoped, name: 'R2_BUCKET' });
  ...
  if (!testCred && isInteractive) {
    await secrets.set({ service: testScoped, name: 'R2_BUCKET', value: 'verify-scoping-success' });
    testCred = await secrets.get({ service: testScoped, name: 'R2_BUCKET' });
    log('  🔄 Created test scoping secret');
  }

  log(`Scoped Test: ${testCred ? '✅ Per-user ENTERPRISE' : '⚠️ Set via loader'}`);
  if (testCred) {
    secretsScore += 2;
    log('Scoping: 2/2');
  }
  ...
  if (testCred && isInteractive) {
    await secrets.delete({ service: testScoped, name: 'R2_BUCKET' });
  ...
  for (const [envKey, secretName] of Object.entries(envToSecretMap)) {
    if (Bun.env[envKey] && !(await secrets.get({ service: SERVICE, name: secretName }))) {
      await secrets.set({ service: SERVICE, name: secretName, value: Bun.env[envKey]! });
...
async function enhancedSetupCheck() {
  ...
  results.totalMs = totalMs.toFixed(0);
  results.timestamp = new Date().toISOString();
  Bun.write('setup-results.json', JSON.stringify(results, null, 2));
  log('\n📄 Results exported to setup-results.json');
}
