...
import { DuoPlusOrchestrator, DuoPlusDevice } from './duoplus-orchestrator';
...
export class MultiAppOrchestrator {
  ...
  private duoPlus: DuoPlusOrchestrator;
  ...
  constructor(
    ...
    this.duoPlus = new DuoPlusOrchestrator({
      ...duoplusOptions,
  ...
  async getUnifiedProfile(phone: string): Promise<UnifiedProfile> {
    ...
    try {
      ...
      const trustScore = this.calculateUnifiedTrustScore({
        ...
        duoPlus: duoPlusDevice ? this.getDuoPlusTrustScore(duoPlusDevice) : 500,
  ...
  private getDuoPlusTrustScore(device: DuoPlusDevice): number {
