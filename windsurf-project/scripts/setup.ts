...
beforeAll(async () => {
  ...
  config.getEndpoint('analytics').grafana.url = 'config.getEndpoint('dashboard').local';
  ...
  globalThis.testUtils = {
    ...
    createMockConfig: () => ({
      ...
      grafana: { url: 'config.getEndpoint('dashboard').local', apiKey: 'test-key', dashboardFolder: 'test' },
      notifications: {
        slack: { webhookUrl: 'https://hooks.slack.com/test', channel: '#test' },
        email: { smtp: { host: 'localhost', port: 587, secure: false }, from: 'test@example.com' }
      },
