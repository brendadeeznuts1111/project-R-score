...
**Validation Features**:
- ✅ YAML/JSON parsing validation (uses `Bun.YAML.parse()`)
- ✅ Required sections check (metadata, ui_context_defaults, feature_flags, html_rewriter_policies)
- ✅ Type validation (booleans, arrays, objects)
- ✅ Schema version check (validates against 8.0.0.0.0.0.0)
- ✅ Warning for missing optional fields
- ✅ Detailed error messages with file paths
- ✅ Summary statistics (feature flags count, policies count)

**Example Output**:
```
🔍 Validating UI Policy Manifest: config/ui-policy-manifest.yaml
