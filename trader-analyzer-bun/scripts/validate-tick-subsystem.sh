...
if bun run bench:ticks --duration=10s --rate=50000 2>&1 | tee /tmp/bench-result.txt | grep -q "Ingestion rate"; then
    INGESTION_RATE=$(grep -oP 'Ingestion rate: \K[\d,]+' /tmp/bench-result.txt | tr -d ',' || echo "0")
...
if [ -f "test/ticks/correlation-accuracy.test.ts" ]; then
    if bun test test/ticks/correlation-accuracy.test.ts 2>&1 | tee /tmp/test-result.txt | grep -q "passing"; then
        PASS_RATE=$(grep -oP 'passing: \K\d+%' /tmp/test-result.txt | sed 's/%//' || echo "0")
...
if bun run query:benchmark --query="getRecentTicks" --iterations=1000 2>&1 | tee /tmp/query-result.txt | grep -q "p50"; then
    P50_LATENCY=$(grep -oP 'p50: \K[\d.]+' /tmp/query-result.txt | head -1 || echo "1000")
...
if [ -f "test/ticks/fuzz-correlation.test.ts" ]; then
    if bun test --fuzz-runs=10000 test/ticks/fuzz-correlation.test.ts 2>&1 | tee /tmp/fuzz-result.txt | grep -q "passing"; then
        CRASH_COUNT=$(grep -c "CRASH" /tmp/fuzz-result.txt 2>/dev/null || echo "0")
...
if bun run test:circuit-breaker --overload --duration=30s 2>&1 | tee /tmp/cb-result.txt | grep -q "Trip time"; then
    TRIP_TIME=$(grep -oP 'Trip time: \K\d+' /tmp/cb-result.txt | head -1 || echo "10000")
...
if bun run test:layer4-ticks --market=NFL-2025-001 --target=NBA-2025-001 2>&1 | tee /tmp/layer4-result.txt | grep -q "Signal strength"; then
    SIGNAL_STRENGTH=$(grep -oP 'Signal strength: \K[\d.]+' /tmp/layer4-result.txt | head -1 || echo "0")
...
if bun run mlgs.system.verifyRouterIntegration 2>&1 | tee /tmp/ready-result.txt | grep -q "Status"; then
    READY_STATUS=$(grep -oP 'Status: \K\w+' /tmp/ready-result.txt | tail -1 || echo "NOT_READY")
