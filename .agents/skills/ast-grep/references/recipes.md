...
> **This monorepo:** see [monorepo.md](monorepo.md) for workspace-specific paths, `map`, and bundled scan rules.
...
## This monorepo (Bun / sports-terminal / kimi-plugin)

Verified against `/Users/nolarose/Projects`:

```bash
AG=.agents/skills/ast-grep/scripts/ast_grep_helper.py

# Repo orientation (repo-map.json)
python3 $AG map
...
- `references/monorepo.md` — workspace paths and agent workflow.
