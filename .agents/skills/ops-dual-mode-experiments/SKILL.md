...
description: >-
  ...
  experiments (FactorialEngine, phases, switchback, cluster), coverage
  prediction backtests, and champion/challenger shadow eval on FactoryWager
  operations. Use when working on provisioning queues, sandbox-gated WebView
  signup, experiment variants/phases, prediction_accuracy, ops-summary C4/C5
  panels, or ops:diagnose / ops:snapshot freshness.
---
...
- Provisioning queue / WebView signup / sandbox gate / `is_test` coverage
  exclude
...
Deep maps (do not duplicate here):
[`lib/experiments/README.md`](../../../lib/experiments/README.md) ·
[`lib/prediction/README.md`](../../../lib/prediction/README.md) ·
[`lib/provisioning/README.md`](../../../lib/provisioning/README.md) ·
[`docs/harness/ops-summary-endpoint.md`](../../../docs/harness/ops-summary-endpoint.md)
...
2. **Claim lane** — write only the allowlist above; name the split in the
   commit.
3. **Sandbox / policy** — live books fail closed; system factors are not
   partner-randomized.
4. **Prove** — lane tests + CLI `--help` smoke (below).
5. **Freeze for Pages** — `bun run ops:snapshot` when C4/C5 panels must ship;
   triage with `bun run ops:diagnose`.
...
| `PROVISION_USERNAME`        | per-invocation sandbox username; inject through the environment |
| `PROVISION_PASSWORD`        | per-invocation sandbox password; inject through the environment |
| `PROVISION_EMAIL`           | per-invocation sandbox email; inject through the environment    |
...
Live books must fail closed. Test accounts set
`partner_platform_accounts.is_test = 1` and are excluded from coverage “covered”
counts.

Provisioning retries are operator-controlled because signup submission is not
idempotent. `retry --id=<task>` returns a failed task to `pending` only below
the retry ceiling; the worker never retries a failed signup automatically.
...
Variant config may set: `min_coverage_pct` · `coverage_floor` ·
`minPlatformCoverage`.

### CLI (sandbox)

Production defaults: **10** partners/cell, **28**-day min duration
(`OPS_EXPERIMENT_SANDBOX=1` or `--sandbox` relaxes).
...
`dailyCheck` harm pause is **operational** (mean gap + min n), not statistical
significance. System-scoped factors (`model`, `automation_frequency`, …) →
champion/challenger shadow, not partner randomization.
...
**Local:** `bun run serve:public` → `/api/operations/summary` is **live**
`buildOpsSummary`. **Pages:** snapshot only — freeze with `bun run ops:snapshot`
then deploy `public/`. Empty liquidity/plays with green proofs = empty DB, not a
broken API.
