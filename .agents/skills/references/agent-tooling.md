...
Shared gates and MCP tools for all `.agents/skills/*` skills. Prefer MCP over
shell when the `ast-grep` server is enabled in the workspace authority
`.mcp.json`.
...
bun scripts/pre-commit-ast-grep.ts --changed  # ast-grep paths differ from HEAD
bun scripts/pre-commit-ast-grep.ts --staged   # same logic as husky hook
...
Continuous scan + drift + pluggable effects (`log`, `alert`, `fix`, `report`,
custom):
...
Effect flags: `--effect alert.url=...`, `--effect fix`,
`--effects-dir effects --effect custom`
...
Registry: `skill-loop-registry.json` · presets: `quick`, `full`, `ci`,
`close-loop`
...
Bake both: `bun run ops:snapshot --no-routing` (skills section). Do not conflate
Kimi `cascade-mover` with repo `cascade-mover-v3`.
...
Reload MCP after skill changes:
`cd .agents/skills/ast-grep && ./scripts/verify-mcp.sh`
...
3. Let husky run — fix failures, re-stage, new commit (no `--amend` unless
   asked)

On hook failure, read stderr for which gate failed (hygiene · harness · rules ·
semver · packages).
