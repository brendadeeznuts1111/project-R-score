...
/**
 * Get webhook secret from OS credential store or env fallback
 */
async function getWebhookSecret(provider: 'paypal' | 'venmo' | 'cashapp'): Promise<string | null> {
  // Try Bun.secrets first (OS-level secure storage)
  try {
...
/**
 * Store webhook secret in OS credential store
 */
...
// ============================================================================
// P2P Proxy Class with Bun-Native Crypto
// ============================================================================

class P2PProxy {
  /**
   * Verify webhook signature using Bun.CryptoHasher (native HMAC)
   */
  ...
  /**
   * Generate stealth user ID using Bun.hash (fast, non-cryptographic)
   * For cryptographic hashing, use Bun.CryptoHasher instead
   */
  ...
  /**
   * Calculate bonus based on user habits
   */
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    if (url.pathname === '/admin/secret' && req.method === 'POST') {
      try {
        const { provider, secret } = JSON.parse(body);
        await storeWebhookSecret(provider, secret);
...
<body>
  ...
  <div class="bun-native">
    ...
    <ul style="text-align: left; display: inline-block;">
      <li>✅ <code>Bun.CryptoHasher</code> - HMAC signature verification</li>
      <li>✅ <code>Bun.secrets</code> - OS-level secret storage</li>
      <li>✅ <code>Bun.hash</code> - Fast stealth ID generation</li>
